...
* @copyright  2019 Rekire.com All right reserved
* @license    Rekire
...
class rkrregistroampliado extends Module
{
    ...
    public function install()
    {
        ...
        // Notificación
        $all_contacts = array('guardias@rekire.com');
    ...
    public function installTab()
    {
        $tab = new Tab();
        $tab->active = 1;
		$tab->class_name='AdminNombreControlador';	//nombre del controlador al que llama
        $tab->name = array();
        foreach (Language::getLanguages(false) as $lang) {
            $tab->name[$lang['id_lang']] = 'a Mostrar en menú';
        }
        ...
        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            ...
            $tab->id_parent = (int)Db::getInstance(_PS_USE_SQL_SLAVE_)
                                ->getValue('SELECT MIN(id_tab)
                                        ...
                                        ); //AdminParentOrders(en menú pedidos), mas class_name en la tabla ps_tab
        } else {
            // AdminAdmin
    ...
    public function uninstall()
    {
        ...
        if (!parent::uninstall() || !Configuration::deleteByName('rkrregistroampliado') || !$this->uninstallTab()){
          return false;
        }
    ...
    {
        $id_tab = (int)Tab::getIdFromClassName('AdminNombreControlador');
        if ($id_tab) {
            $tab = new Tab($id_tab);
            return $tab->delete();
        } else {

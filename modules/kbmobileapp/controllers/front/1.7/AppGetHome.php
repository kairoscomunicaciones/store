...
//        $this->content['newproducts'] = $this->getNewProductsList();
        ...
        if (Configuration::get('KB_MARKETPLACE') !== false && Configuration::get('KB_MARKETPLACE') == 1 && $hyperlocal_enable && isset($settings['kbmp_enable_hyperlocal']) && $settings['kbmp_enable_hyperlocal']) {
            $hyperlocal_category = array(
                ...
                'second_children' => array());
            $this->content['Menu_Categories'][] = $hyperlocal_category;
        }
        ...
        if ($results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($query)) {
            if (!empty($results)) {
                foreach ($results as $res) {
                    ...
                    if($res['redirect_activity'] == 'layout') {
                        $target_id = '';
                    }
                    $slides[] = array(
                    ...
                    if (is_array($banner_data)) {
                        ...
                        if ($number_of_entries > 0) {
                            ...
                            foreach ($banner_data as $k => $bd) {
                                ...
                                /* changes made by vibhaas */  else if ($bd['redirect_activity'] == 'product') 
                                    {
                                    $data['target_id'] = $bd['product_id'];
                                }
                                else

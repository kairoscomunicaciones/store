...
//    }
    ...
    {
        $data = 0;
        $query = 'SELECT count(*) FROM ' . _DB_PREFIX_ . 'kbmobileApp_unique_verification where id_shop = '. (int)$id_shop.' AND fid = "'. pSQL($user_data['unique_fingerprint_id']).'" AND id_customer = '. (int) $customer['id_customer'];
        $data = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($query);
        return $data;
    }
    /* Chnages over */

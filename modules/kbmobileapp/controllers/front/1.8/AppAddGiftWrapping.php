 ...
 * Description
 *
 * API to add wrapping cost into cart
 * Called from cart page in APP
 */

require_once 'AppCore.php';

class AppAddGiftWrapping extends AppCore
{
    /**
     * This function is trigger whenever this class is called in API call
    ...
    /**
     * Validate Cart and return its data
     */
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            $this->content['message'] = '';
            $this->writeLog('Gift Wrapping applied to the cart');
            $this->context->cart->update();
            ...
            if ($this->context->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS) < $minimal_purchase) {
                    $this->content['minimum_purchase_message'] = sprintf(
                        parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('A minimum purchase total of %1s is requi'
                                    . 'red to validate your order, current total is %2s '),
                            'AppAddGiftWrapping'
                        ),

...
class AppGetProductDetails extends AppCore {
        ...
        if (!(int) Tools::getValue('product_id', 0)) {
            $this->content['product_result'] = array(
                ...
                'message' => parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Product id is missing'), 'AppGetProductDetails'
                )
            );
        } else {
            $this->product = new Product(
                    Tools::getValue('product_id', 0), true, $this->context->language->id, $this->context->shop->id, $this->context
            );
    ...
    public function getProduct() {
        ...
        if ($priceDisplay >= 0 && $priceDisplay <= 2) {
            if ($price_without_reduction <= 0 || !$this->product->specificPrice) {
                ...
                if ($this->product->specificPrice && $this->product->specificPrice['reduction_type'] == parent::PRICE_REDUCTION_TYPE_PERCENT) {
                    $product['discount_percentage'] = $this->product->specificPrice['reduction'] * 100;
                } elseif ($this->product->specificPrice && $this->product->specificPrice['reduction_type'] == 'amount' && $this->product->specificPrice['reduction'] > 0) {
        ...
        if ($this->isProductCommentsEnable()) {
            ...
            if ($product['number_of_reviews'] > 0) {
                ...
                $product['averagecomments'] = Tools::ps_round($avg_rating['grade'], 1, PS_ROUND_UP);
            }
        }
        ...
        if ($this->isBookingCalenderEnable()) {
            ...
            if ($product['booking_calender_data']['is_booking_product'] == null) {
                ...
                $product['customization_fields'] = array('is_customizable' => "0", 'customizable_items' => array());
            }
        } else {
            $product['is_booking_calender_data'] = 0;
        }
                ...
                if ($accessory['available_for_order']) {
                    $accessory_products[$index] = array(
                        ...
                        'src' => $this->context->link->getImageLink(
                                ...
                                /* Changes over */ $accessory['id_image'], $this->getImageType('large')
                        )
                    );
                ...
                foreach ($pack_items as $item) {
                    $pack_products[$index] = array(
                        ...
                        'src' => $this->context->link->getImageLink(
                                ...
                                /* Changes over */ $item['id_image'], $this->getImageType('large')
                        ),
    ...
    public function getRelatedProducts() {
        ...
        if ($this->isRelatedProductEnable()) {
            ...
            if (!empty($products)) {
                $has_related_products = "1";
            }
        }
        return array('has_related_products' => $has_related_products, 'related_products_items' => $products);
    }

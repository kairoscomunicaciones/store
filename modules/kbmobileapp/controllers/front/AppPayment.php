...
class KbMobileAppAppPaymentModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (!(int) $cart_id) {
            ...
            if ($this->validateCustomer()) {
                ...
                */
                ...
                $this->context->cart = new Cart((int) $cart_id, $lang_id);
                ...
                if (!Validate::isLoadedObject($this->context->cart)) {
                    ...
                    if (!validate::isLoadedObject($address)) {
                       ...
                       if (!empty(Tools::getValue('order_comment'))) {
                            $txt_message = urldecode(Tools::getValue('order_comment', ''));
        ...
        if ($this->error) {
            ...
            if(empty($test)) {
            Tools::redirect('index.php?fc=module&module=kbmobileapp&controller=AppPayment&version=1.7&iso_code='.Tools::getValue('iso_code').'&id_shipping_address='.Tools::getValue('id_shipping_address').'&order_message='.Tools::getValue('order_message').'&email='.Tools::getValue('email').'&session_data='.Tools::getValue('session_data').'&id_currency='.Tools::getValue('id_currency')."&test=1");
            } else {

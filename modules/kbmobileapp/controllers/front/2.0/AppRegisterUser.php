...
class AppRegisterUser extends AppCore
{
    ...
    public function getPageData()
    {
        ...
        $user_data = Tools::getValue('signup', json_encode(array()));
        ...
        if (!empty($user_data)) {
            ...
            } elseif (_PS_VERSION_ < '8.0.0' && !Validate::isPasswd($user_data->password)) {
                $this->content['status'] = 'failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false), 
            ...
            }elseif (_PS_VERSION_ >= '8.0.0' && !Validate::isAcceptablePasswordLength($user_data->password)) {
               // include_once _PS_ROOT_DIR_.'/src/Core/Security/PasswordPolicyConfiguration.php';
        ...
        if (!empty($user_data)) {
            if (Customer::customerExists(strip_tags($user_data->email))) {
                ...
                $this->content['signup_user'] = array(
                    ...
                    'cart_count' => (int) Cart::getNbProducts($this->context->cookie->id_cart)
                );

            ...
            foreach ($cat_products as &$product) {
                if (isset($product['id_product_attribute']) && $product['id_product_attribute'] && isset($product['product_attribute_minimal_quantity'])) {
                 ...
                 /**
                     ...
                     */
//                $is_available = '0';
//                if (((int) $product['quantity'] >= (int) $product['minimal_quantity']) || (int) Configuration::get('PS_STOCK_MANAGEMENT') == 0) {
//                    $is_available = '1';
//                } else {
//                    if ((int) $product['out_of_stock'] == 1) {
//                        $is_available = '1';
//                    } else if ((int) $product['out_of_stock'] == 2 && (int) Configuration::get('PS_ORDER_OUT_OF_STOCK') == 1) {
//                        $is_available = '1';
//                    }
//                }
                ...
                $response[$index] = array(
                    ...
                    'on_sale_products' => (int) $product['on_sale'],
                    ...
                    'minimum' => (String) $product['minimal_quantity'],
                );
                ...
                if ($this->isProductCommentsEnable()) {
                    ...
                    if ($response[$index]['number_of_reviews'] > 0) {
                        ...
                        $response[$index]['averagecomments'] = (String) Tools::ps_round($avg_rating['grade'], 1, PS_ROUND_UP);
                        $response[$index]['number_of_reviews'] = (String) $response[$index]['number_of_reviews'];
                    } else {
                        $response[$index]['averagecomments'] = "0";
                        $response[$index]['number_of_reviews'] = (String) $response[$index]['number_of_reviews'];
                    }
                }
                ...
                if (!empty($product['specific_prices']) && count($product['specific_prices']) > 0) {
                ...
                if($disc_per == 0) {
                    $response[$index]['discount_percentage'] = "0";
                } else {
                    $response[$index]['discount_percentage'] = (string) $disc_per .
                            " " .
    ...
    public function getAvailableProducts($selected_filter) {
        ...
        $sql .= 'WHERE product_shop.`active` = 1
                AND product_shop.`visibility` != \'none\' AND p.`id_product` IN ' . pSQL($in);
    ...
    private function getSelectedFilters() {
        ...
        $filters = array();
        if (!empty($post_filters->filter_result)) {

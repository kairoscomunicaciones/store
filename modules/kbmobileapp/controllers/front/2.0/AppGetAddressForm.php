...
class AppGetAddressForm extends AppCore
{
    ...
    {
        if (!empty(Tools::getValue('id_shipping_address'))) {
    ...
    public function assignKbAddress()
    {
        ...
        $this->content['default_state_id'] = isset($this->session->data['payment_address']['zone_id']) ? $this->session->data['payment_address']['zone_id'] : '';
        if ($this->content['default_state_id'] == '') {
            $this->content['default_state_id'] = '0';
        }
    }
    /**
    ...
    public function getFormData()
    {
        ...
        foreach ($ordered_adr_fields as $field) {
            if ($field == 'vat_number') {
                ...
                $form_data[$form_index]['value'] = "";
                /**
                 * Set required as 0 to fix the issue when we save the address without saving the address2, getting required field error even this field is optional.
                 * TGapr2023 address2_required
                 * @date 28-04-2023
...
//                $form_index++;
        ...
        $form_data[$form_index]['validation'] = "";
        $form_data[$form_index]['group_items'] = array();
        $already_added_field = array();
        $update_form_data = array();
        foreach ($form_data as $key => $value) {
            if (in_array($value['name'], $already_added_field)) {
                unset($form_data[$key]);
            } else {
                $update_form_data[] = $value;
                $already_added_field[] = $value['name'];
            }
        }
    ...
    public function assignCountriesList()
    {
        ...
        /**
        * Fixes: If default country is not exists in the list, then send first index of  as $country_list  default_country_id 
        * TGmay2023 default-country
        * @date 17-05-2023
        * @author Tanisha Gupta
        */
        if(!$this->country->active && isset($country_list[1]['id'])){
           $this->country = new country($country_list[1]['id']); 
        }
        $this->content['default_country_id'] = (String) $this->country->id;
        return $country_list;
    }
...
//                    $form_index++;
            ...
            $this->content['default_country_id'] = (String) $address->id_country;

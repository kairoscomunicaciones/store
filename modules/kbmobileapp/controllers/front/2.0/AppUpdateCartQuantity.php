 ...
 * Description
 *
 * API to update product quantity in cart
 */

require_once 'AppCore.php';

class AppUpdateCartQuantity extends AppCore
{
    /**
    ...
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            foreach ($cart_summary['products'] as $product) {
                ...
                $cart_products[$index]['discount_price'] = '0';
                $cart_products[$index]['discount_percentage'] = '0';
                ...
                */
                ...
                if ($extra_product_line) {
                    ...
                    */
                    ...
                    $cart_products[$index]['quantity'] = (String) ($product['cart_quantity'] - $quantity_displayed);
                    $cart_products[$index]['customizable_items'] = array();
                    $index++;
                }
            }
            ...
            if (!$cart_summary['is_virtual_cart']) {
                ...
                $this->content['gift_wrapping'] = array(
                    'available' => (String) Configuration::get('PS_GIFT_WRAPPING'),
            ...
            $this->content['cart_id'] = (String) $this->context->cart->id;
        }
    }
    ...
    public function processChangeProductInCart()
    {
        ...
        if (empty($product_data)) {
            ...
            $qty_to_update = $product_data->cart_products[0]->quantity;
            if ($qty_to_update == 0) {
                $this->content['status'] = 'Failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('Quantity should be greater than zero.'),
                    'AppUpdateCartQuantity'
                );
                $this->writeLog('Null quantity provided.');
                return;
            }
            $product = new Product(
                $id_product,
            ...
            if (!Validate::isLoadedObject($product)) {
                ...
                if (!$product->id
                    ...
                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false),
                        $this->l('This product is no longer available.'),
                        'AppUpdateCartQuantity'
                    );
                    $this->writeLog('This product is no longer available.');
                    return;
                } else {
                    $cart_data = $this->fetchList();
                    $cart_summary = $cart_data['summary'];
...
//                    $cart_products = $this->context->cart->getProducts();
                    ...
                    /**
                     * Added isset condition if id_customization_field is getting or not in the request data
                     * @date 08-05-2023
                     * @modifier Tanisha Gupta
                     */
                    if(isset($product_data->cart_products[0]->id_customization_field)){
                        $id_customization = $product_data->cart_products[0]->id_customization_field;
                    }else{
                        $id_customization = 0;
                    }
//                    $id_address_delivery = $product_data->cart_products[0]->id_address_delivery;
                    $id_address_delivery = $this->context->cart->id_address_delivery;
                    if (empty($id_customization)) {
                        $id_customization = null;
                    }
                    if (is_array($cart_summary['products']) && $cart_summary['products']) {
                        foreach ($cart_summary['products'] as $cart_product) {
                            if (($cart_product['id_product_attribute'] == $id_product_attribute) &&
                                    $cart_product['id_product'] == $id_product) {
                                if ($id_customization != null) {
                                    $c_ip = $cart_product['id_product'];
                                    $c_pa = $cart_product['id_product_attribute'];
                                    $c_ida = $cart_product['id_address_delivery'];
                                    if (isset($customized_data[$c_ip][$c_pa][$c_ida][$id_customization])) {
                                        $c_q = $customized_data[$c_ip][$c_pa][$c_ida][$id_customization]['quantity'];
                                        if ($qty_to_update == $c_q) {
                                            $update = false;
                                        } else {
                                            $qty_to_update = $c_q - $qty_to_update;
                                            $update = true;
                                        }
                                    } else {
                                        $this->content['status'] = 'failure';
                                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                            Tools::getValue('iso_code', false),
                                            $this->l('No customization field for available'
                                                    . ' shipping address and product details.'),
                                            'AppUpdateCartQuantity'
                                        );
                                        $this->writeLog(
                                            'No customization field for available '
                                            . 'shipping address and product details.'
                                        );
                                        return;
                                    }
                                } else {
                                    if (!empty($cart_product['customization_quantity'])) {
                                        $c_cq = $cart_product['customization_quantity'];
                                        $cart_quantity = $cart_product['cart_quantity'] - $c_cq;
                                    } else {
                                        $cart_quantity = $cart_product['cart_quantity'];
                                    }
                                    if ($qty_to_update == $cart_quantity) {
                                        $update = false;
                                    } else {
                                        $qty_to_update = $cart_quantity - $qty_to_update;
                                        $update = true;
                                    }
                                }
                                break;
                    ...
                    if ($update) {
                        ...
                        if ($id_product_attribute) {
                            ...
                            */
                            $checkAttributeQty = true;
                            if (_PS_VERSION_ >= '8.0.0') {
                                $checkAttributeQty = ProductAttribute::checkAttributeQty($id_product_attribute, $qty_to_check);
                            }else{
                                $checkAttributeQty = Attribute::checkAttributeQty($id_product_attribute, $qty_to_check);
                            }
                            if (!Product::isAvailableWhenOutOfStock($product->out_of_stock)
                                ...
                                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false),
                                    $this->l('There isn\'t enough product in stock.'),
                                    'AppUpdateCartQuantity'
                                );
                                $this->writeLog('There isn\'t enough product in stock.');
                            } else {
                                ...
                                if ($update_quantity < 0) {
                                    //If product has attribute, min quantity is set with minimal quantity of attribute
                                    $pma = $product->minimal_quantity;
                                    /**
                                    * Added PS version 8 condition as Attribute class change, which has been renamed to ProductAttribute
                                    * TGfeb2023 attribute_class
                                    * @date 17-02-2023
                                   ...
                                   if (_PS_VERSION_ >= '8.0.0') {
                                       $minimal_quantity = ($id_product_attribute) ?
                                        ProductAttribute::getAttributeMinimalQty($id_product_attribute) : $pma;
                                   }else{
                                       $minimal_quantity = ($id_product_attribute) ?
                                        Attribute::getAttributeMinimalQty($id_product_attribute) : $pma;
                                   }
                                    $this->content['status'] = 'failure';
                                    ...
                                    $this->writeLog('Must have minimum quantity for product id ' . $product->id);
                                } elseif (!$update_quantity) {
                                    $this->content['status'] = 'failure';
                                    
                                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false),
                                        $this->l('You already have the maximum quantity available for this product.'),
                                        'AppUpdateCartQuantity'
                                    );
                                    $this->writeLog(
                                    ...
                                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false),
                                        $this->l('Product Successfully updated'),
                                        'AppUpdateCartQuantity'
                                    );
                                    $this->writeLog('Product Successfully updated');
                                }
                            }
                            ...
                            $checkAttributeQty = true;
                            if (_PS_VERSION_ >= '8.0.0') {
                                $checkAttributeQty = ProductAttribute::checkAttributeQty($this->id_product_attribute, $qty_to_check);
                            }else{
                                $checkAttributeQty = Attribute::checkAttributeQty($this->id_product_attribute, $qty_to_check);
                            }
                        ...
                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('No change found in quantity.'),
                            'AppUpdateCartQuantity'
                        );
                        $this->writeLog('No change found in quantity.');
                    }
                }
            }
        }

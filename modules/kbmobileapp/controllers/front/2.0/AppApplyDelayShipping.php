 ...
 * Description
 *
 * API to apply dlayed shipping into the cart
 * Called from cart page in APP
 */

require_once 'AppCore.php';

class AppApplyDelayShipping extends AppCore
{
    /**
    ...
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            /* Apply Delay Shipping*/
            if (!Configuration::get('PS_SHIP_WHEN_AVAILABLE')) {
                $this->content['status'] = 'success';
                $this->content['message'] = '';
                $this->writeLog('Delay shipping is not available');
            } else {
                $this->content['status'] = 'success';
                $this->content['message'] = '';
                $this->writeLog('Delay shipping applied');
                $this->context->cart->allow_seperated_package = (bool) 1;
            }
            $this->context->cart->update();

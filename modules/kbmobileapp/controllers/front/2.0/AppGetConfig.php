...
class AppGetConfig extends AppCore
{
    ...
    public function getPageData()
    {
        ...
        if (Tools::file_exists_no_cache($module_controller_dir . $class_name . '.php')) {
            ...
            $this->content['home_response'] = $response['content'];
        }
        $this->content['languages_record'] = $this->returnLanguageRecordAsArray();
        $url = 'appGetTranslations';
        $class_name = 'appGetTranslations';
        $api_version = Tools::getValue('version');
        if ($api_version != '1.1') {
            $class_name = Tools::ucfirst($class_name);
        }
        ...
        if (Tools::file_exists_no_cache($module_controller_dir . $class_name . '.php')) {
            ...
            $request = new $class_name($url);
            $response = $request->fetch();
            $this->content['translated_texts'] = $response['content'];
        }

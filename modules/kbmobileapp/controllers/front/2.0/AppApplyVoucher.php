 ...
 * Description
 *
 * API to apply voucher into the cart
 * Called from cart page in APP
 *
 */

require_once 'AppCore.php';

class AppApplyVoucher extends AppCore
{
    /**
    ...
    public static function getProductDetailsByID($id_product)
    {
        ...
        return Db::getInstance()->getRow('SELECT a.* FROM '._DB_PREFIX_.'kb_booking_product'.' as a INNER JOIN '._DB_PREFIX_.'product p on (a.id_product=p.id_product) INNER JOIN '._DB_PREFIX_.'product_shop ps on (a.id_product=ps.id_product AND ps.id_shop='.(int)$id_shop.') WHERE a.active=1 AND a.id_product='.(int)$id_product);
    }
    ...
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            foreach ($cart_summary['products'] as $product) {
                ...
                //Added ps_stock_management condition by Tanisha on 19-DEC-2022  
                if (($product['cart_quantity'] <= StockAvailable::getQuantityAvailableByProduct(
                    $product_obj->id,
                    $product['id_product_attribute'])) || ((int) Configuration::get('PS_STOCK_MANAGEMENT') == 0)
                ) {
                ...
                if ($this->isBookingCalenderEnable()) {
                    $booking_product_details = $this->getProductDetailsByID($product_obj->id);
                    if (!empty($booking_product_details)) {
                        $cart_products[$index]['is_booking_product'] = 1;
                    } else {
                        $cart_products[$index]['is_booking_product'] = 0;
                    }
                } else {
                    $cart_products[$index]['is_booking_product'] = 0;
                }
                /*end:changes started by Aayushi on 3rd July 2019 to integrate booking module*/
                /**
                * Fixed issue: when the voucher is applied or removed the products in the cart will be shown out of stock.
                * TGmay2023 voucher-stock
                * @date 23-05-2023
                * @author Shivam Bansal
                * @commenter Tanisha Gupta
                */
                if (!$cart_products[$index]['stock']) {
                    if ((int) Configuration::get('PS_STOCK_MANAGEMENT') == 0) {
                            $cart_products[$index]['stock'] = true;
                    } elseif ((int) $product_obj->out_of_stock == 1) {
                            $cart_products[$index]['stock'] = true;
...
//                    }
     ...
     * Fetch cart summary
     *
     * @return array return cart summary
     */
    ...
    {
        if (CartRule::isFeatureActive()) {
            if (!($code = trim(Tools::getValue('voucher')))) {
                $this->content['status'] = 'Failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('You must enter a voucher code'),
                    'AppApplyVoucher'
                );
                $this->writeLog('Voucher code is blank');
            } elseif (!Validate::isCleanHtml($code)) {
                $this->content['status'] = 'Failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('The voucher code is invalid'),
                    'AppApplyVoucher'
                );
                $this->writeLog('The voucher code is invalid');
            } else {
                /**
                 * Changes done by Tanisha to set cookie for app voucher
                 */
                $this->context->cookie->kbmobileappVoucher = 1;
                if (($cart_rule = new CartRule(CartRule::getIdByCode($code)))
                    ...
                    if ($error = $cart_rule->checkValidity($this->context, false, true)) {
                        $this->content['status'] = 'Failure';
                        if (is_array($error)) {
                            $this->content['message'] = implode('<br>', $error);
                        } else {
                            $this->content['message'] = $error;
                        }
                        $this->writeLog('Error in validating the cart rule');
                    } else {
                        $this->context->cart->addCartRule($cart_rule->id);
                        $this->content['status'] = 'success';
                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('Voucher successfully applied to cart'),
                            'AppApplyVoucher'
                        );
                        $this->writeLog('Voucher successfully applied to cart' . $this->context->cart->id);
                    }
                } else {
                    $this->content['status'] = 'Failure';
                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
            ...
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false),
                $this->l('This feature is not active for this voucher'),
                'AppApplyVoucher'
            );
            $this->writeLog('This feature is not active for this voucher');
        }

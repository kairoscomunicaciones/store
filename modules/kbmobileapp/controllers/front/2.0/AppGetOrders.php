...
class AppGetOrders extends AppCore {
            ...
            if ($orders = Order::getCustomerOrders($this->context->customer->id)) {
                ...
                foreach ($orders as &$order) {
                    ...
                    if (Validate::isLoadedObject($myOrder)) {
                        ...
                        $temp = $this->makeOrdersResponseArray($myOrder);
                        if($temp != null) {
                            $orders_detail[$index] = $temp;
                            $index++;
                        }
                    } else {
                        $error = true;
                ...
                if ($error) {
                    ...
                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Unable to load order history'), 'AppGetOrders'
                    );
    ...
    public function validateCustomer() {
        ...
        if (!Validate::isEmail($email)) {
            $this->content['status'] = 'failure';
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false), $this->l('Email address is not valid'), 'AppGetOrders'
            );
            ...
            if (Customer::customerExists(strip_tags($email))) {
                 ...
                 * Dom : 20/09/2018
                 */
                $query = 'SELECT count(*) FROM ' . _DB_PREFIX_ . 'kbmobileApp_unique_verification where id_shop = ' . (int) $this->context->shop->id . ' AND id_customer = ' . (int) $customer->id;
                $exists = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($query);
                if ($exists > 0) {
                    $data = array();
                    $query = 'SELECT * FROM ' . _DB_PREFIX_ . 'kbmobileApp_unique_verification where id_shop = ' . (int) $this->context->shop->id . ' AND id_customer = ' . (int) $customer->id;
                    $data2 = Db::getInstance(_PS_USE_SQL_SLAVE_)->getrow($query);
                    $customer_data['mobile_number'] = $data2['mobile_number'];
                    $customer_data['country_code'] = $data2['country_code'];
                } else {
...
//                $this->content['personal_info'] = $customer_data;
                ...
                $this->content['status'] = 'failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                Tools::getValue('iso_code', false), $this->l('Customer with this email not exist'), 'AppGetOrders'
                );
    ...
    private function makeOrdersResponseArray($order) {
        ...
        if($order_state->name == null) {
            return null;
        }
        $order_tmp['order_id'] = (String) $order->id;
        $order_tmp['cart_id'] = $order->id_cart;
        $order_tmp['order_number'] = (String) Order::getUniqReferenceOf($order->id);
        ...
        $order_tmp['item_count'] = (String) count($products);
...
//                            Tools::getValue('iso_code', false), $this->l('Quantity'), 'AppGetOrders'
//                    ),
//                    'value' => $result["product_quantity"]
//                ),
//                array(
//                    'name' => parent::getTranslatedTextByFileAndISO(
//                            Tools::getValue('iso_code', false), $this->l('SKU'), 'AppGetOrders'
//                    ),
...
//                        $result["product_id"], $result["image"]
//                );
//            } else {
//                $order_tmp['products'][$product_index]['images'] = '';
//            }
        ...
        if (empty($order_product_image)) {
            ...
            if (Configuration::get('KB_MOBILEAPP_URL_ENCODING') == 1) {
                return $this->context->link->getImageLink(
                                urlencode($product->link_rewrite), $iso_code . '-default', $this->getImageType('large')
                );
            } else {
                return $this->context->link->getImageLink(
                                $product->link_rewrite, $iso_code . '-default', $this->getImageType('large')
                );
            }
        }
        if (Configuration::get('KB_MOBILEAPP_URL_ENCODING') == 1) {
            return $this->context->link->getImageLink(
                            urlencode($product->link_rewrite), $order_product_image->id, $this->getImageType('large')
            );
        } else {
            return $this->context->link->getImageLink(
                            $product->link_rewrite, $order_product_image->id, $this->getImageType('large')
            );
        }
        /* Changes over */
    }

 ...
 * Description
 *
 * API to save gift message to cart provided at cart page
 */

require_once 'AppCore.php';

class AppSaveGiftMessage extends AppCore
{
    /**
    ...
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            if (!Validate::isMessage($message)) {
                ...
                $this->writeLog('Invalid gift message.');
            } else {
                $this->context->cart->gift_message = strip_tags($message);
                $this->content['status'] = 'success';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('Gift message saved'),
                    'AppSaveGiftMessage'
                );
                $this->writeLog('Gift message saved');
            }
            $this->context->cart->update();

...
class AppFCMregister extends AppCore
{
    ...
    public function getPageData()
    {
        ...
        $device_type = Tools::getValue('mobile_platform', 'both');
        if ($cart_id && $fcm_id) {
            $this->setFcmData($cart_id, $fcm_id, $device_type);
        } else {
            $this->content['status'] = 'failure';
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false), 
                $this->l('Cart id or FCM id not found.'), 
                'AppFCMregister'
            );
        }
     ...
     *
     */
    public function setFcmData($cart_id, $fcm_id, $deviceType)
    {
        ...
        if (!$fcm_data) {
            $query = "INSERT INTO `"._DB_PREFIX_."kb_fcm_details` ("
                    ...
                    . " `date_add`, `date_upd`, `device_type`) VALUES ("
                    . "NULL, '". (int)$cart_id."', '".pSQL($fcm_id)."', '0', now(), now(),'".  pSQL($deviceType)."')";

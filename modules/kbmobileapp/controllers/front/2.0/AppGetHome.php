...
class AppGetHome extends AppCore {
    ...
    public function getPageData() {
        ...
        $home_response = array();
//        $home_response['install_module'] = '';
        $home_response['languages'] = $this->getLanguagesDataForHome();
        $home_response['currencies'] = $this->getCurrenciesDataForHome();
        $home_response['languages_record'] = $this->returnLanguageRecordAsArray();
        $home_response['elements'] = $this->getElementData();
//        $home_response['topslider'] = $this->getTopSliders();
//        $home_response['topbanner'] = $this->getTopBanners();
//        $home_response['fproducts'] = $this->getFeaturedProducts();
//        $home_response['sproducts'] = $this->getSpecialProducts();
//        $home_response['bsproducts'] = $this->getBestSellerProducts();
//        $home_response['newproducts'] = $this->getNewProductsList();

        $home_response['fontStyle'] = Configuration::get('KBMOBILEAPP_FONT');
        $home_response['is_tabbar_enabled'] = Configuration::get('tab_bar_status');
        $home_response['tabBar'] = $this->getTabBarData();
        $tab_bar_background_color = explode('#', Configuration::get('tab_bar_background_color'));
        $home_response['tabBar']['tab_bar_background_color'] = $tab_bar_background_color[1];
        $tab_bar_tint_color = explode('#', Configuration::get('tab_bar_tint_color'));
        $home_response['tabBar']['tab_bar_tint_color'] = $tab_bar_tint_color[1];
        $tab_bar_background_color_disabled = explode('#', Configuration::get('tab_bar_background_color_disabled'));
        $home_response['tabBar']['tab_bar_background_color_disabled'] = $tab_bar_background_color_disabled[1];
        $home_response['total_cart_items'] = Cart::getNbProducts($this->context->cart->id);
        $email = Tools::getValue('email', '');
        if (isset($email) && !empty($email)) {
            $home_response['total_wishlist_items'] = $this->getWishListCount($this->context->customer->id);
        }

        //New Keys added by Shivam Bansal
        $home_response['message'] = "";
            /*changes made by vibhaas */
       // print_r(Configuration::get('cart_button_status'));die;
         if (Configuration::get('cart_button_status') == 1) {
            $home_response['cart_button_status'] = "1";
        } else {
            $home_response['cart_button_status'] = "0";
        }
         if (!empty(Configuration::get('snackbar_background_color'))) {
            $home_response['snackbar_background_color'] = Configuration::get('snackbar_background_color');
        } else {
            $home_response['snackbar_background_color'] = "3b3b3b";
        }
          if (!empty(Configuration::get('snackbar_text_color'))) {
            $home_response['snackbar_text_color'] = Configuration::get('snackbar_text_color');
        } else {
            $home_response['snackbar_text_color'] = "ffffff";
        }
         if (!empty(Configuration::get('app_button_text_color'))) {
            $home_response['app_button_text_color'] = Configuration::get('app_button_text_color');
        } else {
            $home_response['app_button_text_color'] = "ffffff";
        }
        /*changes over */
...
//        $home_response['snackbar_text_color'] = 'ffffff';
//        $home_response['app_button_text_color'] = "ffffff";
        $home_response['is_tab_bar_enabled'] = "1";
        ...
        $home_response['spin_win_response'] = $spin_win_response;
        /* changes over */
        if (!Module::isInstalled('blockwishlist') || !Module::isEnabled('blockwishlist')) {
            $home_response['wishlist_active'] = 0;
        } else {
            $home_response['wishlist_active'] = 1;
        }
        ...
        //To get cms content
        $home_response['cms_links'] = $this->getCmsPagesLink();
        if (Configuration::get('FOOTER_CONTACT')) {
            $home_response['contact_us_available'] = 1;
            $contact_url = (_PS_VERSION_ >= 1.5) ? 'contact' : 'contact-form';
            $contact_link = $this->context->link->getPageLink($contact_url);
            $home_response['contact_us_link'] = $contact_link;
        } else {
            $home_response['contact_us_available'] = 0;
            $home_response['contact_us_link'] = "";
        }
        
        $home_response['Menu_Categories'] = $this->getMenus();
        /* start:changes started by Tarun Gupta on 8/9/2020 to set the hyperlocal static category id */
        ...
        if (Configuration::get('KB_MARKETPLACE') !== false && Configuration::get('KB_MARKETPLACE') == 1 && $hyperlocal_enable && isset($settings['kbmp_enable_hyperlocal']) && $settings['kbmp_enable_hyperlocal']) {
            $hyperlocal_category = array(
                'id' => "available_products",
                'name' => parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Near By Products'), 'AppGetHome'
                ),
                'children' => array());
            $home_response['Menu_Categories'][] = $hyperlocal_category;
        }
        /* end:changes started by Tarun Gupta on 8/9/2020 to set the hyperlocal static category id */
        /* To know marketplace is enabled or not */
        $home_response['is_marketplace'] = ($this->isMarketplaceEnable()) ? '1' : '0';
        ...
        $home_response['app_background_color'] = $background_color_array[1];
        $home_response['app_button_color'] = $button_color_array[1];
        $home_response['app_theme_color'] = $theme_color_array[1];
        
        /**
         * Custom changes: To send key for social login
         * @author Tanisha Gupta
         * @date 30-01-2023
         */
        if (Configuration::get('KB_MOBILEAPP_SOCIAL_LOGIN') == 1) {
            $home_response['is_social_login_enabled'] = "1";
        } else {
            $home_response['is_social_login_enabled'] = "0";
        }
        /* end */
    ...
    public function getTabBarData() {
        ...
        $sql = 'Select * From ' . _DB_PREFIX_ . 'kb_mobileapp_tab_bar where store_id=' . (int) $this->context->shop->id;
    ...
    public function getTopSliders() {
        ...
        $data['title'] = parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Top Sliders'), 'AppGetHome'
        );

        $slider_text = parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Slider'), 'AppGetHome'
        );
        $data['slides'] = array();
    ...
    public function getTopBanners() {
        $data = array();
        $data['title'] = parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Top Banners'), 'AppGetHome'
        );

        $banner_txt = parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false), $this->l('Banner'), 'AppGetHome'
        );
        $data['banners'] = array();
...
//                    }
                    ...
                    /*changes made by vibhaas */
                    if($res['id_product_attribute'] != "0")
                    {
                        $products[$index]['cart_quantity'] = 0;
                    }
                    /*changes over */
                    $index++;
                }
                ...
                case 'CAT':
                    $qry = 'SELECT c.id_category, c.id_parent, cl.name
                            ...
                            WHERE 1 AND `id_lang` = ' . (int) $this->context->language->id . '
                             AND c.`active` = 1 
                            ' . (isset($groups) && $active ? ' AND cg.`id_group` IN (' . pSQL(implode(',', $groups)) . ')' : '') . '
                            ' . (!$this->context->language->id || (isset($groups) && $active) ? ' GROUP BY c.`id_category`' : '') . '
                        ...
                        foreach ($results as $m) {
                            ...
                            if ($m['id_category'] == $id) {
                                $categories[$index] = array(
                                    ...
                                    'name' => $m['name'],
                                    'image_src' => $image_url,
                                    'children' => $this->getSubChild($m['id_category'], $results)
                                );
                                $index++;
                            }
                        }
            ...
            if ($cat['id_parent'] == $id_parent) {
                ...
                if ($further_child) {
                    ...
                    $sub_categories[$index]['children'] = $this->getSubChild(
                            $cat['id_category'], $categories, true
                    );
                    /* end:changes */
                }
                $index++;
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    ...
                    $conf .= (string) ($key > 1 ? ',' : '') . Configuration::get(
                                    'MOD_BLOCKTOPMENU_ITEMS', null, $shop_group_id, $shop_id
                    );
                }
            } else {
                $shop_id = (int) $shops[0];
                ...
                if ($component_type == 'top_category') {
                    ...
                    if (is_array($categories)) {
                        if (count($categories) > 0) {
                            /**
                             * Send blank heading as in the app heading is showing as null
                             * TGmay2023 top-heading
                             * @date 16-05-2023
                             * @author Tanisha Gupta
                             */
                                ...
                                if ($value1 > 0) {
                                    if ($value1 == 1) {
                                        ...
                                        if (isset($category_heading_data[$j][$id]) && !empty($category_heading_data[$j][$id])) {
                                            $data['name'] = $category_heading_data[$j][$id];
                                        } else {
                                            $data['name'] = '';
                                        }
                                    } else if ($value1 == 2) {
                    ...
                    if (is_array($banner_data)) {
                        ...
                        if ($number_of_entries > 0) {
                            ...
                            foreach ($banner_data as $k => $bd) {
                                ...
                                /* changes made by vibhaas */ else if ($bd['redirect_activity'] == 'layout') {
                                    $data['target_id'] = $bd['layout_id'];
                                } else {
                                    $data['target_id'] = $bd['product_id'];
                                }

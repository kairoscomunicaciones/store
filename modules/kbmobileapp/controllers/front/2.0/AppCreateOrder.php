...
class AppCreateOrder extends AppCore
{
    ...
    {
        $payment_data = Tools::getValue('payment_info', json_encode(array()));
        $payment_data = json_decode($payment_data);
        /* Changes start by rishabh jain on 4th sep 2018
         * to check empty payment method or empty payment status
        */
        $payment_method_empty = '';
        $payment_status_empty = '';
        if ($payment_data->payment_method_code == '' || $payment_data->payment_method_codes = null) {
            $this->content['status'] = 'failure';
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false),
                $this->l('There is an issue while creating your order.Kindly try again.'),
                'AppCreateOrder'
            );
            $payment_method_empty = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false),
                $this->l('Empty Payment method name.'),
                'AppCreateOrder'
            );
            $this->writeLog('Empty Payment method name.');
            return false;
        } else if ($payment_data->status == '' || $payment_data->status = null) {
            $this->content['status'] = 'failure';
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false),
                $this->l('There is an issue while creating your order.Kindly try again.'),
            ...
            $payment_status_empty = parent::getTranslatedTextByFileAndISO(
                Tools::getValue('iso_code', false),
                $this->l('Empty Payment Status.'),
                'AppCreateOrder'
            );
            $this->writeLog('Empty Payment Status.');
            return false;
        }
        /* Changes over */
    ...
    public function appValidateOrder(
        ...
        */
        $lang_id = (int)Language::getIdByIso(Tools::getValue('iso_code', ''));
        $this->context->cart = new Cart((int)$id_cart, $lang_id);
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    $order->id_customer = (int)$this->context->cookie->id_customer;
                    ...
                    $payment_data = Tools::getValue('payment_info', json_encode(array()));
                    $payment_data = json_decode($payment_data);
                    if ($payment_data->payment_method_code == "cod") {
                        $order->module = 'COD';
                    } else if ($payment_data->payment_method_code == "paypal") {
                        $order->module = 'paypal';
                    } else {
                    ...
                    if (!is_null($carrier)) {
                     ...
                     if (!empty(Tools::getValue('order_comment'))) {
                         
                        DB::getInstance()->execute(
                        'Insert into `' . _DB_PREFIX_ . 'message`'
                        . ' set `id_order` = "' . (int)$order->id. '",'
                        . ' `message`="' . pSQL(Tools::getValue('order_comment')) . '",'
                        . ' `id_customer`="' . pSQL((int)$this->context->cookie->id_customer) . '",'
                        . ' `id_cart`="' . pSQL(Tools::getValue('session_data', 0)) . '",'
                        . ' `id_employee`= 0,'
                        . ' `private`= 0'   
                                );

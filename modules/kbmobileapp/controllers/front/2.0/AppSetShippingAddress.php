 ...
 * Description
 *
 * API to set selected Shipping address to the cart
 */

require_once 'AppCore.php';

class AppSetShippingAddress extends AppCore
{
    /**
...
//                            }
                            ...
                            $this->content['total_cost'] = (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::BOTH), 2);
                                            ...
                                            if ($type == Product::CUSTOMIZE_TEXTFIELD) {
                                                foreach ($custom_data as $tf) {
                                                    ...
                                                    if ($tf['name']) {
                                                        ...
                                                        $cart_products[$index][$t_str][$c_i]['title'] = "Text #".($t_i+1);
                                                        $t_i++;
                                                    }
                                                    $c_i++;
                                                }
                            ...
                            if ($this->context->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS) < $minimal_purchase) {
                                $this->content['checkout_page']['minimum_purchase_message'] = sprintf(
                                    parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false),
                                        $this->l('A minimum purchase total of %1s is required to validate your order, current purchase total is %2s'),
                                        'AppSetShippingAddress'
                                    ),
    ...
    /**
     * Set the selected shipping address to cart context and set the address details in shipping_address paramter
     *
            ...
            if (count($address_without_carriers) && !$this->context->cart->isVirtualCart()) {
                ...
                foreach ($errors as $error) {
                    if ($error == Carrier::SHIPPING_WEIGHT_EXCEPTION && !$flag_error_message) {
                        ...
                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('The product selection cannot be delivered by the available carrier(s): it is too heavy. Please amend your cart to lower its weight.'),
                            'AppSetShippingAddress'
                        );
                        $this->writeLog(
                        ...
                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('The product selection cannot be delivered by the available carrier(s). Please amend your cart.'),
                            'AppSetShippingAddress'
                        );
                        $this->writeLog(
                        ...
                        $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                            Tools::getValue('iso_code', false),
                            $this->l('The product selection cannot be delivered by the available carrier(s): its size does not fit. Please amend your cart to reduce its size.'),
                            'AppSetShippingAddress'
                        );
                        $this->writeLog(
     ...
     *
     * @param int $id_billing address id
     * @return bool
     */

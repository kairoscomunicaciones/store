 ...
 */
/**
 * use file to use PasswordPolicyConfiguration::CONFIGURATION_MINIMUM_LENGTH/CONFIGURATION_MAXIMUM_LENGTH 
 * @date 03-05-2023
...
class AppUpdatePassword extends AppCore
{
    ...
    public function getPageData()
    {
        ...
        if (!Tools::getIsset('mobile_number') || !Tools::getIsset('country_code') || !Tools::getIsset('new_password')) {
            ...
            } elseif (_PS_VERSION_ < '8.0.0' && !Validate::isPasswd($user_data['new_password'])) {
                $this->content = array(
                    'status' => 'failure',
                    'message' => parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false),
            ...
            } elseif (_PS_VERSION_ >= '8.0.0' && !Validate::isAcceptablePasswordLength($user_data['new_password'])) {
                $this->content = array(
                    'status' => 'failure',
                    'message' => springf(parent::getTranslatedTextByFileAndISO(
                        Tools::getValue('iso_code', false),

...
class AppGetOrderDetails extends AppCore
{
    ...
    private function makeOrdersResponseArray($order)
    {
        ...
        $order_tmp['order_history']['order_id'] = (String) $order->id;
        ...
        foreach ($products as $result) {
            $order_tmp['products'][$p]['product_id'] = $result["product_id"];
            ...
            $order_tmp['products'][$p]['total_price'] = $this->formatOrderPrice($result["total_price_tax_incl"], $order_currency);
        ...
        if (Module::isInstalled('kbdeliveryboyapp') && Module::isEnabled('kbdeliveryboyapp')) {
            $check_exists = "SELECT * FROM " . _DB_PREFIX_ . 'kb_assigned_orders WHERE id_order='.(int)$order->id.' AND assignment_status = "ACCEPTED" AND pickup_status = 1 AND delivered_status = "0"';
            $data = Db::getInstance()->getRow($check_exists);
            if (!empty($data)) {
                $delivery_boy_details = array(
                    'email' => $data['email'],
                );
                $boy_details_sql= 'SELECT * FROM ' . _DB_PREFIX_ . 'kb_delivery_boys WHERE email="'. pSQL($data['email']) .'"';
                $boy_data = Db::getInstance()->getRow($boy_details_sql);
                if (!empty($boy_data)) {
                    $delivery_boy_details['name'] = $boy_data['first_name'].' '.$boy_data['last_name'];
                    $delivery_boy_details['contact_number'] = $boy_data['phone'];
                }
                $order_tmp['delivery_boy_details'] = $delivery_boy_details;
            }
        }

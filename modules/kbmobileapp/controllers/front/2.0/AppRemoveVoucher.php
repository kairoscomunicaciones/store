 ...
 * Description
 *
 * API to remove voucher from the cart
 */

require_once 'AppCore.php';

class AppRemoveVoucher extends AppCore
{
    /**
    ...
    public function getCartData()
    {
        if (!Validate::isLoadedObject($this->context->cart)) {
            ...
            foreach ($cart_summary['products'] as $product) {
                ...
                //Added by Tanisha on 20-DEC-2022 to fix out of stock issue
                ...
                */
                ...
                if ($extra_product_line) {
                    ...
                    //Added by Tanisha on 20-DEC-2022 to fix out of stock issue
                     /**
//                     * Fixed issue: when the voucher is applied or removed the products in the cart will be shown out of stock.
                       * Commented below code and added new fixes 
//                     * TGmay2023 voucher-stock
//                     * @date 23-05-2023
//                     * @author Shivam Bansal
//                     * @commenter Tanisha Gupta
...
//                     */
    ...
    /**
     * Remove vocuher from the cart
     *
     */
    public function removeVoucher()
    {
        if (CartRule::isFeatureActive()) {
            if (($id_cart_rule = (int) Tools::getValue('voucher')) && Validate::isUnsignedId($id_cart_rule)) {
                $this->context->cart->removeCartRule($id_cart_rule);
                $this->content['status'] = 'success';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('Voucher successfully removed'),
                    'AppRemoveVoucher'
                );
                $this->writeLog('Voucher successfully removed');
            } else {
                $this->content['status'] = 'Failure';
                $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                    Tools::getValue('iso_code', false),
                    $this->l('Error occured while removing voucher'),
                    'AppRemoveVoucher'
                );
                $this->writeLog('Error occured while removing voucher');
            }
        } else {
            $this->content['status'] = 'Failure';
            $this->content['message'] = parent::getTranslatedTextByFileAndISO(

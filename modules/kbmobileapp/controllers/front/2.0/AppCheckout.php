...
class AppCheckout extends AppCore
{
            ...
            if ($this->validateCustomer()) {
                ...
                */
                ...
                if (!Validate::isLoadedObject($this->context->cart)) {
                    ...
                    /* start:changes started by Tarun Gupta on 8/9/2020 to check the products in the cart is available or not on the basis of shipping address*/
                    ...
                    if (Configuration::get('KB_MARKETPLACE') !== false && Configuration::get('KB_MARKETPLACE') == 1 && $hyperlocal_enable && isset($settings['kbmp_enable_hyperlocal']) && $settings['kbmp_enable_hyperlocal']) {
                        $this->isHyperlocalCheckoutAllowed($id_shipping);
                    }
                    /* end:changes started by Tarun Gupta on 8/9/2020 to check the products in the cart is available or not on the basis of shipping address*/
                    if (!$this->getShippingAddress($id_shipping)) {
                        $this->content['checkout_page']['shipping_address'] = array();
                    }
                    if (!$this->getBillingAddress($id_billing)) {
                        $this->content['checkout_page']['billing_address'] = array();
                    }
                    ...
                    if (Tools::getIsset('pp_shippings')) {
                        ...
                        $selected_carriers = Tools::getValue('pp_shippings', json_encode(array()));
                        $selected_carriers = json_decode($selected_carriers);
                    ...
                    $this->content['checkout_page']['cart_id'] = (String) $this->context->cart->id;
                    $this->content['checkout_page']['coupons'] = array();
                    $this->content['checkout_page']['order_total'] = (float) Tools::ps_round(
                                    (float) $this->context->cart->getOrderTotal(true, Cart::BOTH), 2
                    );
                    $this->content['checkout_page']['voucher_html'] = '';
                    $this->content['delay_shipping'] = array(
                        "applied" => "0",
                        "available" => "0"
                    );
                    $this->content['native_payments'] = array();
                    if (Configuration::get('KB_MOBILEAPP_PAYMENT_METHODS')) {
                        ...
                        foreach ($payment_data as $data) {
                            if ($data['status'] == 1 && $data['payment_code'] == 'cod') {
                        ...
                        $this->content['native_payments'] = $payment_method_array;
                    }
                    ...
                    foreach ($cart_summary['products'] as $product) {
                        ...
                        $cart_products[$index] = array(
                            'product_id' => (String) $product_obj->id,
                            'title' => $product['name'],
                            'is_gift_product' => "0",
                            'id_product_attribute' => $product['id_product_attribute'],
                            'id_address_delivery' => (String) $product['id_address_delivery'],
                            'stock' => true
                        );
                                ...
                                foreach ($customization['datas'] as $type => $custom_data) {
                                    if ($type == Product::CUSTOMIZE_TEXTFIELD) {
                                        foreach ($custom_data as $tf) {
                                            ...
                                            if ($product['id_customization'] == $id_c) {
                        ...
                        if ($customization) {
                            $cart_products[$index]['price'] = $this->formatPrice($product['price_wt']);
                            $cart_products[$index]['total_price'] = $this->formatPrice($product['total_customization_wt']);
                            $cart_products[$index]['quantity'] = (String) $product['customizationQuantityTotal'];
                            $cart_products[$index]['product_items'] = array(
                                array(
                                    'name' => parent::getTranslatedTextByFileAndISO(
                                            Tools::getValue('iso_code', false), $this->l('Reference'), 'AppCheckout'
                                    ),
                            ...
                            $cart_products[$index]['price'] = $this->formatPrice($product['price_wt']);
                            $cart_products[$index]['total_price'] = $this->formatPrice($product['total_wt']);
                            $cart_products[$index]['quantity'] = (String) $product['cart_quantity'];
                            $cart_products[$index]['product_items'] = array(
                                array(
                                    'name' => parent::getTranslatedTextByFileAndISO(
                                            Tools::getValue('iso_code', false), $this->l('Reference'), 'AppCheckout'
                        ...
                        if (isset($product['attributes']) && $product['attributes']) {
                            $cart_products[$index]['product_items'][] = array(
                                'name' => parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false), $this->l('Attributes'), 'AppCheckout'
                                ),
                        ...
                        if ($extra_product_line) {
                            ...
                            if ($product['cart_quantity'] <= StockAvailable::getQuantityAvailableByProduct(
                                            $product_obj->id, $product['id_product_attribute']
                                    )) {
                            ...
                            $cart_products[$index]['quantity'] = (String) $tot_qty;
                            $cart_products[$index]['customizable_items'] = array();
                            $index++;
                        }
                    }
                    ...
                    $this->content['message'] = parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Cart information loaded successfully'), 'AppCheckout'
                    );
                    ...
                    $cart_total_details[] = array(
                        'name' => parent::getTranslatedTextByFileAndISO(
                                Tools::getValue('iso_code', false), $this->l('Total Products(tax excl.)'), 'AppCheckout'
                        ),
                    ...
                    if ($cart_summary['total_discounts'] > 0) {
                        $cart_total_details[] = array(
                            'name' => parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Total vouchers'), 'AppCheckout'
                            ),
                    ...
                    if ($cart_summary['total_wrapping'] > 0 && $this->context->cart->gift) {
                        $cart_total_details[] = array(
                            'name' => parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Total gift wrapping'), 'AppCheckout'
                            ),
                    ...
                    if ($cart_summary['total_shipping'] > 0) {
                        $cart_total_details[] = array(
                            'name' => parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Total shipping'), 'AppCheckout'
                            ),
                        ...
                        if (!$cart_summary['is_virtual_cart']) {
                            $cart_total_details[] = array(
                                ...
                                'value' => parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false), $this->l('Free shipping!'), 'AppCheckout'
                                )
                            );
                        }
                    }
                    if ($cart_summary['total_tax'] > 0) {
                        $cart_total_details[] = array(
                            'name' => parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('Total Tax'), 'AppCheckout'
                            ),
                    ...
                    $cart_total_details[] = array(
                        'name' => parent::getTranslatedTextByFileAndISO(
                                Tools::getValue('iso_code', false), $this->l('Total Price'), 'AppCheckout'
                        ),
                    ...
                    $a_txt = parent::getTranslatedTextByFileAndISO(
                                    Tools::getValue('iso_code', false), $this->l('(Additional Cost of '), 'AppCheckout'
                    );
                    $gift_wrapping_cost = Tools::convertPriceFull(
                                    $this->context->cart->getGiftWrappingPrice(), null, $this->context->currency
                    );
                    ...
                    $minimal_purchase = Tools::convertPrice(
                                    (float) Configuration::get('PS_PURCHASE_MINIMUM'), $currency
                    );

                    if ($this->context->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS) < $minimal_purchase) {
                        $this->content['checkout_page']['minimum_purchase_message'] = sprintf(
                                parent::getTranslatedTextByFileAndISO(
                                        Tools::getValue('iso_code', false), $this->l('A minimum purchase total of %1s is required to validate'
                                                . ' your order, current purchase total is %2s'), 'AppCheckout'
                                ), Tools::displayPrice($minimal_purchase, $currency), Tools::displayPrice(
                                        $this->context->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS), $currency
                                )
                        );
                    } else {
    ...
    {
        /**
         * Show option on checkout page in the selected langauge
         * TGapr2023 Option-Language
         * @date 28-04-2023
         * @modifier Tanisha Gupta
         */
        $lang_id = (int)Language::getIdByIso(Tools::getValue('iso_code', ''));
        $this->context->cart->id_lang = $lang_id;
    ...
    public function getShippingAddress($id_shipping)
    {
        ...
        if (!validate::isLoadedObject($address)) {
            ...
            $shipping_address['id_shipping_address'] = (String) $address->id;
            ...
            if ($address->id_state != 0) {
                ...
                $shipping_address['zone_id'] = (String) $address->id_state;
            } else {
            ...
            $shipping_address['country_id'] = (String) $address->id_country;
    ...
    public function getBillingAddress($id_billing)
    {
        ...
        if (!validate::isLoadedObject($address)) {
            ...
            $billing_address['id_shipping_address'] = (String) $address->id;
            ...
            if ($address->id_state != 0) {
                ...
                $billing_address['zone_id'] = (String) $address->id_state;
            } else {
                $billing_address['state'] = "";
                $billing_address['zone_id'] = "";
            }
            $billing_address['country'] = Country::getNameById(
                $this->context->language->id,
                $address->id_country
            );
            $billing_address['country_id'] = (String) $address->id_country;

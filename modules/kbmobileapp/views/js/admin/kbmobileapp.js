...
$(document).ready(function()
{
    ...
    $('#kb_gdpr_settings_form_form').addClass('col-lg-10 col-md-9');
    $("#kb_gdpr_settings_form_form").css("float", "right");
    $('#vss-load-gdpr-page').appendTo($("#kb_gdpr_cms_name").parent());
    $('#vss-load-gdpr-page').addClass('col-lg-1');
    ...
    $('#push_notification_redirect_website_url').parent().parent().hide();
...
//    }
    ...
    /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
    /* start:changes started by Aayushi Agarwal on 30 August 18 to add gdpr tab*/
    else if ( default_tab == 'GdprSettings') {
        change_tab($("#link-"+default_tab), 7);
    }
    /* end:changes started by Aayushi Agarwal on 30 August 18 to add gdpr tab*/
    else {
    ...
    $("#load_gdbr").on('click', function () {
    //$('#kb_gdpr_cms_name').on('change', function(e) {
        lang_id = email_lang;
        var lang = active_languages;
        for (i = 1; i <= lang.length; i++) {
            if ($('.lang-' + i).is(":visible") == true) {
                var lang_id = i;
                //alert(i);
                break;
            }
        ...
        $.ajax({
            url: ajaxaction + "&configure=kbmobileapp&getCmsContent=true",
            data: 'cms_id=' + $('#kb_gdpr_cms_name').val() + '&selected_lang=' + lang_id,
            type: "post",
            dataType: 'json',
            beforeSend: function() {
                $('#loader_gdbr').html('<img id="loader_module_list" style="text-align: center;width:35px;height:35px;align:right" src=" ' + loader_url + '" alt="" border="0" />');
            },
            success: function (data)
            {
                $('#loader_gdbr').html('');

                //$('#load_gdbr').parent().parent().append('<img id="loader_module_list" style="text-align: center;width:35px;height:35px;align:right" src=" ' + loader_url + '" alt="" border="0" />');
                if (data) {
                    
                    tinyMCE.get('kb_gdpr_page_content_' + lang_id).setContent(data['body']);
                    $('#kb_gdpr_page_content_'+ lang_id).html(data['body']);
                } 
            }
        });
...
//        }
    ...
    $('input:radio[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]').on('click change', function (e) {
        ...
        if ($(this).val() == 1) {
            ...
            /* This will show the chat icon in the preview of general settings if whatsapp chat is enabled*/
            $('.chatBoxIcon').show();
            ...
            /*start:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/
            /* This will hide the chat icon in the preview of general settings if whatsapp chat is disabled*/
            $('.chatBoxIcon').hide();
            /*end:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/
        }
    });
    if ($('input[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]:checked').val() == "1") {
        ...
        $('#KB_MOBILE_WHATSAPP_CHAT_NUMBER').parent().parent().hide();
        /*start:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/
        /* This will show the chat icon in the preview of general settings if whatsapp chat is disabled*/
        $('.chatBoxIcon').hide();
        /*end:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/
    }
    /* Changes over */
...
//            }
    ...
    $("#push_notification_redirect_type").on('change', function() {
        if ($(this).val() == 'category') {
            ...
            $('#push_notification_redirect_product_name').parent().parent().hide();
            $('#push_notification_redirect_webiste_url').parent().parent().hide();
        } else if ($(this).val() == 'product') {
            $('#push_notification_redirect_category_id').parent().parent().hide();
            $('#push_notification_redirect_product_name').parent().parent().show();
            $('#push_notification_redirect_webiste_url').parent().parent().hide();
            /*changes made by vibhaas to add website url redirection type */
        } else if ($(this).val() == 'website') {
            $('#push_notification_redirect_category_id').parent().parent().hide();
            $('#push_notification_redirect_product_name').parent().parent().hide();
           $('#push_notification_redirect_webiste_url').parent().parent().show();
        }else {
            $('#push_notification_redirect_category_id').parent().parent().hide();
            $('#push_notification_redirect_product_name').parent().parent().hide();
            $('#push_notification_redirect_webiste_url').parent().parent().hide();
...
//                $('#vss-button').appendTo($(".panel-heading"));
        $('#general_form').show();
        /*start:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/ 
        /* To show preview device on general setting*/
        $('#general_settings_mobile_preview').show();
        $('#general_form').find('.help-block').parent().attr('class','col-lg-3');
        $('#general_form').find('.chosen-container').parent().attr('class','col-lg-3');
        $('div[id="rightText"]').parent().show();
        /*end:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/ 
        $('#push_form').hide();
        $('#payments_form').hide();
        $('#push_notification_settings_form').hide();
        ...
        /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
        /* start:changes started by Aayushi Agarwal on 30 August 18 to add gdpr tab*/
        $('#kb_gdpr_settings_form_form').hide();
        /* end:changes started by Aayushi Agarwal on 30 August 18 to add gdpr tab*/
        closePaymentMethod();
        hideSliderForm();
    } else if (b == 3) {
        ...
        $('#general_form').hide();
        /*start:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/ 
        /* To hide preview device on other settings*/
        $('#general_settings_mobile_preview').hide();
        $('div[id="rightText"]').parent().hide();
        /*end:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/ 
        $('#push_form').show();
        $('#push_notification_settings_form').hide();
        $('#push_notification_image_url').parent().parent().hide();
...
//        hideSliderForm();
        ...
        $('#vss-add-new-tab-icon-button').hide();
        $('#vss-add-new-layout-button').hide();
        $('#kb_gdpr_settings_form_form').show();
        /* end:changes started by Aayushi Agarwal on 30 August 18 to add gdpr tab*/
        closePaymentMethod();
        hideSliderForm();
    }  
...
//        hideSliderForm();
        ...
        $('#general_form').show();
        /*start:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/ 
        /* To show preview device on general settings*/
        $('#general_settings_mobile_preview').show();
        $('div[id="rightText"]').parent().show();
        $('#general_form').find('.help-block').parent().attr('class','col-lg-3');
        $('#general_form').find('.chosen-container').parent().attr('class','col-lg-3');
...
//    }
    if ($('input[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]:checked').val() == "1" && $('input[name="KB_MOBILE_APP_CHAT_SUPPORT"]:checked').val() == "1") {
       ...
       general_setting_error = true;
       $('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]').addClass('kb_error_field');
       $('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]').after('<span class="kb_error_message">' + chat_error_text + '</span>');
       $('input[name="KB_MOBILE_APP_CHAT_SUPPORT_KEY"]').addClass('kb_error_field');
       $('input[name="KB_MOBILE_APP_CHAT_SUPPORT_KEY"]').after('<span class="kb_error_message">' + chat_error_text + '</span>');
   } else {
       if ($('input[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]:checked').val() == "1") {
           var chat_number = velovalidation.checkMandatory($('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]'));
           if (chat_number != true)
...
function veloValidateCustomBannerSliderForm(event)
{
    $('.kb_error_message').remove();
    ...
    $('select[name="layout_type_id"]').removeClass('kb_error_field');
    ...
    if ($('#countdown_validity').is(":visible")) {
        if ($('#countdown_validity').val() == '') {
            ...
            $('#countdown_validity').after('<span class="kb_error_message">' + Select_the_Countdown_Validity_Text + '</span>');
    ...
    if ($('#width_of_banner').val() == '') {
        slider_banner_error = true;
            custom_banner_error = width_error;
            $('#width_of_banner').parent().addClass('kb_error_field');
            $('#width_of_banner').parent().after('<span class="kb_error_message">' + width_error + '</span>');
    }
    if ($('#height_of_banner').val() == '') {
        slider_banner_error = true;
            custom_banner_error = height_error;
            $('#height_of_banner').parent().addClass('kb_error_field');
            $('#height_of_banner').parent().after('<span class="kb_error_message">' + height_error + '</span>');
    }
    ...
    if (redirect_activity == 'layout' && layout_type_id == '0') {
        error_message = select_layout_txt;
        custom_banner_error = true;
        $('select[name="layout_type_id"]').addClass('kb_error_field');
        $('select[name="layout_type_id"]').after('<span class="kb_error_message">' + select_layout_txt + '</span>');
    ...
    if (custom_banner_error) {
        return false;
    } else {
        submitcustombannerform(event);
    }
    return false;
}
/*end:changes made by aayushi on 3rd January 2020 to add custom banners*/
...
{  $('.kb_error_message').remove();
    ...
    var redirect_product_id = $.trim($('#redirect_banner_product_id').val());
    var banner_image = $.trim($('#slideruploadedfile').val());
    var redirect_layout_type_id = $.trim($('#layout_type_id').val());
    var error_message = '';
    ...
    /*changes made by vibhaas */
     ...
     if (redirect_activity == '') {
        error_message = select_redirect_txt;
        slider_banner_error = true;
        $('select[name="redirect_activity"]').addClass('kb_error_field');
        $('select[name="redirect_activity"]').after('<span class="kb_error_message">' + select_redirect_txt + '</span>');
    ...
    if (redirect_activity == 'layout' && redirect_layout_type_id == '0') {
        error_message = select_layout_txt;
        slider_banner_error = true;
        $('select[name="layout_type_id"]').addClass('kb_error_field');
        $('select[name="layout_type_id"]').after('<span class="kb_error_message">' + select_layout_txt + '</span>');
...
function sendNotification()
{
    ...
    /*changes made by vibhaas to add the value of website url */
    var redirect_website_url = $.trim($('#push_notification_redirect_website_url').val());
    /*chnages ends here */
...
function delete_confirmation_layout(a)
{
    if (confirm(confirmation_txt + '?'))
    {
        ...
        var default_layout = $("#KBMOBILEAPP_HOME_PAGE_LAYOUT").val();

        if(code == default_layout)
        {
            showErrorMessage(delete_layout_default);
            return false;
        }

        var code = $(a).closest('tr').find('.td-vss-code').html();
        
        //var default_layout = '<?php echo (Configuration::get("KBMOBILEAPP_HOME_PAGE_LAYOUT")); ?>' ;
        $('.alert-success').parent().remove();
        $(a).closest('tr').remove();
...
function showPreview() {
    ...
    {   
        ...
        $('div[id="rightText"]').text(show_preview);
    } else {
        $('#general_settings_mobile_preview').find('.front_preview').show();
        $('div[id="rightText"]').text(hide_preview);
    }
}
/*end:changes made by Aayushi Agarwal on 27th March 2020 to make preview changes*/

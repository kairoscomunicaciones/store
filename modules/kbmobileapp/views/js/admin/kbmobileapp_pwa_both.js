...
$(document).ready(function()
{
    ...
    else if ( default_tab == 'GoogleSetup') {
    ...
    $('#KB_MOBILE_APP_ADD_LOGO_NAVIGATION').on('change', function(e) {
        ...
        if ($(this)[0].files !== undefined && $(this)[0].files.length > 0)
        {
            ...
            if (!file_format)
            {
                $('input[name="KB_MOBILE_APP_ADD_LOGO_NAVIGATION"]').parent().append('<span class="kb_error_message">'+invalid_file_format_txt+'</span>');
                logo_navigation_file_error = true;

            } else if (files.size > default_file_size) {
                $('input[name="KB_MOBILE_APP_ADD_LOGO_NAVIGATION"]').parent().append('<span class="kb_error_message">'+file_size_error_txt+'</span>');
                logo_navigation_file_error = true;
            } else {
                ...
                $('input[name="KB_MOBILE_APP_ADD_LOGO_NAVIGATION"]').parent().find('.kb_error_message').remove();
            }

        }
    });
    /*start: changes made by Aayushi Agarwal on 23rd July 2019 to make the MAB compatible with PWA*/
    
    $('#KB_MOBILE_APP_ICON').on('change', function(e) {
        $('.kb_error_message').remove();
        $('input[name="filename"]').removeClass('kb_error_field')
        if ($(this)[0].files !== undefined && $(this)[0].files.length > 0)
        {
            ...
            if (!file_format)
            {
                $('input[name="KB_MOBILE_APP_ICON"]').parent().append('<span class="kb_error_message">'+invalid_file_format_txt+'</span>');
                logo_navigation_file_error = true;

            } else if (files.size > default_file_size) {
                $('input[name="KB_MOBILE_APP_ICON"]').parent().append('<span class="kb_error_message">'+file_size_error_txt+'</span>');
                logo_navigation_file_error = true;
            } else {
                logo_navigation_file_error = false;
                if (typeof (FileReader) != "undefined") {

                    var image_holder = $("#app_icon_image");

                    image_holder.empty();

                    var reader = new FileReader();
                    reader.onload = function(e) {
                        $('#app_icon_image').attr('src', e.target.result);
                    }
                    image_holder.show();
                    reader.readAsDataURL($(this)[0].files[0]);
                }
                $('input[name="KB_MOBILE_APP_ICON"]').parent().find('.kb_error_message').remove();
            }
...
//    }
    ...
    $('#redirect_product_name').autocomplete(ajaxaction + '&configure=kbmobileapp&ajaxproductaction=true', {
        ...
        $('#redirect_product_id').val(array[1]);
        $('#redirect_product_name').val(array[0]);
    });
...
function veloValidateConfigForms(button_ele)
{
    ...
    $('input[name="KB_MOBILEAPP_PWA_APP_NAME"]').removeClass('kb_error_field');
    /*start: changes made by Aayushi Agarwal on 23rd July 2019 to make the MAB compatible with PWA*/
    $('input[name="KB_MOBILEAPP_FIREBASE_AUTH_DOMAIN"]').removeClass('kb_error_field');
    $('input[name="KB_MOBILEAPP_FIREBASE_DATABASE_URL"]').removeClass('kb_error_field');
    $('input[name="KB_MOBILEAPP_FIREBASE_PROJECT_ID"]').removeClass('kb_error_field');
    $('input[name="KB_MOBILEAPP_FIREBASE_MSG_SENDER_ID"]').removeClass('kb_error_field');
    $('input[name="KB_MOBILEAPP_FIREBASE_APP_ID"]').removeClass('kb_error_field');
    /*end: changes made by Aayushi Agarwal on 23rd July 2019 to make the MAB compatible with PWA*/
    ...
    if ($('input[name="push_notification[create_order][status]"]:checked').val() == "1" || $('input[name="push_notification[order_status_change][status]"]:checked').val() == "1" || $('input[name="push_notification[abandoned_cart][status]"]:checked').val() == "1") {
        ...
        /*start: changes made by Aayushi Agarwal on 23rd July 2019 to make the MAB compatible with PWA*/
        var app_name_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_PWA_APP_NAME"]'));
        if (app_name_mandatory_err != true)
        {
            is_error = true;
            general_setting_error = true;
            $('input[name="KB_MOBILEAPP_PWA_APP_NAME"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_PWA_APP_NAME"]').after('<span class="kb_error_message">' + app_name_mandatory_err + '</span>');
        }
        
        var auth_domain_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_FIREBASE_AUTH_DOMAIN"]'));
        if (auth_domain_mandatory_err != true)
        {
            is_error = true;
            push_notification_error = true;
            $('input[name="KB_MOBILEAPP_FIREBASE_AUTH_DOMAIN"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_FIREBASE_AUTH_DOMAIN"]').after('<span class="kb_error_message">' + auth_domain_mandatory_err + '</span>');
        }
        
        var database_url_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_FIREBASE_DATABASE_URL"]'));
        if (database_url_mandatory_err != true)
        {
            is_error = true;
            push_notification_error = true;
            $('input[name="KB_MOBILEAPP_FIREBASE_DATABASE_URL"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_FIREBASE_DATABASE_URL"]').after('<span class="kb_error_message">' + database_url_mandatory_err + '</span>');
        }
        
        var project_id_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_FIREBASE_PROJECT_ID"]'));
        if (project_id_mandatory_err != true)
        {
            is_error = true;
            push_notification_error = true;
            $('input[name="KB_MOBILEAPP_FIREBASE_PROJECT_ID"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_FIREBASE_PROJECT_ID"]').after('<span class="kb_error_message">' + project_id_mandatory_err + '</span>');
        }
        
        var msg_sender_id_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_FIREBASE_MSG_SENDER_ID"]'));
        if (msg_sender_id_mandatory_err != true)
        {
            is_error = true;
            push_notification_error = true;
            $('input[name="KB_MOBILEAPP_FIREBASE_MSG_SENDER_ID"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_FIREBASE_MSG_SENDER_ID"]').after('<span class="kb_error_message">' + msg_sender_id_mandatory_err + '</span>');
        }
        
        var app_id_mandatory_err = velovalidation.checkMandatory($('input[name="KB_MOBILEAPP_FIREBASE_APP_ID"]'));
        if (app_id_mandatory_err != true)
        {
            is_error = true;
            push_notification_error = true;
            $('input[name="KB_MOBILEAPP_FIREBASE_APP_ID"]').addClass('kb_error_field');
            $('input[name="KB_MOBILEAPP_FIREBASE_APP_ID"]').after('<span class="kb_error_message">' + app_id_mandatory_err + '</span>');
        }
        /*end: changes made by Aayushi Agarwal on 23rd July 2019 to make the MAB compatible with PWA*/
    }
...
//    }
    if ($('input[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]:checked').val() == "1" && $('input[name="KB_MOBILE_APP_CHAT_SUPPORT"]:checked').val() == "1") {
       ...
       general_setting_error = true;
       $('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]').addClass('kb_error_field');
       $('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]').after('<span class="kb_error_message">' + 'Only one char option can be enabled at a time' + '</span>');
       $('input[name="KB_MOBILE_APP_CHAT_SUPPORT_KEY"]').addClass('kb_error_field');
       $('input[name="KB_MOBILE_APP_CHAT_SUPPORT_KEY"]').after('<span class="kb_error_message">' + 'Only one chat option can be enabled at a time' + '</span>');
   } else {
       if ($('input[name="KB_MOBILE_WHATSAPP_CHAT_SUPPORT"]:checked').val() == "1") {
           var chat_number = velovalidation.checkMandatory($('input[name="KB_MOBILE_WHATSAPP_CHAT_NUMBER"]'));
           if (chat_number != true)
...
function veloValidateGoogleSetupForm(button_ele)
{
    ...
    $('.kb_error_message').remove();
    //$('#googlejsonfile-name').removeClass('kb_error_field');
    $('#google_setup_app_id').removeClass('kb_error_field');
    var google_error = false;
    var google_file = $.trim($('#google_setup_app_id').val());
//    var uploadedfile = $.trim($('#googlefilename').val());
    ...
    if (status == 1) {
        if (google_file == '') {
            google_error = true;
            error_message = empty_google_key;
            $('#google_setup_app_id').addClass('kb_error_field');
            $('#google_setup_app_id').parent().append('<span class="kb_error_message">' + error_message + '</span>');
            return false;
        } else if ($("#facebook_setup_app_id").val() == $("#google_setup_app_id").val()) {
            $('#google_setup_app_id').addClass('kb_error_field');
            $('#google_setup_app_id').parent().append('<span class="kb_error_message">' + can_not_be_same + '</span>');
            return false;
        }
    }
...
function veloValidateFacebookSetupForm(button_ele)
{
    ...
    $('.kb_error_message').remove();
    $('input[name="facebook_setup_app_id"]').removeClass('kb_error_field');

    var status = $('input[name="facebook_setup_status"]:checked').val();
    if (status == 1) {
        var facebook_id_mandatory_err = velovalidation.checkMandatory($('input[name="facebook_setup_app_id"]'));
        if (facebook_id_mandatory_err != true)
        {
            $('input[name="facebook_setup_app_id"]').addClass('kb_error_field');
            $('input[name="facebook_setup_app_id"]').after('<span class="kb_error_message">' + facebook_id_mandatory_err + '</span>');
            return false;
        } else if ($("#facebook_setup_app_id").val() == $("#google_setup_app_id").val()) {
            $('input[name="facebook_setup_app_id"]').addClass('kb_error_field');
            $('input[name="facebook_setup_app_id"]').after('<span class="kb_error_message">' + can_not_be_same + '</span>');
            return false;
        } else {
            validateFBKey(true);
            return false;
...
function validateFBKey(submitform) {
        ...
        //$('#facebook_setup_app_id').html();
        $('#facebook_setup_app_id').removeClass('kb_error_field');
        $('#facebook_setup_app_id').removeClass('kb-fb-success');
        $.ajax({
            ...
            {
                if ($.trim(data) == 'false') {
                    $('#facebook_setup_app_id').addClass('kb_error_field');
                    $('#facebook_setup_app_id').parent().append('<span class="kb_error_message">' + invalid_key_txt + '</span>');
                    $('#facebook_setup_app_id').removeClass('kb-fb-success');
...
//                    $("#facebook_app_error").html(invalid_key_txt);
                    ...
                    $('#facebook_setup_app_id').addClass('kb-fb-success');
                    $('#facebook_setup_app_id').removeClass('kb_error_field');
                    $("#facebook_setup_app_id").html(valid_key_txt);
                    if (submitform) {
                        $('#facebook_setup_form').submit();
                    }
                }

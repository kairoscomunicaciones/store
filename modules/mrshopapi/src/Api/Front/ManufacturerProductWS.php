...
use MrAPPs\MrShopApi\Handler\Search\ManufacturerHandler;
use Tools;
use Validate;

class ManufacturerProductWS extends ProductBaseWS
{
    public function __construct($dataHandler)
    {
        parent::__construct($dataHandler);
        $this->allowCacheParameters(['manufacturer_id']);
    }
    ...
    public function getList($params, $id_customer = null, $echoResponse = true)
    {
        ...
        $manufacturerId = array_key_exists('manufacturer_id', $params) ? $params['manufacturer_id'] : null;
        $manufacturer = new Manufacturer((int) $manufacturerId);

        $handler = new ManufacturerHandler($manufacturer);
    ...
    private function getList16($params, $id_customer = null, $echoResponse = true)
    {
        ...
        if (!Validate::isLoadedObject($manufacturer)) {
            ResponseHandler::notFound($this->module->l('Manufacturer not found', 'manufacturerproductws'));
        }
        ...
        $orderOptions = explode('.', $order);

        if ($orderOptions !== null && count($orderOptions) === 3) {
            $orderBy = $orderOptions[1];
            $orderWay = Tools::strtoupper($orderOptions[2]);
        } else {
            // Default sort
            $order = Tools::strtolower(trim(Configuration::get('MRSHOP_DEFAULT_ORDER_MANUFACTURER_PRODUCTS')));
        ...
        $items = Manufacturer::getProducts(
            $manufacturer->id,
        ...
        $count = Manufacturer::getProducts(
            $manufacturer->id,
            (int) Context::getContext()->language->id,
            $page,
            $limit,
        ...
        $sortOrders = $count > 0 ? [
            [
                ...
                'label' => $this->module->l('Name, A to Z', 'manufacturerproductws'),
            ],
            [
                'encoded_order' => 'product.name.desc',
                'label' => $this->module->l('Name, Z to A', 'manufacturerproductws'),
            ],
            [
                'encoded_order' => 'product.price.asc',
                'label' => $this->module->l('Price, lowest first', 'manufacturerproductws'),
            ],
            [
                'encoded_order' => 'product.price.desc',
                'label' => $this->module->l('Price, highest first', 'manufacturerproductws'),
            ],
        ] : [];

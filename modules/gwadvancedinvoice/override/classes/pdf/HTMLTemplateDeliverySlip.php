...
class HTMLTemplateDeliverySlip extends HTMLTemplateDeliverySlipCore
{
    public function getHeaderGw($data){
        ...
        if(Configuration::get('GWADVANCEDINVOICE_ACTIVE', null, $id_shop_group, $id_shop)){
            ...
            include_once(_PS_MODULE_DIR_.'/gwadvancedinvoice/model/gwadvancedinvoicetemplateModel.php');
            $template_by = Configuration::get('GWADVANCEDDELIVERY_CUSTOMER_TEMPLATE_TYPE', null, $id_shop_group, $id_shop);
            if($template_by == 'payment_method'){
                $id_payment = Module::getModuleIdByName($this->order->module);
                $_id_template = (int)Configuration::get('GWADVANCEDDELIVERY_PAYMENT_TEMPLATE_'.(int)$id_payment, null, $id_shop_group, $id_shop);
                if($_id_template > 0){$id_template = (int)$_id_template;} 
            }elseif($template_by == 'order_status'){
                $current_state = $this->order->current_state;
                $_id_template = (int)Configuration::get('GWADVANCEDDELIVERY_STATUS_TEMPLATE_'.(int)$current_state, null, $id_shop_group, $id_shop);
                if($_id_template > 0){$id_template = (int)$_id_template;}
    ...
    public function assignData($id_template){
		...
		foreach ($order_details as  &$order_detail)
		{
		    ...
		    if(version_compare(_PS_VERSION_,'1.7.5') >= 0)
                if(method_exists('StockAvailable','getLocation')){

...
class AdminGwadvancedinvoicetemplateController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        $this->toolbar_btn['exportgtemplate'] = array(
			'href' => 'submitExportgtemplate',
			'desc' => $this->l('Export Templates'),
		);
    }
	...
	{
	   if(Tools::isSubmit('submitGexport_template'))
       {
	       $gid_templates = Tools::getValue('gid_templates');
           if($gid_templates !=''){
                echo $this->module->downloadExportTemplate($gid_templates);
           }
           die();
	   }
       elseif (Tools::isSubmit('submitGimport_template')){
           ...
           if (isset($_FILES['zipfile']['name']) && !empty($_FILES['zipfile']['name']) && !empty($_FILES['zipfile']['tmp_name']))
    		{
               ...
               {
                    $temp_name = tempnam($filepath,'gtemplate');
                    ...
                    if (!Tools::ZipTest($temp_name)){
                        ...
                        $result = $this->module->importGTemplate($salt.'.zip');
                        if(isset($result['error']) && $result['error'] != 0){
                            $this->errors[] = $result['warrning'];
                        }else{
                            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGwadvancedinvoicetemplate', true,array(),array('conf'=>4)));
                        }
                    }
               }else{
       ...
       elseif(Tools::isSubmit('submitAjaxCall')){
        		...
        		foreach ($languages as $lang)
        		{
                      ...
                      if($titles)
                        foreach($titles as &$title){
                            $title = htmlentities($title,ENT_COMPAT| ENT_IGNORE, "UTF-8");
                        }
                      // #fix utf8 title error when json_encode
            ...
            foreach ($languages as $language)
      		{
				...
				)
				{
					...
					if (isset($temp_name))
						@unlink($temp_name);
                    if($template && isset($template->watermark[$language['id_lang']]) && $template->watermark[$language['id_lang']] !=''){
      		            if(file_exists(_PS_MODULE_DIR_.$this->module->name.'/views/img/watermark/'.$template->watermark[$language['id_lang']]))
                          @unlink(_PS_MODULE_DIR_.$this->module->name.'/views/img/watermark/'.$template->watermark[$language['id_lang']]);
                    }
    ...
    public function renderList()
    {
        ...
        $template = array();
        $template = gwadvancedinvoicetemplateModel::getAllBlock();
        $action = '';
        Context::getContext()->smarty->assign('template',count($template));
        if (count($template) <= 0) {
            $action = $this->context->link->getAdminLink('AdminGwadvancedinvoicetemplate', false).'&addgwadvancedinvoicetemplate&token='.Tools::getAdminTokenLite('AdminGwadvancedinvoicetemplate');
            Context::getContext()->smarty->assign('action',$action);
            $tpl = _PS_MODULE_DIR_.'gwadvancedinvoice/views/templates/admin/notemplate.tpl';
            $no_template = Context::getContext()->smarty->fetch($tpl);

            $import_template = '';
            $extra = _PS_MODULE_DIR_.'gwadvancedinvoice/views/templates/admin/extrahtml.tpl';
            Context::getContext()->smarty->assign('form_import_template',1);
            $import_template = Context::getContext()->smarty->fetch($extra);
            Context::getContext()->smarty->assign('form_import_template',0);

            return $no_template.$import_template;
        }

        $import_template = '';
        $tpl = _PS_MODULE_DIR_.'gwadvancedinvoice/views/templates/admin/extrahtml.tpl';
        Context::getContext()->smarty->assign('form_import_template',1);
        $import_template = Context::getContext()->smarty->fetch($tpl);
        Context::getContext()->smarty->assign('form_import_template',0);
        return parent::renderList().$import_template;
    }
    public function renderForm()
    {
        if (!Tools::isSubmit('choose_design') && !Tools::isSubmit('id_gwadvancedinvoicetemplate'))
		{
            $ginvoice = Module::getInstanceByName('gwadvancedinvoice');
            $ginvoice_pagesize = $ginvoice->hookAllSize();
	        ...
	        $this->fields_form = array(
                ...
                'input' => array(
                    array(
                        'type' => 'sidebar',
                        'labels' => $this->l('Page size'),
                        'hints' => $this->l('Select a page size for your invoice template.'),
                        'classs' => 'pagesize',
                        'name' => 'pagesize',
                        'options' => array(
                            'query' => $ginvoice_pagesize,
                            'id' => 'value',
                        )),
                ),
                'submit' => array(
                    'title' => $this->l('Next'), 
                    'name' =>'chooseTemplate',
                    'class'=>'btn btn-default pull-right chooseTemplate_ajaxcall hide'
                ),
            );
		    ...
		    if(!Tools::isSubmit('id_gwadvancedinvoicetemplate')){
                ...
                foreach ($languages as $lang)
        		{
        		    $fields['productcolumns'][$lang['id_lang']] = Tools::jsonDecode(Tools::jsonEncode($base_design['productcolumns']),true);
        		}
        		...
        		{
  		            // fix utf8 title error when json_encode
                    $productcolumns = Tools::jsonDecode($template->productcolumns[$lang['id_lang']], true);
            ...
            $urlimg = _MODULE_DIR_.'gwadvancedinvoice/views/img/';
            ...
            $this->fields_form = array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'desc' => $this->l('Select barcode type for the Invoice'),
                        'name' => 'barcodetype',
                        'required' => false,
                        'lang' => false,
                        'class' => 'barcodetype',
                    ...
                    array(
                        ...
                        'desc' => $this->l('Select barcode type for the Product'),
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Hidden Discount after product list'),
                        'name' => 'hidden_discount',
                        'is_bool' => true,
                        'values' => array(array(
                                'id' => 'hidden_discount_on',
                                'value' => 1,
                                'label' => $this->l('Yes')), array(
                                'id' => 'hidden_discount_off',
                                'value' => 0,
                                'label' => $this->l('No')))),
                    array(
                        'type' => 'select',

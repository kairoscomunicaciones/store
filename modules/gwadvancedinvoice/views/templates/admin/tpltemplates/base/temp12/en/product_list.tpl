    ...
    <thead>
    	<tr >
            {foreach $titles as $key=>$value}
                <th class="header_title product header small {if $align[$key] !=''} align{$align[$key]|escape:'html':'UTF-8'} {else} alignleft {/if}" width="{$widthtitles[$key]|escape:'html':'UTF-8'}%">{$titles[$key]|escape:'html':'UTF-8'}</th>
            {/foreach}
        </tr>
    </thead>
	<tbody>

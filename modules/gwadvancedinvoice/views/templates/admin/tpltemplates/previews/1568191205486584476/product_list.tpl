...
<table class="product" width="100%" cellpadding="5" cellspacing="0">
    <thead>
    	<tr>
                            <th class="product header small  aligncenter " width="%">Barcode</th>
                            <th class="product header small  alignleft " width="%">Product</th>
                            <th class="product header small  aligncenter " width="%">Unit Price</th>
                            <th class="product header small  aligncenter " width="%">Qty</th>
                            <th class="product header small  aligncenter " width="%">Total Price</th>
                    </tr>
    </thead>
	<tbody>
	...
	{foreach $order_details as $order_detail}
		...
		<tr class="product {$bgcolor_class}">
			               ...
			               <td class="product  aligncenter "  width="%">{$order_detail.barcode}</td>
                           <td class="product  alignleft "  width="%"><p class="product_name">{$order_detail.product_name}</p>{$order_detail.description_short}</td>
                           <td class="product  aligncenter "  width="%">{displayPrice currency=$order->id_currency price=$order_detail.unit_price_tax_excl_including_ecotax}</td>
                           <td class="product  aligncenter "  width="%">{$order_detail.product_quantity}</td>
                           <td class="product  aligncenter "  width="%">{displayPrice currency=$order->id_currency price=$order_detail.total_price_tax_excl_including_ecotax}</td>
            		</tr>

...
class Ets_pmn_defines
{
    ...
    public function _installDb(){
        ...
        `id_shop` INT(11),
        `name` VARCHAR(1000) NOT NULL , 
        `excluded` VARCHAR(1000) NOT NULL , 
    ...
    public function getListCategories($id_product)
    {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-categories.tpl');
    }
    public function getListFeatures($id_product)
    {
        ...
        if($features)
        {
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-features.tpl');
        }
        else
            return '--';
    }
    public function getListCombinations($id_product)
    {
        ...
        LEFT JOIN `'._DB_PREFIX_.'stock_available` sa ON (pa.id_product_attribute = sa.id_product_attribute AND sa.id_shop="'.(int)Context::getContext()->shop->id.'")
        WHERE pas.id_shop="'.(int)Context::getContext()->shop->id.'" AND pa.id_product="'.(int)$id_product.'"');
        if($product_attributes)
        {
            foreach($product_attributes as &$product_attribute)
            {
                $product_attribute['name'] = $this->getProductAttributeName($product_attribute['id_product_attribute']);
                $product_attribute['price'] = Tools::displayPrice($product_attribute['price']);
            }
            $this->smarty->assign(
                array(
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-attributes.tpl');
        }
        else
            return '';
    }
    ...
    public function getListSpecificPrices($id_product)
    {
        ...
        if($specific_prices)
        {
            foreach($specific_prices as &$specific_price)
            {
                ...
                {
                    $specific_price['price'] = Tools::displayPrice($specific_price['price']);
                }
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-specific-prices.tpl');
        }
        else{
            return Module::getInstanceByName('ets_productmanager')->displayText('--','span',array('class'=>'text-center'));
        }
    }
    public function getListtags($id_product,$html = true)
    {
        ...
        WHERE pt.id_product="'.(int)$id_product.'" AND pt.id_lang="'.(int)$this->context->language->id.'"';
        ...
        if($tags)
        {
            if($html)
            {
                ...
                return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-tags.tpl');
            }
            else
            {
                $text = '';
    ...
    public function getListAttachments($id_product)
    {
        ...
        if($attachments)
        {
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-attachments.tpl');
        }
        else
            return '--';
    }
    public function getListSuppliers($id_product)
    {
        ...
        if($suppliers)
        {
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-suppliers.tpl');
        }
        else
            return '--';
    }
    public function getListCustomizations($id_product)
    {
        ...
        if($customizations)
        {
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->name.'/views/templates/hook/list-customizations.tpl');
        }
        else
            return '--';
    }
    ...
    {
        require_once dirname(__FILE__).'/../../ets_extraproducttabs/classes/EtsEptTab.php';
        require_once dirname(__FILE__).'/../../ets_extraproducttabs/classes/EtsEptProduct.php';
        $etsEptTab = new EtsEptTab($id_tab,$this->context->language->id);
        $onlyproduct = false;
        if(!$etsEptTab->is_global)
        {
            
            if(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_ept_product` WHERE id_ets_ept_tab="'.(int)$id_tab.'" AND id_product="'.(int)$id_product.'" AND id_lang="'.($id_lang ? (int)$id_lang: (int)$this->context->language->id).'"'))
            {
                $content = Db::getInstance()->getValue('SELECT '.($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_FILE ? 'file_name' :'content').' FROM `'._DB_PREFIX_.'ets_ept_product` WHERE id_ets_ept_tab="'.(int)$id_tab.'" AND id_product="'.(int)$id_product.'" AND id_lang="'.($id_lang ? (int)$id_lang: (int)$this->context->language->id).'"');
                $onlyproduct=true;
            }
            else
            {
                $content = $etsEptTab->tab_type==EtsEptTab::TAB_TYPE_FILE ? $etsEptTab->file_name: $etsEptTab->content;
                
                if($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_RADIO || $etsEptTab->tab_type==EtsEptTab::TAB_TYPE_SELECT || $etsEptTab->tab_type==EtsEptTab::TAB_TYPE_CHECKBOX)
                   $content = Module::getInstanceByName('ets_extraproducttabs')->getOptionSelect($content,$etsEptTab->tab_type);
            }
        }
        else
        {
            if(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_ept_product` WHERE id_ets_ept_tab="'.(int)$id_tab.'" AND id_product="'.(int)$id_product.'" AND use_global_content=0 AND id_lang="'.($id_lang ? (int)$id_lang: (int)$this->context->language->id).'"'))
            {
                $content= Db::getInstance()->getValue('SELECT '.($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_FILE ? 'file_name' :'content').' FROM `'._DB_PREFIX_.'ets_ept_product` WHERE id_ets_ept_tab="'.(int)$id_tab.'" AND id_product="'.(int)$id_product.'" AND id_lang="'.($id_lang ? (int)$id_lang: (int)$this->context->language->id).'"');
                $onlyproduct = true;
            }
        ...
        if($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_RICH_TEXT || $etsEptTab->tab_type==EtsEptTab::TAB_TYPE_HTML)
        {
            $content = strip_tags($content);
        }
        elseif($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_FILE) // file
        {
            $content = Module::getInstanceByName('ets_productmanager')->displayText($content,'a',array('class'=>'check_input_color','target'=>'_blank','href'=> $this->context->link->getModuleLink('ets_extraproducttabs','download',array('idTab'=>$etsEptTab->id)).($onlyproduct ? '&idProduct='.(int)$id_product.'&onlyProduct=1':'') ));
        }
        elseif($etsEptTab->tab_type==EtsEptTab::TAB_TYPE_IMAGE && $content) // image
        {
            $content = Module::getInstanceByName('ets_productmanager')->displayText('','img',array('src'=>$this->context->shop->getBaseURI().'img/ets_extraproducttabs/'.$content));;
        }
        return $content;
    }
    ...
    {
        if(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_seo_product` WHERE id_product='.(int)$id_product.' AND id_lang='.(int)$id_lang))
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'ets_seo_product` SET key_phrase ="'.pSQL($focus_keyphrase).'" WHERE id_product='.(int)$id_product.' AND id_lang='.(int)$id_lang);
        else
            Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'ets_seo_product`(id_product,id_lang,key_phrase) VALUES("'.(int)$id_product.'","'.(int)$id_lang.'","'.pSQL($focus_keyphrase).'")');
    }
    public static function updateMinorKeyPhrase($id_product,$id_lang,$minor_key_phrase)
    {
        if(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_seo_product` WHERE id_product='.(int)$id_product.' AND id_lang='.(int)$id_lang))
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'ets_seo_product` SET minor_key_phrase ="'.pSQL($minor_key_phrase).'" WHERE id_product='.(int)$id_product.' AND id_lang='.(int)$id_lang);
        else
            Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'ets_seo_product`(id_product,id_lang,minor_key_phrase) VALUES("'.(int)$id_product.'","'.(int)$id_lang.'","'.pSQL($minor_key_phrase).'")');
    }
    ...
    public function displayListCombinations($id_product)
    {
        ...
        if($productAttributes)
        {
            foreach($productAttributes as &$productattribute)
            {
                ...
                {
                    $tax = $this->getTaxValue($product->id_tax_rules_group);
        ...
        return  $this->context->smarty->fetch(_PS_MODULE_DIR_.'ets_productmanager/views/templates/hook/product/list_combinations.tpl');
    }
                ...
                if($attributeGroup['is_color_group'] && $attributeGroup['attributes'])
                {
                    foreach($attributeGroup['attributes'] as &$attribute)
                    {
                        if(file_exists(_PS_COL_IMG_DIR_.$attribute['id_attribute'].'.jpg'))
                            $attribute['image'] = $this->getBaseLink().'/img/co/'.$attribute['id_attribute'].'.jpg';
                    }
                }
            }
        }
    ...
    public function getStockAvailables($id_product)
    {
        ...
        INNER JOIN `'._DB_PREFIX_.'product_attribute_shop` pas ON (pa.id_product_attribute = pas.id_product_attribute)
        LEFT JOIN `'._DB_PREFIX_.'stock_available` sa ON (sa.id_product_attribute = pa.id_product_attribute and sa.id_shop="'.(int)$this->context->shop->id.'")
    ...
    public function renderSpecificPrice($id_product,$id_specific_price=0)
    {
        ...
        if($productAttributes)
        {
            foreach($productAttributes as &$productattribute)
            {
                ...
                $productattribute['name_attribute'] = Ets_pmn_defines::getInstance()->getProductAttributeName($productattribute['id_product_attribute']);
                $attribute_images = Db::getInstance()->executeS('SELECT id_image FROM `'._DB_PREFIX_.'product_attribute_image` WHERE id_product_attribute='.(int)$productattribute['id_product_attribute']);
                $productattribute['images'] = array();
                if($attribute_images)
                {
    ...
    public function getRelatedProducts($id_product)
    {
        ...
        if($products)
        {
            ...
            return $this->context->smarty->fetch(_PS_MODULE_DIR_.'ets_productmanager/views/templates/hook/product/related_products.tpl');
        }
        return '--';
    }
        ...
        if($related_products)
        {
            ...
            foreach($related_products as &$related_product)
            {
                if(!$related_product['id_image'])
                    $related_product['id_image'] = $this->getIdImageByIdProduct($related_product['id_product']);
                if($related_product['id_image'])
    ...
    public function getIdImageByIdProduct($id_product)
    {
    ...
    {
        return Db::getInstance()->getValue('SELECT '.($key=='focus_keyphrase' ? 'key_phrase':'minor_key_phrase').' FROM `'._DB_PREFIX_.'ets_seo_product` WHERE id_product="'.(int)$id_product.'" AND id_lang='.(int)$id_lang);
    }

...
class AdminProductManagerAjaxController extends ModuleAdminController
{
    ...
    public function initContent()
    {
        ...
        if(Tools::isSubmit('submitCustomizationProduct') && ($id_product = (int)Tools::getValue('id_product')) && ($custom_fields = Tools::getValue('custom_fields')) && Ets_productmanager::validateArray($custom_fields) )
        {
        ...
        if(Tools::isSubmit('unitProductAction') && ($id_product= (int)Tools::getValue('id_product')) && ($action_field = Tools::getValue('action_field')) && in_array($action_field,array('active','on_sale','is_must_have')))
        {
            ...
            elseif(Module::isEnabled('ets_customfields'))
            {
                require_once dirname(__FILE__) . '/../../../ets_customfields/classes/EtsCzfProduct.php';
                $czfProduct  = Module::getInstanceByName('ets_customfields')->getCzfProductByProductId($id_product,$this->context->language->id);
                if($czfProduct && $czfProduct['id_ets_czf_product'])
                    $czfProduct_class = new EtsCzfProduct($czfProduct['id_ets_czf_product']);
                else
                    $czfProduct_class = new EtsCzfProduct();
                $czfProduct_class->is_must_have = (int)Tools::getValue('value_field'); 
                if($czfProduct_class->id)
                {
                    $czfProduct_class->update();
                }
                else
                {
                    $czfProduct->id_product = $id_product;
                    $czfProduct_class->add();
                }
    ...
    public function _checkBeforSubmitProduct()
    {
        ...
        if(in_array('sav_quantity',$list_fields) && Tools::isSubmit('sav_quantity') && ($sav_quantity = Tools::getValue('sav_quantity')) && !Validate::isInt($sav_quantity))
        {
    ...
    public function _submitProductChangeInLine($product)
    {
        ...
        if($list_fields)
        {
            foreach($list_fields as $key => $field)
            {
                if($key !='tax_name' && property_exists($product,$key) && isset($field['input']) && ($input = $field['input']))
                {
                    ...
                    if(isset($input['lang']) && $input['lang'])
                    {
                        ...
                        else
                        {
                            ...
                            $columns[] = array(
                                'name' => $key,
                                'value' =>$key=='name' ? Module::getInstanceByName('ets_productmanager')->displayText($product->{$key}[$this->context->language->id],'a',array('href'=>$this->context->link->getAdminLink('AdminProducts',true,array('id_product'=>$product->id)).'#tab-step1')) : $product->{$key}[$this->context->language->id],
                            );
                        }
                    }
                    else
                    {
                        if($key=='price')
                        {
                            ...
                            $columns[] = array(
                                'name' => $key,
                                'value' => Module::getInstanceByName('ets_productmanager')->displayText(
                ...
                elseif($key =='price_final')
                {
                    ...
                    $columns[] = array(
                        'name' => $key,
                        'value' => Module::getInstanceByName('ets_productmanager')->displayText(Tools::displayPrice($price_final),'a',array('href'=>$this->context->link->getAdminLink('AdminProducts',true,array('id_product'=>$product->id)).'#tab-step2')) ,
                    );
                }
                if($key=='tags')
                {
        ...
        if($product->update())
        {
            ...
            if(Tools::isSubmit('sav_quantity') && ($sav_quantity = Tools::getValue('sav_quantity'))!='' && Validate::isInt($sav_quantity))
            {
                ...
                $columns[] = array(
                    'name' => 'sav_quantity',
                    'value' => Module::getInstanceByName('ets_productmanager')->displayText($sav_quantity,'a',array('href'=>$this->context->link->getAdminLink('AdminProducts',true,array('id_product'=>$product->id)).'#tab-step3')),
                );
            }
    ...
    public function updateExtraField($id_product,$list_fields,&$columns)
    {
        ...
        foreach($list_fields as $key=> $field)
        {
            ...
            if(isset($czfProduct_class))
            {
                if(isset($czfProduct_class->{$key}) && $key!='name' && $key!='description' && $key!='doc_name')
                {
                    if(isset($field['input']['lang']) && $field['input']['lang'])
                    {
                        $val_default = Tools::getValue($key.'_'.$id_lang_default);
                        foreach($languages as $language)
                        {
                            $val = Tools::getValue($key.'_'.$language['id_lang']);
                            $czfProduct_class->{$key}[$language['id_lang']] = $val && Validate::isCleanHtml($val) ? $val : ($val_default && Validate::isCleanHtml($val_default) ? $val_default :'');
                        }
                        $columns[] = array(
                            'name' => $key,
                            'value' =>$czfProduct_class->{$key}[$this->context->language->id],
                        );
                    }
                    else
                    {
                        $czfProduct_class->{$key} = Tools::getValue($key);
                        $columns[] = array(
                            'name' => $key,
                            'value' =>$czfProduct_class->{$key},
                        );
                    }
                }
                elseif($key=='module_name')
                {
                    $post_ets_customfields = true;
                    $val_default = Tools::getValue($key.'_'.$id_lang_default);
                    foreach($languages as $language)
                    {
                        $val = Tools::getValue($key.'_'.$language['id_lang']);
                        $czfProduct_class->display_name[$language['id_lang']] = $val && Validate::isCleanHtml($val) ? $val : ($val_default && Validate::isCleanHtml($val_default) ? $val_default :'');
                    }
                    $columns[] = array(
                        'name' => $key,
                        'value' =>$czfProduct_class->display_name[$this->context->language->id],
                    );
                }
                elseif($key=='module_description')
                {
                    ...
                    {
                        $val = Tools::getValue($key.'_'.$language['id_lang']);
                        $czfProduct_class->description[$language['id_lang']] = $val && Validate::isCleanHtml($val) ? $val : ($val_default && Validate::isCleanHtml($val_default) ? $val_default :'');
                    }
                    $columns[] = array(
                        'name' => $key,
                        'value' =>$czfProduct_class->description[$this->context->language->id],
                    );
                }
                $post_ets_customfields = true;
            }
        ...
        if(isset($post_ets_customfields) && $post_ets_customfields && Module::isEnabled('ets_customfields'))
        {
            if($czfProduct_class->id)
                $czfProduct_class->update();
            else
            {
                $czfProduct_class->id_product = $id_product;
                $czfProduct_class->add();
            }
        }
    }
    public function _submitQuanittyProduct($id_product)
    {
        ...
        die(
            json_encode(
                array(
                    'success' => $this->l('Updated successfully'),
                    'row_value' => Module::getInstanceByName('ets_productmanager')->displayText($sav_quantity,'a',array('href'=>$this->context->link->getAdminLink('AdminProducts',true,array('id_product'=>$id_product)).'#tab-step3')),
            ...
            if($image->delete())
            {
                ...
                if($image->cover)
                {
                    ...
                    if($images = Ets_pmn_defines::getInstance()->getListImages($image->id_product))
                    {
                        ...
                        $product = new Product($newImage->id_product,false,$this->context->language->id);
                        $newImage->cover=1;
                        $newImage->update();
                    }
                }
    ...
    public function _submitProductAttachment($id_product)
    {
        ...
        elseif(($product = new Product($id_product)) && !Validate::isLoadedObject( $product))
            $errors[] = $this->l('Product is not valid');
        if(!$errors)
        {
    ...
    public function _submitSavePecificPrice($id_product)
    {
        ...
        if(!$errors)
        {
            ...
            if($specific['price']>=0)
            {
                ...
                $specific['price_text'] = Module::getInstanceByName('ets_productmanager')->displayText('--','span',array('class'=>'text-center'));
                    ...
                    die(
                        json_encode(
                            array(
                                ...
                                'row_value' =>$download->display_filename ? Module::getInstanceByName('ets_productmanager')->displayText($download->display_filename,'a',array('href'=>$this->module->getLinkAdminController('admin_product_virtual_download_file_action',array('idProduct'=>$id_product)))):'--',
                                'success' => $this->l('Updated successfully'),
                                'id_lang' => $this->context->language->id,
    ...
    {
        require_once _PS_MODULE_DIR_ .'ets_customfields/classes/EtsCzfProduct.php';
        $czfProduct = Module::getInstanceByName('ets_customfields')->getCzfProductByProductId($id_product);
        if ($czfProduct) {
            $czf = new EtsCzfProduct($czfProduct['id_ets_czf_product']);
            if($czf->logo)
                $logo = __PS_BASE_URI__.$czf->logo.'?v='.rand(1111,9999);
        }
        $product = new Product($id_product,false,$this->context->language->id);
        $this->context->smarty->assign(
            array(
                'id_product'  => $id_product,
                'module_logo' => isset($logo) ? $logo : false,
                'product_name' => $product->name,
            )
        );
    ...
    {
        require_once _PS_MODULE_DIR_ .'ets_customfields/classes/EtsCzfProduct.php';
        $listLangs = Language::getLanguages(true);
    ...
    {
        require_once _PS_MODULE_DIR_ .'ets_extraproducttabs/classes/EtsEptProduct.php';
        require_once _PS_MODULE_DIR_ .'ets_extraproducttabs/classes/EtsEptTab.php';
        require_once _PS_MODULE_DIR_ .'ets_extraproducttabs/classes/EtsEptDefine.php';
        $activeLanguages = Language::getLanguages(true);
        $listExtraTabs = Module::getInstanceByName('ets_extraproducttabs')->getListTabs($id_product,null,null,false,null,$id_tab);
        $linkDownload = $this->context->link->getModuleLink('ets_extraproducttabs', 'download');
        $this->context->smarty->assign(array(
            'listExtraTabs' => $listExtraTabs,
            'tabTypes' => Module::getInstanceByName('ets_extraproducttabs')->getTabTypes(),
            ...
            'id_tab_extra'=>$id_tab,
            'listHooks' => Module::getInstanceByName('ets_extraproducttabs')->getListHooks(false)
        ));
    ...
    public function _submitExtraTabProduct($id_product,$id_tab_extra)
    {
        if(Module::isEnabled('ets_extraproducttabs'))
        {
            ...
            if(($tabContent = Tools::getValue('ets_ept_content')) && is_array($tabContent)){
                $errors = Module::getInstanceByName('ets_extraproducttabs')->validateDataTab($tabContent);
                ...
                Module::getInstanceByName('ets_extraproducttabs')->saveDataTab($id_product, $tabContent, $ets_ept_use_global);
            }
        }
        Hook::exec('actionProductUpdate',array('product' => new Product($id_product),'id_product'=>$id_product));
        die(
    ...
    {
        require_once _PS_MODULE_DIR_ .'ets_customfields/classes/EtsCzfProduct.php';
        $czfData = Tools::getValue('ets_czf');
        $czfFile = isset($_FILES['ets_czf_doc_file']) ? $_FILES['ets_czf_doc_file'] : array();
        $errors = array();
        $czf = Module::getInstanceByName('ets_customfields')->getCzfProductByProductId($id_product);
        if(!Ets_productmanager::validateArray($czfData))
            $errors[] = $this->l('Custom field data is not valid');
        if(!$czf)
        {
            $errors[] = $this->l('Please upload a virtual product and save product before uploading your document file');
        }
        else
        {
            $czfProduct = new EtsCzfProduct((int)$czf['id_ets_czf_product']);
            
            if($czfFile)
            {
                $names = $czfFile['name'];
                $tmp_names = $czfFile['tmp_name'];
                $sizes = $czfFile['size'];
                $maxSize = (float)Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE');
                $doc_sizes = array();
                $doc_names = array();
                foreach(Language::getLanguages(true) as $lang)
                {
                    if(isset($names[$lang['id_lang']]) && $names[$lang['id_lang']] && isset($tmp_names[$lang['id_lang']]) && $tmp_names[$lang['id_lang']])
                    {
                        $names[$lang['id_lang']] = str_replace(' ','_',$names[$lang['id_lang']]);
                        if(!Validate::isFileName($names[$lang['id_lang']]))
                            $errors[] = sprintf($this->l('The file name "%s" is invalid'),$names[$lang['id_lang']]);
                        elseif(($ext = pathinfo($names[$lang['id_lang']], PATHINFO_EXTENSION)) && $ext!='pdf' &&  $ext != 'zip' )
                        {
                            $errors[] = sprintf($this->l('The document file "%s" allowed format: *.zip, *.pdf'),$names[$lang['id_lang']]);
                        }
                        elseif($sizes[$lang['id_lang']] / 1048576 > $maxSize){
                            $errors[] = sprintf($this->l('The document file "%s" is too large. Max file size: %s'),$names[$lang['id_lang']], $maxSize.'Mb');
                        }
                        else
                        {
                            $doc_names[$lang['id_lang']] = $names[$lang['id_lang']];
                            $doc_sizes[$lang['id_lang']] = $sizes[$lang['id_lang']];
                        }
                    }
                }
            }
        ...
        if(!$errors)
        {
            ...
            foreach (Language::getLanguages(true) as $lang)
            {
                ...
                if(isset($names[$lang['id_lang']]) && $names[$lang['id_lang']] && isset($tmp_names[$lang['id_lang']]) && $tmp_names[$lang['id_lang']])
                {
                    $ext = pathinfo($names[$lang['id_lang']], PATHINFO_EXTENSION);
                    $fileName = md5($this->module->name.time()).'.'.$ext;
                    $docSize = round((float)$sizes[$lang['id_lang']] / 1048576, 2);
                    if(Module::getInstanceByName('ets_customfields')->uploadFile($tmp_names[$lang['id_lang']], $fileName)){
                        $new_files[] = $fileName;
                        if(isset($czfProduct->doc_file[$lang['id_lang']]))
                            $old_files[] = $czfProduct->doc_file[$lang['id_lang']];
                        $czfProduct->doc_file[$lang['id_lang']] =$fileName;
                        $czfProduct->doc_size[$lang['id_lang']] =$docSize;
                    }
                    else
                        $errors[] = sprintf($this->l('Cannot upload file "%s"'),$names[$lang['id_lang']]);
                }
                $czfProduct->doc_name[$lang['id_lang']] = isset($czfData['doc_name'][$lang['id_lang']]) && $czfData['doc_name'][$lang['id_lang']]
                    ? $czfData['doc_name'][$lang['id_lang']] :
                    (isset($czfData['doc_name'][$idDefaultLang]) && (!isset($czfProduct->doc_name[$lang['id_lang']]) || $czfProduct->doc_name[$lang['id_lang']] === null) ? $czfData['doc_name'][$idDefaultLang] : null);
    
                $czfProduct->doc_desc[$lang['id_lang']] = isset($czfData['doc_desc'][$lang['id_lang']]) && $czfData['doc_desc'][$lang['id_lang']]
                    ? $czfData['doc_desc'][$lang['id_lang']] :
                    (isset($czfData['doc_desc'][$idDefaultLang]) && (!isset($czfProduct->doc_desc[$lang['id_lang']]) || $czfProduct->doc_desc[$lang['id_lang']] === null) ? $czfData['doc_desc'][$idDefaultLang] : null);
                if(isset($names[$lang['id_lang']]) && $names[$lang['id_lang']])
                    $czfProduct->doc_display_name[$lang['id_lang']] = $names[$lang['id_lang']];
            }
            if(!$errors)
            {
                if($czfProduct->save())
                {
                    if($old_files)
                    {
                        ...
                        {
                            if($file && @file_exists(_PS_UPLOAD_DIR_.'ets_customfields/'.$file)){
                                @unlink(_PS_UPLOAD_DIR_.'ets_customfields/'.$file);
                            }
                        }
                    }
                    if(isset($czfProduct->doc_file[$this->context->language->id]) && $czfProduct->doc_file[$this->context->language->id])
                    {
                        $row_value = Module::getInstanceByName('ets_productmanager')->displayText(
                            Module::getInstanceByName('ets_productmanager')->displayText(
                                Module::getInstanceByName('ets_productmanager')->displayText(
                                    Module::getInstanceByName('ets_productmanager')->displayText('','path',array('d'=>'M768 384v-128h-128v128h128zm128 128v-128h-128v128h128zm-128 128v-128h-128v128h128zm128 128v-128h-128v128h128zm700-388q28 28 48 76t20 88v1152q0 40-28 68t-68 28h-1344q-40 0-68-28t-28-68v-1600q0-40 28-68t68-28h896q40 0 88 20t76 48zm-444-244v376h376q-10-29-22-41l-313-313q-12-12-41-22zm384 1528v-1024h-416q-40 0-68-28t-28-68v-416h-128v128h-128v-128h-512v1536h1280zm-627-721l107 349q8 27 8 52 0 83-72.5 137.5t-183.5 54.5-183.5-54.5-72.5-137.5q0-25 8-52 21-63 120-396v-128h128v128h79q22 0 39 13t23 34zm-141 465q53 0 90.5-19t37.5-45-37.5-45-90.5-19-90.5 19-37.5 45 37.5 45 90.5 19z')),
                                    'svg',array('viewBox'=>'0 0 1792 1792','xmlns'=>'http://www.w3.org/2000/svg')),
                                'i',array('class'=>'ets_svg_icon doc-icon doc-icon-zip')).$czfProduct->doc_display_name[$this->context->language->id],
                            'a',array('href'=>$this->context->link->getModuleLink('ets_customfields', 'documentation', array('boDownload' => 1, 'idLang' => $this->context->language->id,'idProduct'=>$id_product ))));
                ...
                {
                    $errors[] = $this->l('An error occurred while saving');
                    if($new_files)
                    {

...
class AdminProductManagerMassiveEditController extends ModuleAdminController
{
        ...
        if(Tools::isSubmit('submitSearchProductEdit'))
        {
            if(Tools::isSubmit('getPreviewProducts') && ($id_massedit = (int)Tools::getValue('id_massedit')) && ($massedit = new Ets_pmn_massedit($id_massedit)) && Validate::isLoadedObject($massedit))
            {
                ...
                die(
                    json_encode(
                        array(
                            'product_list' => $massedit->renderListProductsByConditions($conditions,$product_excluded,false,(int)Tools::getValue('page'),(int)Tools::getValue('paginator_matching_products_select_limit',10),(int)Tools::getValue('stepNumber')),
                        )
                    )
                );
            }
            else
            {
                ...
                die(
                    json_encode(
                        array(
                            'product_list' => Ets_pmn_massedit::getInstance()->renderListProductsByConditions($conditions,Validate::isCleanHtml($product_excluded) ? $product_excluded :'',true,(int)Tools::getValue('page'),(int)Tools::getValue('paginator_matching_products_select_limit',10),(int)Tools::getValue('stepNumber')),
                        )
                    )
                );
            }
    ...
    public function _submitSaveMassedit()
    {
        ...
        if(!$errors)
        {
            ...
            $product_excluded = Tools::getValue('product_excluded');
            $product_excluded = Validate::isCleanHtml($product_excluded) ? trim($product_excluded,','):'';
            $nbProducts = Ets_pmn_massedit::getInstance()->getProducts($conditions,true,0,false,Validate::isCleanHtml($product_excluded) ? $product_excluded :'');
            if(!$nbProducts)
    ...
    public function renderFormCategories()
    {
        ...
        $fields = array(
            array(
                ...
                'categories_tree'=> $this->module->displayProductCategoryTre(Ets_pmn_defines::getInstance()->getCategoriesTree(),$selected_categories,'id_category_default',$disabled_categories,0,array(),true,true,'radio'),
    ...
    public function renderFormBrandAndFeatures()
    {
        ...
        if(($product_features = $this->displayProductFeatures()))
        {
            $fields[] = array(
    ...
    public function renderFormRelatedProducts()
    {
        $fields = array(
            array(
                'type' => 'custom_form',
    ...
    public function displayProductFeatures()
    {
    ...
    public function renderFormOptions()
    {
        $fields = array(
            ...
            array(
                ...
                'name' => 'customization',
                'html_form' => $this->renderFormCustomizationProduct(),
                'condition_fields' => $this->condition_multi_fields,
                'condition_fields_selected' => 'off'
            )
        );
    ...
    public function renderFormCustomizationProduct()
    {
    ...
    public function renderFormPrice()
    {
        $fields = array(
            ...
            array(
                ...
                'html_form' => $this->renderFormSpecificPrice(),
    ...
    public function renderFormSpecificPrice($id_specific_price=0)
    {
    ...
    {
        $attributeGroups = Ets_pmn_massedit::getAttributeGroups();
        $this->context->smarty->assign(
            array(
    ...
    public function _submitSaveMasseEditProduct()
    {
        ...
        if(($id_ets_pmn_massedit = (int)Tools::getValue('id_ets_pmn_massedit')) && ($massedit = new Ets_pmn_massedit($id_ets_pmn_massedit)) && Validate::isLoadedObject($massedit))
        {
            ...
            if($condition_actions && Validate::isLoadedObject($masseditHistory))
            {
                $limit = (int)Configuration::get('ETS_PMN_NUMBER_PRODUCT_EDIT_EACH_AJAX') ? :false;
                $conditions = $massedit->getConditions();
                $start = ($current_page-1)*$limit;
                $products = $masseditHistory->getProducts($conditions,false,$start,$limit,$massedit->excluded);

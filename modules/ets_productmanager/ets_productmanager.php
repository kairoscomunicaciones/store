...
class Ets_productmanager extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        if (version_compare(_PS_VERSION_, '1.7.3.4', '>'))
            $this->is1725 = false;
        else
            $this->is1725 = true;
    ...
    public function getTwigs()
    {
        ...
        return array(
            ...
            'ets_pmn_view_text'=>$this->l('View'),
            'ets_pmn_close_text'=>$this->l('Close'),
            'ets_pmn_cancel_text'=>$this->l('Cancel'),
            'ets_pmn_save_text'=>$this->l('Save'),
            ...
            'low_stock_alert_text'=>$this->l('Send me an email when the quantity is below or equals this level'),
            'Available_for_order_text'=>$this->l('Available for order'),
            'Show_price_text'=>$this->l('Show price'),
            'viewmore'=>$this->l('View more'),
            'viewless'=>$this->l('View less'),
            'Web_only_text'=>$this->l('Web only (not sold in your retail store)'),
            'Display_condition_on_product_page_text'=>$this->l('Display condition on product page'),
            ...
            'ets_pmn_languages'=>$languages,
            'ets_pmn_count_languages'=>Count($languages),
            'ets_pmn_id_lang'=>$this->context->language->id,
            'ets_pmn_iso_lang'=>$this->context->language->iso_code,
    ...
    {
        if(($request = $this->getRequestContainer()) && $request->get('_route')=='admin_product_catalog')
        {
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        if($controller == 'AdminProductManagerMassiveEdit' || $controller=='AdminProductManagerMassiveEditLog' || $controller=='AdminProductManagerMassiveEditTemplates' || ($controller=='AdminProducts' && !$id_product))
        {
            $this->addJquery();
        }  
        if($controller=='AdminProducts' && !$id_product && $this->active)
        {
            if(!$id_product)
            {
                Media::addJsDef(
        ...
        if($controller=='AdminProductManagerMassiveEdit' || $controller=='AdminProductManagerMassiveEditLog' || $controller=='AdminProductManagerMassiveEditTemplates')
        {
    ...
    public function renderFormCombinations($id_product)
    {
        ...
        {
            $attributeGroups = Ets_pmn_defines::getInstance()->getAttributeGroups();
            $this->smarty->assign(
                array(
    ...
    public function getConfigInputs()
    {
        return array(
            ...
            array(
                ...
                'desc' => $this->displayText($this->l('View Mass Edit log here'),'a',array('href'=> $this->context->link->getAdminLink('AdminProductManagerMassiveEditLog'))),
    ...
    public function getFormEditInlineProduct($id_product)
    {
        ...
        if($fields)
        {
            if(Module::isEnabled('ets_customfields'))
            {
                ...
                $czfProduct  = Module::getInstanceByName('ets_customfields')->getCzfProductByProductId($product_class->id,$this->context->language->id);
            ...
            foreach(array_keys($fields) as $key)
            {
                ...
                elseif($key=='tax_name')
                    $valueFieldPost['tax_name'] = $product_class->id_tax_rules_group;
                if(Module::isEnabled('ets_customfields') && isset($czfProduct_class->{$key}))
                    $valueFieldPost[$key] = $czfProduct_class->{$key};
                elseif($key=='module_name')
                    $valueFieldPost[$key] = $czfProduct_class->display_name;
                elseif($key=='module_description')
                    $valueFieldPost[$key] = $czfProduct_class->description;
            ...
            {
                if(isset($field['input']) && $field['input'] && isset($field['input']['lang']) && $field['input']['lang'])
                {
    ...
    public function hookActionAdminProductsListingFieldsModifier($params)
    {
        ...
        $sql_select = array(
            ...
            'location' => array('table'=>'p','field'=>'location', 'filtering' => ' %s '),
        ...
        if($this->is1725)
        {
            unset($sql_select['low_stock_threshold']);
            unset($sql_select['low_stock_alert']);
            unset($sql_select['additional_delivery_times']);
            unset($sql_select['delivery_in_stock']);
            unset($sql_select['delivery_out_stock']);
        }
        if($sql_table)
    ...
    public function hookActionAdminProductsListingResultsModifier($params)
    {
        ...
        if($products && !(isset($products[0]['total']) && !$products[0]['total']))
        {
            $list_fields = Ets_pmn_defines::getInstance()->getFieldsByIdEmployee();
            foreach($products as &$product)
            {
                ...
                {
                    $unit_price = $product['unit_price_ratio']!=0 ? $product['price_float']/$product['unit_price_ratio'] :0;
                    $product['unit_price_float'] = $unit_price;
                    if($unit_price)
                        $product['unit_price'] = Tools::displayPrice($unit_price);
                    else
                        $product['unit_price'] = $this->displayText('--','span',array('class'=>'text-center'));
                }
                ...
                if(in_array('weight',$list_fields))
                {
                    if($product['weight']!=0)
                    {
                        ...
                        $product['weight'] = Tools::ps_round($product['weight']).'(kg)';
                    }
                    else
                    {
                ...
                if(in_array('tags',$list_fields))
                {
                    ...
                    $product['tags_no_html'] = Ets_pmn_defines::getInstance()->getListtags($product['id_product'],false);
                }
                ...
                if(isset($product['module_logo']) && $product['module_logo'])
                    $product['module_logo'] = $this->displayText('','img',array('href'=>$this->getBaseLink().'/'.$product['module_logo']));
                ...
                if(isset($product['fo_link']))
                {
                    ...
                    if($product['fo_link'])
                        $product['fo_link'] = $this->displayText($product['fo_link'],'a',array('href'=>$product['fo_link'],'target'=>'_blank'));
                ...
                if(isset($product['bo_link']))
                {
                    ...
                    if($product['bo_link'])
                        $product['bo_link'] = $this->displayText($product['bo_link'],'a',array('href'=>$product['bo_link'],'target'=>'_blank'));
                ...
                if(isset($product['doc_name']) && isset($product['doc_file']))
                {
                    ...
                    if($product['doc_file'])
                        $product['doc_name'] = $this->displayText(
                            $this->displayText(
                                $this->displayText(
                                    $this->displayText(
                ...
                if($fields = Ets_pmn_defines::getInstance()->getProductFields())
                {
                    foreach($fields as $key=> $field)
                    {
                        if(in_array($key,$list_fields) && $key!='tax_name' && isset($field['input']['type'])  && $field['input']['type']=='select' && isset($field['input']['values']['query']) && ($values = $field['input']['values']) && ($queries = $values['query']))
                        {
                            if(isset($product[$key]) && !isset($product[$field['input']['name']]))
                                $product[$field['input']['name']] = $product[$key];
                            foreach($queries as $query)
                            {
                                if($query[$values['id']]==$product[$key])
                                {
                                    $product[$key]=$query[$values['name']];
                                    break;
                                }
                            }
                        }elseif(in_array($key,$list_fields) && $key=='tax_name' && !$product[$key])
                            $product[$key]= $this->l('No tax');
                    }
                }
            }     
        }
    ...
    public function displayListCarriers($id_product)
    {
        if($carriers = Ets_pmn_defines::getProductCarriers($id_product))
        {
            ...
            foreach($carriers as $carrier)
                $text .= $this->displayText($carrier['name'] ? : $this->context->shop->name,'p');
            return $text;
        }
        return '--';
    }
    ...
    public function renderListLogs($history=false)
    {
        ...
        $fields_list = array(
            ...
            'lang_name' => array(
                ...
                'filter_list' => array(
                    'id_option' => 'id_lang',
                    'value' => 'name',
                    'list' => array_merge(array(array('id_lang'=>0,'name'=>$this->l('No language'))),Language::getLanguages()),
                )
            ),
            'old_value' => array(
        ...
        if($logmassedits)
        {
            ...
            {
                $logmassedit['product_name'] = $this->displayText($logmassedit['product_name'],'a',array('href' => Context::getContext()->link->getAdminLink('AdminProducts',true,array('id_product'=>$logmassedit['id_product']))));

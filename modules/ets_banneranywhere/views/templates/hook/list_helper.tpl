...
var text_update_position='{l s='Successfully updated' mod='ets_banneranywhere'}';
</script>
            ...
            {if isset($show_add_new) && $show_add_new}            
                <a class="btn btn-default add_new_link" href="{if isset($link_new)}{$link_new|escape:'html':'UTF-8'}{else}{$currentIndex|escape:'html':'UTF-8'}{/if}">  
        		     <i class="ets_icon">
                        ...
                        </i> {if isset($add_new_text)}{$add_new_text|escape:'html':'UTF-8'}{else} {l s='Add new' mod='ets_banneranywhere'}{/if}
                </a>            
            {/if}
        </span>
    </div>
                ...
                <table class="table configuration list-{$name|escape:'html':'UTF-8'}">
                    <thead>
                        <tr class="nodrag nodrop">
                            ...
                            {foreach from=$fields_list item='field' key='index'}
                                <th class="{$index|escape:'html':'UTF-8'}{if isset($field.class)} {$field.class|escape:'html':'UTF-8'}{/if}" {if $show_action && !$actions && count($fields_list)==$i}colspan="2"{/if}>
                                    <span class="title_box">
                                        ...
                                        {if isset($field.toltip)}
                                        <span class="ets_bn_tooltip">
                                        ?<span class="toltip">{$field.toltip|escape:'html':'UTF-8'}</span>
                                        </span>
                            ...
                            {if $show_action && $actions}
                                <th class="table_action" style="text-align: right;">{l s='Action' mod='ets_banneranywhere'}</th>
                            {/if}
                        </tr>
                        {if $show_toolbar}
                                                ...
                                                <select  {if isset($field.width)}style="width: {$field.width|intval}px;"{/if}  name="{$index|escape:'html':'UTF-8'}">
                                                    ...
                                                    {if isset($field.filter_list.list) && $field.filter_list.list}
                                                        ...
                                                        {foreach from=$field.filter_list.list item='option'}
                                                            <option {if ($field.active!=='' && $field.active==$option.$id_option) || ($field.active=='' && $index=='has_post' && $option.$id_option==1 )} selected="selected"{/if} value="{$option.$id_option|escape:'html':'UTF-8'}">{$option.$value|replace:'[highlight]':''|replace:'[end_highlight]':'' nofilter}</option>
                                                        {/foreach}
                                                    {/if}
                                                </select>                                            
                                            {/if}
                                            {if $field.type=='int'}
                                                <label for="{$index|escape:'html':'UTF-8'}_min"><input type="text" placeholder="{l s='Min' mod='ets_banneranywhere'}" name="{$index|escape:'html':'UTF-8'}_min" value="{$field.active.min|escape:'html':'UTF-8'}" /></label>
                                                <label for="{$index|escape:'html':'UTF-8'}_max"><input type="text" placeholder="{l s='Max' mod='ets_banneranywhere'}" name="{$index|escape:'html':'UTF-8'}_max" value="{$field.active.max|escape:'html':'UTF-8'}" /></label>
                                            {/if}
                                            {if $field.type=='date'}
                                                <fieldset class="form-group"> 
                                                    <div class="input-group ets_baw_datepicker">
                                                        <input id="{$index|escape:'html':'UTF-8'}_min" autocomplete="off" class="form-control" name="{$index|escape:'html':'UTF-8'}_min" placeholder="{l s='From' mod='ets_banneranywhere'}" value="{$field.active.min|escape:'html':'UTF-8'}" type="text" autocomplete="off" />
                                                ...
                                                <fieldset class="form-group"> 
                                                    <div class="input-group ets_baw_datepicker">
                                                        <input id="{$index|escape:'html':'UTF-8'}_max" autocomplete="off" class="form-control" name="{$index|escape:'html':'UTF-8'}_max" placeholder="{l s='To' mod='ets_banneranywhere'}" value="{$field.active.max|escape:'html':'UTF-8'}" type="text" autocomplete="off" />
                                                        <div class="input-group-append">
                                                            <div class="input-group-text">
                                                                <i class="icon icon-date"></i>
                                                            </div>
                                ...
                                {if $show_action}
                                    <th class="actions">
                                        <span class="pull-right flex">
                                            ...
                                            {if $show_reset}<a  class="btn btn-warning"  href="{$currentIndex|escape:'html':'UTF-8'}"><i class="icon-eraser"></i> {l s='Reset' mod='ets_banneranywhere'}</a> &nbsp;{/if}
                                            <button class="btn btn-default" name="ets_baw_submit_{$name|escape:'html':'UTF-8'}" id="ets_baw_submit_{$name|escape:'html':'UTF-8'}" type="submit">
            									<i class="icon-search"></i> {l s='Filter' mod='ets_banneranywhere'}
            								</button>
                                        </span>
                                    </th>
                                {/if}
                    ...
                    <tbody id="list-{$name|escape:'html':'UTF-8'}">
                        ...
                        {foreach from=$field_values item='row'}
                            <tr id="list-{$name|escape:'html':'UTF-8'}-{$row.$identifier|intval}" {if isset($row.read) && !$row.read}class="no-read"{/if} data-id="{$row.$identifier|intval}">
                                {assign var='i' value=1}
                                        ...
                                        {elseif $field.type != 'active'}
                                            ...
                                            {else}
                                                ...
                                                {if isset($row.$key) && is_array($row.$key) && isset($row.$key.image_field) && $row.$key.image_field}
                                                    <a class="ets_baw_fancy" href="{$row.$key.img_url|escape:'html':'UTF-8'}"><img style="{if isset($row.$key.height) && $row.$key.height}max-height: {$row.$key.height|intval}px;{/if}{if isset($row.$key.width) && $row.$key.width}max-width: {$row.$key.width|intval}px;{/if}" src="{$row.$key.img_url|escape:'html':'UTF-8'}" /></a>
                                                {/if} 
                                                {if isset($field.update_position) && $field.update_position}
                                                    </div>
                                                    </span>
                                                ...
                                                <div class="btn-group pull-right">
                                                        {if $actions[0]=='view'}
                                                            {if isset($row.child_view_url) && $row.child_view_url}
                                                                <a class="btn btn-default link_view" href="{$row.child_view_url|escape:'html':'UTF-8'}" {if isset($view_new_tab) && $view_new_tab} target="_blank" {/if}><i class="icon-search-plus fa fa-search-plus"></i> {l s='View' mod='ets_banneranywhere'}</a>
                                                            {elseif !isset($row.action_edit) || $row.action_edit}
                                                                <a class="btn btn-default link_edit" href="{$currentIndex|escape:'html':'UTF-8'}&edit{$name|escape:'html':'UTF-8'}=1&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}" ><i class="icon icon-pencil fa fa-pencil"></i> {l s='Edit' mod='ets_banneranywhere'}</a>
                                                            {/if}
                                                        {/if}
                                                        {if $actions[0]=='delete'}
                                                            <a class="btn btn-default" onclick="return confirm('{l s='Do you want to delete this item?' mod='ets_banneranywhere' js=1}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes"><i class="icon icon-trash fa fa-trash"></i> {l s='Delete' mod='ets_banneranywhere'}</a>
                                                        {/if}
                                                                ...
                                                                {foreach from=$actions item='action' key='key'}
                                                                    {if $key!=0}
                                                                        {if $action=='delete' && (!isset($row.view_order_url) || (isset($row.view_order_url) && !$row.view_order_url) )}
                                                                            <li><a class="btn btn-default" onclick="return confirm('{l s='Do you want to delete this item?' mod='ets_banneranywhere' js=1}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes{if isset($row.type)}&type={$row.type|escape:'html':'UTF-8'}{/if}"><i class="fa fa-trash icon icon-trash"></i> {l s='Delete' mod='ets_banneranywhere'}</a></li>
                                                                        {/if}
                                                                        
                                                                        {if $action=='delete_all'}
                                                                            <li><a class="btn btn-default" onclick="return confirm('{l s='Do you want to delete this shop and all of its data?' mod='ets_banneranywhere' js=1}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&delall=yes{if isset($row.type)}&type={$row.type|escape:'html':'UTF-8'}{/if}"><i class="fa fa-trash icon icon-trash"></i> {l s='Delete all' mod='ets_banneranywhere'}</a></li>
                                                                        {/if}
                                                                        {if $action=='view'}
                                                                            {if isset($row.child_view_url) && $row.child_view_url}
                                                                                <li><a class="btn btn-default" href="{$row.child_view_url|escape:'html':'UTF-8'}"><i class="fa fa-search-plus icon icon-search-plus"></i> {l s='View' mod='ets_banneranywhere'}</a></li>
                                                                            {else}
                                                                                <li><a class="btn btn-default" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="fa fa-pencil icon icon-pencil"></i> {l s='Edit' mod='ets_banneranywhere'}</a></li>
                                                                            {/if}
                                                                        {/if}
                                                                        {if $action =='edit'}
                                                                            <li><a class="btn btn-default" href="{$currentIndex|escape:'html':'UTF-8'}&edit{$name|escape:'html':'UTF-8'}=1&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="fa fa-pencil icon icon-pencil"></i> {l s='Edit' mod='ets_banneranywhere'}</a></li>
                                                                        {/if}
                                                                    {/if}
                                                                {/foreach}
                                                            </ul>
                           ...
                           <tr class="no-record not_items_found list-empty">
                               <td colspan="100%">
                                   <p class="list-empty-msg">
                                       ...
                                       {l s='No items found' mod='ets_banneranywhere'}</p>
                               </td>
                           </tr>
                        {/if}                
                    </tbody>
                                ...
                                <div class="d-inline-block hide">
                                    <div class="btn-group dropdown bulk-catalog">
                                        <button id="product_bulk_menu" class="btn btn-outline-secondary dropdown-toggle" data-toggle="dropdown" aria-expanded="true" style="color:black;">
                                            {l s='Bulk actions' mod='ets_banneranywhere'}
                                            <i class="icon-caret-up"></i>
                                        </button>
                                        <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 35px, 0px); top: 0px; left: 0px; will-change: transform;">
                                            <div class="dropdown-divider"></div>
                                            <button class="dropdown-item" name="submitBulkEnabled" type="submit" style="border:none;background:none" onclick="return confirm('{l s='Do you want to enable selected items?' mod='ets_banneranywhere' js=1}');">
                                                <i class="icon-check" aria-hidden="true"></i>
                                                {l s='Enable selection' mod='ets_banneranywhere'}
                                            </button>
                                            <button class="dropdown-item" name="submitBulkDiasabled" type="submit" style="border:none;background:none" onclick="return confirm('{l s='Do you want to disable selected items?' mod='ets_banneranywhere' js=1}');">
                                                <i class="icon-remove" aria-hidden="true"></i>
                                                {l s='Disable selection' mod='ets_banneranywhere'}
                                            </button>
                                            <button class="dropdown-item" name="submitBulkDelete" type="submit" style="border:none;background:none" onclick="return confirm('{l s='Do you want to delete selected items?' mod='ets_banneranywhere' js=1}');">
                                                <i class="icon-trash-o" aria-hidden="true"></i>
                                                {l s='Delete selected' mod='ets_banneranywhere'}
                                            </button>
                                        </div>
                                    </div>
                                </div>

...
<script type="text/javascript">
    ...
    var idorder = {$idorder|intval};
    var loadaddnewcustomer = 0;
	var id_customer = 0;
    ...
    var admin_customer_link =  decodeURIComponent("{$admin_customer_link|escape:'htmlall':'UTF-8'}");
    var admin_cartrules_link = decodeURIComponent("{$admin_cartrules_link|escape:'htmlall':'UTF-8'}");
    var admin_cart_link = decodeURIComponent("{$admin_cart_link|escape:'htmlall':'UTF-8'}");
	...
	var id_currency = '';
	var id_lang = '';
	//var txt_show_carts = '{l s='Show carts and orders for this customer.' js=1 mod='g_ordermanager'}';
	//var txt_hide_carts = '{l s='Hide carts and orders for this customer.' js=1 mod='g_ordermanager'}';
	var defaults_order_state = new Array();
	var customization_errors = false;
	var pic_dir = '{$pic_dir|escape:'html':'UTF-8'}';
	...
	{foreach from=$defaults_order_state key='module' item='id_order_state'}
		defaults_order_state['{$module|escape:'html':'UTF-8'}'] = '{$id_order_state|escape:'html':'UTF-8'}';
	{/foreach}
	$(document).ready(function() {
        ...
        $('#customerorder').typeWatch({
			captureLength: 3,
			highlight: true,
			wait: 100,
			callback: function(){ searchCustomers2(); }
			});
		$('#product').typeWatch({
			captureLength: 1,
			highlight: true,
		...
		$.ajaxSetup({ type:"post" });
		$("#voucher").autocomplete(admin_cartrules_link, {
					minChars: 3,
					max: 15,
					width: 250,
					selectFirst: false,
					...
					parse: function(data) {
						if (!data.found)
							$('#vouchers_err').html("{l s='No voucher was found' mod='g_ordermanager'}").show();
						else
							$('#vouchers_err').hide();
						var mytab = new Array();
		...
		$('#customer_part').on('click','button.setup-customer',function(e){
			...
			$(this).removeClass('setup-customer').addClass('change-customer').html("<i class='icon-refresh'></i>&nbsp;{l s='Change' mod='g_ordermanager'}").blur();
        ...
        $('#customer_part').on('click','button.setup-customer2',function(e){
			e.preventDefault();
			setupCustomer2($(this).data('customer'));
			$(this).removeClass('setup-customer2').addClass('change-customer').html("<i class='icon-refresh'></i>&nbsp;{l s='Change' mod='g_ordermanager'}").blur();
			$(this).closest('.customerCard').addClass('selected-customer');
			$('.selected-customer .panel-heading').prepend('<i class="icon-ok text-success"></i>');
			$('.customerCard').not('.selected-customer').remove();
			$('#search-customer-form-group').hide();
	...
	function resetBind()
	{
        ...
        $('.fancybox_customer2').fancybox({
			'type': 'iframe',
			'width': '90%',
			'height': '90%',
            'beforeShow': function(){
                loadaddnewcustomer = 1;
            },
			'afterClose' : function () {
				searchCustomers2();
                loadaddnewcustomer = 0;
			}
		});
			...
			success : function(res)
			{
				if(res.found)
				{
					...
					$.each(res.customers, function() {
						...
						html += '<a href="{$link->getAdminLink('AdminCustomers')|escape:'html':'UTF-8'}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1" class="btn btn-default fancybox"><i class="icon-search"></i> {l s='Details' mod='g_ordermanager'}</a>';
						html += '<button type="button" data-customer="'+this.id_customer+'" class="setup-customer btn btn-default pull-right"><i class="icon-arrow-right"></i> {l s='Choose' mod='g_ordermanager'}</button>';
						html += '</div>';
						html += '</div>';
						html += '</div>';
					});
					...
					html = '<div class="alert alert-warning">{l s='No customers found' mod='g_ordermanager'}</div>';
				$('#customers').html(html);
				resetBind();
			}
		});
   	...
   	function searchCustomers2()
	{
		...
		$.ajax({
			...
			data : {
				...
				tab: "AdminCustomers",
				action: "searchCustomers",
				customer_search: $('#customerorder').val()},
			success : function(res)
			{
				if(res.found)
				{
					...
					$.each(res.customers, function() {
						...
						html += '<div class="panel-footer">';
						html += '<a href="{$link->getAdminLink('AdminCustomers')|escape:'html':'UTF-8'}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1" class="btn btn-default fancybox"><i class="icon-search"></i> {l s='Details' mod='g_ordermanager'}</a>';
						html += '<button type="button" data-customer="'+this.id_customer+'" class="setup-customer2 btn btn-default pull-right"><i class="icon-arrow-right"></i> {l s='Choose' mod='g_ordermanager'}</button>';
						html += '</div>';
						html += '</div>';
						html += '</div>';
					});
	...
	{
        if(loadaddnewcustomer) return setupCustomer2(idCustomer);
		$('#carts').show();
		$('#products_part').show();
		$('#vouchers_part').show();
		$('#address_part').show();
			...
			success : function(res)
			{
				...
				if(res.found)
				{
					...
					$.each(res.carts, function() {
						...
						html_carts += '<a title="{l s='View this cart' mod='g_ordermanager'}" class="fancybox btn btn-default" href="index.php?tab=AdminCarts&id_cart='+this.id_cart+'&viewcart&token={getAdminToken tab='AdminCarts'}&liteDisplaying=1#"><i class="icon-search"></i>&nbsp;{l s='Details' mod='g_ordermanager'}</a>';
						html_carts += '&nbsp;<a href="#" title="{l s='Use this cart' mod='g_ordermanager'}" class="use_cart btn btn-default" rel="'+this.id_cart+'"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='g_ordermanager'}</a>';
						html_carts += '</td>';
						html_carts += '</tr>';
					});
					...
					$.each(res.orders, function() {
						...
						html_orders += '<a href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&id_order='+this.id_order+'&vieworder&liteDisplaying=1#" title="{l s='View this order' mod='g_ordermanager'}" class="fancybox btn btn-default"><i class="icon-search"></i>&nbsp;{l s='Details' mod='g_ordermanager'}</a>';
						html_orders += '&nbsp;<a href="#" "title="{l s='Duplicate this order' mod='g_ordermanager'}" class="duplicate_order btn btn-default" rel="'+this.id_order+'"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='g_ordermanager'}</a>';
						html_orders += '</td>';
						html_orders += '</tr>';
					});
	...
	function setupCustomer2(idCustomer)
	{
		//$('#carts').show();
		$('#products_part').show();
		$('#vouchers_part').show();
		...
		var address_link = $('#new_address').attr('href');
		id_customer = idCustomer;
        console.debug(id_cart+':'+id_customer);
		$('#new_address').attr('href', address_link.replace(/id_customer=[0-9]+/, 'id_customer='+id_customer));
		$.ajax({
			type:"POST",
			url : admin_ordermanager_tab_link,/* admin_order_tab_link, */
			async: false,
			dataType: "json",
			data : {
				ajax: "1",
				token: "{getAdminToken tab='AdminOrdermanager'}",
				tab: "AdminOrdermanager",
				'setupCustomer2': "setupCustomer2",
				id_customer: id_customer,
				id_cart: id_cart
			},
			success : function(res)
	...
	function updateDeliveryOptionList(delivery_option_list)
	{
		...
		{
			$('#carrier_form').hide();
			$('#carriers_err').show().html('{l s='No carrier can be applied to this order' mod='g_ordermanager'}');
           ...
           /* url: admin_order_tab_link,
			...
			success : function(res)
			{
				...
				if(res.found)
				{
					...
					$('#products_found').show();
					products_found += '<label class="control-label col-lg-3">{l s='Product' mod='g_ordermanager'}</label><div class="col-lg-6"><select id="id_product" onclick="display_product_attributes();display_product_customizations();"></div>';
					attributes_html += '<label class="control-label col-lg-3">{l s='Combination' mod='g_ordermanager'}</label><div class="col-lg-6">';
					$.each(res.products, function() {
						...
						{
							customization_html += '<div class="bootstrap"><div class="panel"><div class="panel-heading">{l s='Customization' mod='g_ordermanager'}</div><form id="customization_'+id_product+'" class="id_customization" method="post" enctype="multipart/form-data" action="'+admin_cart_link+'" style="display:none;">';
				...
				{
					$('#products_found').hide();
					$('#products_err').html('{l s='No products found' mod='g_ordermanager'}');
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(gifts, function() {
			...
			cart_content += '<td>{l s='Gift' mod='g_ordermanager'}</td><td>'+this.cart_quantity+'</td><td>{l s='Gift' mod='g_ordermanager'}</td></tr>';
		});
		...
		if (typeof(vouchers) == 'object')
			$.each(vouchers, function(){
				if (parseFloat(this.value_real) === 0 && parseInt(this.free_shipping) === 1)
					var value = '{l s='Free shipping' mod='g_ordermanager'}';
				else
					var value = this.value_real;

				vouchers_html += '<tr><td>'+this.name+'</td><td>'+this.description+'</td><td>'+value+'</td><td class="text-right"><a href="#" class="btn btn-default delete_discount" rel="'+this.id_discount+'"><i class="icon-remove text-danger"></i>&nbsp;{l s='Delete' mod='g_ordermanager'}</a></td></tr>';
			});
	...
	function updateAddressesList(addresses, id_address_delivery, id_address_invoice)
	{
        ...
        var _id_address_delivery = '';
        var _id_address_invoice = '';
		$.each(addresses, function() {
			if (this.id_address == id_address_invoice)
			{
			    _id_address_invoice = id_address_invoice;
				address_invoice_detail = this.formated_address;
				invoice_address_edit_link = admin_address_link+"&id_address="+this.id_address+"&updateaddress&realedit=1&liteDisplaying=1&submitFormAjax=1#";
			}

			if(this.id_address == id_address_delivery)
			{
			    _id_address_delivery = id_address_delivery;
				address_delivery_detail = this.formated_address;
				delivery_address_edit_link = admin_address_link+"&id_address="+this.id_address+"&updateaddress&realedit=1&liteDisplaying=1&submitFormAjax=1#";
			}
        ...
        if(_id_address_invoice == '' || _id_address_delivery == ''){
            updateAddresses();
        }

		if (addresses.length == 0)
		{
			$('#addresses_err').show().html('{l s='You must add at least one address to process the order.' mod='g_ordermanager'}');
	...
	<div class="panel form-horizontal" id="customer_part">
		...
		<div id="search-customer-form-group" class="form-group">
			<label class="control-label col-lg-3">
				<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Search for an existing customer by typing the first letters of his/her name.' mod='g_ordermanager'}">
					{l s='Search for a customer' mod='g_ordermanager'}
				</span>
			</label>
			<div class="col-lg-9">
				<div class="row">
					<div class="col-lg-6">
                        {if $idorder > 0}
                            <div class="input-group">
    							<input type="text" id="customerorder" value="" />
					...
					<div class="col-lg-6">
						<span class="form-control-static">{l s='Or' mod='g_ordermanager'}&nbsp;</span>
						<a class="{if $idorder > 0} fancybox_customer2 {else} fancybox_customer {/if} btn btn-default" href="{$link->getAdminLink('AdminCustomers')|escape:'html':'UTF-8'}&amp;addcustomer&amp;liteDisplaying=1&amp;submitFormAjax=1#">
							<i class="icon-plus-sign-alt"></i>
							{l s='Add new customer' mod='g_ordermanager'}
						</a>
					</div>
				</div>
			</div>
			...
			<ul id="old_carts_orders_navtab" class="nav nav-tabs">
				<li class="active">
					<a href="#nonOrderedCarts" data-toggle="tab">
						...
						{l s='Carts' mod='g_ordermanager'}
					</a>
				</li>
				<li>
					<a href="#lastOrders" data-toggle="tab">
						<i class="icon-credit-card"></i>
						{l s='Orders' mod='g_ordermanager'}
					</a>
				</li>
			</ul>
			<div id="old_carts_orders" class="tab-content panel collapse in">
				<div id="nonOrderedCarts" class="tab-pane active">
					<table class="table">
						<thead>
							<tr>
								<th><span class="title_box">{l s='ID' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Date' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Total' mod='g_ordermanager'}</span></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
					...
					<table class="table">
						<thead>
							<tr>
								<th><span class="title_box">{l s='ID' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Date' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Products' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Total paid' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Payment' mod='g_ordermanager'}</span></th>
								<th><span class="title_box">{l s='Status' mod='g_ordermanager'}</span></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
...
<form class="form-horizontal" action="{$link->getAdminLink('AdminOrdermanager',true)|escape:'html':'UTF-8'}" method="post" autocomplete="off">
	<div class="panel" id="products_part" style="display:none;">
		<div class="panel-heading">
			<i class="icon-shopping-cart"></i>
			{l s='Cart' mod='g_ordermanager'}
		</div>
		<div class="form-group">
			<label class="control-label col-lg-3">
				<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Search for an existing product by typing the first letters of its name.' mod='g_ordermanager'}">
					{l s='Search for a product' mod='g_ordermanager'}
				</span>
			</label>
			<div class="col-lg-9">
				<input type="hidden" value="" id="id_cart" name="cart_summary[cart_id]" />
				<div class="input-group">
		...
		<div id="products_found">
			...
			<div class="form-group">
				<label class="control-label col-lg-3" for="qty">{l s='Quantity' mod='g_ordermanager'}</label>
				<div class="col-lg-9">
					<input type="text" name="qty" id="qty" class="form-control fixed-width-sm" value="1" />
					<p class="help-block">{l s='In stock' mod='g_ordermanager'} <span id="qty_in_stock"></span></p>
				</div>
			</div>
			...
			<div class="form-group">
				<div class="col-lg-9 col-lg-offset-3">
					...
					{l s='Add to cart' mod='g_ordermanager'}
				</div>
			</div>
		</div>
				...
				<table class="table" id="customer_cart">
					<thead>
						<tr>
							...
							<th><span class="title_box">{l s='Description' mod='g_ordermanager'}</span></th>
							<th><span class="title_box">{l s='Reference' mod='g_ordermanager'}</span></th>
							<th><span class="title_box">{l s='Unit price' mod='g_ordermanager'}</span></th>
							<th><span class="title_box">{l s='Quantity' mod='g_ordermanager'}</span></th>
							<th><span class="title_box">{l s='Price' mod='g_ordermanager'}</span></th>
						</tr>
					</thead>
					<tbody>
					</tbody>
		...
		<div class="form-group">
			<div class="col-lg-9 col-lg-offset-3">
				<div class="alert alert-warning">{l s='The prices are without taxes.' mod='g_ordermanager'}</div>
			</div>
		</div>

		<div class="form-group">
			<label class="control-label col-lg-3" for="id_currency">
				{l s='Currency' mod='g_ordermanager'}
			</label>
			<script type="text/javascript">
				{foreach from=$currencies item='currency'}
					currencies['{$currency.id_currency|escape:'html':'UTF-8'}'] = '{$currency.sign|escape:'html':'UTF-8'}';
				{/foreach}
			</script>
			<div class="col-lg-9">
				<select id="id_currency" name="id_currency">
					{foreach from=$currencies item='currency'}
						<option rel="{$currency.iso_code|escape:'html':'UTF-8'}" value="{$currency.id_currency|escape:'html':'UTF-8'}">{$currency.name|escape:'html':'UTF-8'}</option>
					{/foreach}
				</select>
			</div>
		</div>
		<div class="form-group">
			<label class="control-label col-lg-3" for="id_lang">
				{l s='Language' mod='g_ordermanager'}
			</label>
			<div class="col-lg-9">
				<select id="id_lang" name="id_lang">
					{foreach from=$langs item='lang'}
						<option value="{$lang.id_lang|escape:'html':'UTF-8'}">{$lang.name|escape:'html':'UTF-8'}</option>
					{/foreach}
				</select>
			</div>
		</div>
	...
	<div class="panel" id="vouchers_part" style="display:none;">
		<div class="panel-heading">
			...
			{l s='Vouchers' mod='g_ordermanager'}
		</div>
		<div class="form-group">
			<label class="control-label col-lg-3">
				{l s='Search for a voucher' mod='g_ordermanager'}
			</label>
			<div class="col-lg-9">
				<div class="row">
					<div class="col-lg-6">
					...
					<div class="col-lg-6">
						...
						<a class="fancybox btn btn-default" href="{$link->getAdminLink('AdminCartRules')|escape:'html':'UTF-8'}&amp;addcart_rule&amp;liteDisplaying=1&amp;submitFormAjax=1#">
							<i class="icon-plus-sign-alt"></i>
							{l s='Add new voucher' mod='g_ordermanager'}
						</a>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<table class="table" id="voucher_list">
				<thead>
					<tr>
						<th><span class="title_box">{l s='Name' mod='g_ordermanager'}</span></th>
						<th><span class="title_box">{l s='Description' mod='g_ordermanager'}</span></th>
						<th><span class="title_box">{l s='Value' mod='g_ordermanager'}</span></th>
						<th></th>
					</tr>
				</thead>
				<tbody>
	...
	<div class="panel" id="address_part" style="display:none;">
		<div class="panel-heading">
			...
			{l s='Addresses' mod='g_ordermanager'}
		</div>
		...
		<div class="row">
			<div id="address_delivery" class="col-lg-6">
				<h4>
					...
					{l s='Delivery' mod='g_ordermanager'}
				</h4>
				<div class="row-margin-bottom">
					<select id="id_address_delivery" name="id_address_delivery"></select>
				</div>
				<div class="well">
					<a href="" id="edit_delivery_address" class="btn btn-default pull-right fancybox"><i class="icon-pencil"></i> {l s='Edit' mod='g_ordermanager'}</a>
			...
			<div id="address_invoice" class="col-lg-6">
				...
				<div class="well">
					<a href="" id="edit_invoice_address" class="btn btn-default pull-right fancybox"><i class="icon-pencil"></i> {l s='Edit' mod='g_ordermanager'}</a>
		...
		<div class="row">
			<div class="col-lg-12">
				<a class="fancybox btn btn-default" id="new_address" href="{$link->getAdminLink('AdminAddresses')|escape:'html':'UTF-8'}&amp;addaddress&amp;id_customer=42&amp;liteDisplaying=1&amp;submitFormAjax=1#">
					<i class="icon-plus-sign-alt"></i>
					{l s='Add a new address' mod='g_ordermanager'}
				</a>
			</div>
		</div>
	</div>
	<div class="panel" id="carriers_part" style="display:none;">
		<div class="panel-heading">
			<i class="icon-truck"></i>
			{l s='Shipping' mod='g_ordermanager'}
		</div>
		<div id="carriers_err" style="display:none;" class="alert alert-warning"></div>
		<div id="carrier_form">
			<div class="form-group">
				<label class="control-label col-lg-3">
					{l s='Delivery option' mod='g_ordermanager'}
				</label>
				<div class="col-lg-9">
			...
			<div class="form-group">
				<label class="control-label col-lg-3" for="shipping_price">
					{l s='Shipping price (Tax incl.)' mod='g_ordermanager'}
				</label>
				<div class="col-lg-9">
			...
			<div class="form-group">
				<div class="checkbox col-lg-9 col-offset-3">
					<label for="carrier_recycled_package">
						...
						{l s='Recycled package' mod='g_ordermanager'}
					</label>
				</div>
			</div>
			{/if}
			...
			<div class="form-group">
				<div class="checkbox col-lg-9 col-offset-3">
					<label for="order_gift">
						...
						{l s='Gift' mod='g_ordermanager'}
					</label>
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-lg-3" for="gift_message">{l s='Gift message' mod='g_ordermanager'}</label>
				<div class="col-lg-9">
	...
	<div class="panel" id="summary_part" style="display:none;">
		<div class="panel-heading">
			...
			{l s='Summary' mod='g_ordermanager'}
		</div>
			...
			<div class="row">
				<div class="col-lg-2">
					<div class="data-focus">
						<span>{l s='Total products' mod='g_ordermanager'}</span><br/>
						<span id="total_products" class="size_l text-success"></span>
					</div>
				</div>
				<div class="col-lg-2">
					<div class="data-focus">
						<span>{l s='Total vouchers (Tax excl.)' mod='g_ordermanager'}</span><br/>
						<span id="total_vouchers" class="size_l text-danger"></span>
					</div>
				</div>
				<div class="col-lg-2">
					<div class="data-focus">
						<span>{l s='Total shipping (Tax excl.)' mod='g_ordermanager'}</span><br/>
						<span id="total_shipping" class="size_l"></span>
					</div>
				</div>
				<div class="col-lg-2">
					<div class="data-focus">
						<span>{l s='Total taxes' mod='g_ordermanager'}</span><br/>
						<span id="total_taxes" class="size_l"></span>
					</div>
				</div>
				<div class="col-lg-2">
					<div class="data-focus">
						<span>{l s='Total (Tax excl.)' mod='g_ordermanager'}</span><br/>
						<span id="total_without_taxes" class="size_l"></span>
					</div>
				</div>
				<div class="col-lg-2">
					<div class="data-focus data-focus-primary">
						<span>{l s='Total (Tax incl.)' mod='g_ordermanager'}</span><br/>
		...
		<div class="row">
			<div class="order_message_right col-lg-12">
				<div class="form-group">
					<label class="control-label col-lg-3" for="order_message">{l s='Order message' mod='g_ordermanager'}</label>
					<div class="col-lg-6">
						<textarea name="cart_summary[order_message]" id="order_message" rows="3" cols="45"></textarea>
					</div>
				</div>
				<div class="form-group">
					{if !$PS_CATALOG_MODE}
					<div class="col-lg-9 col-lg-offset-3">
						<a href="javascript:void(0);" id="send_email_to_customer" class="btn btn-default">
							<i class="icon-credit-card"></i>
							{l s='Send an email to the customer with the link to process the payment.' mod='g_ordermanager'}
						</a>
						<a id="go_order_process" href="" class="btn btn-link _blank">
							{l s='Go on payment page to process the payment.' mod='g_ordermanager'}
				...
				<div class="form-group">
					<label class="control-label col-lg-3">{l s='Payment' mod='g_ordermanager'}</label>
					<div class="col-lg-9">
						<select name="cart_summary[payment_module]" id="payment_module_name">
							{if !$PS_CATALOG_MODE}
							...
							{/foreach}
							{else}
								<option value="boorder">{l s='Back office order' mod='g_ordermanager'}</option>
							{/if}
						</select>
					</div>
				</div>
				<div class="form-group">
					<label class="control-label col-lg-3">{l s='Order status' mod='g_ordermanager'}</label>
					<div class="col-lg-9">
						<select name="cart_summary[order_state]" id="id_order_state">
							{foreach from=$order_states item='order_state'}
								<option value="{$order_state.id_order_state|escape:'html':'UTF-8'}" {if isset($smarty.post.id_order_state) && $order_state.id_order_state == $smarty.post.id_order_state}selected="selected"{/if}>{$order_state.name|escape:'html':'UTF-8'}</option>
							{/foreach}
						</select>
					</div>
				</div>
				<div class="form-group">
					<div class="col-lg-9 col-lg-offset-3">
						<button type="submit" name="submitCreateOrder"  class="btn btn-default" />
							<i class="icon-check"></i>
							{l s='Create the order' mod='g_ordermanager'}
						</button>
					</div>
				</div>
			</div>

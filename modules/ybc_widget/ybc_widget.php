...
class Ybc_widget extends Module
{
	...
	{	   
		...
		parent::__construct();
        //die($this->context->shop->id.'vvvvv');
    ...
    public function _installDb()
    {
        $tbls = array(
            "CREATE TABLE IF NOT EXISTS `"._DB_PREFIX_."ybc_widget_widget` (
              ...
              `custom_class` varchar(500) DEFAULT NULL,
    ...
    private function _postWidget()
    {
        ...
        {            
            ...
            $widget->id_shop = $this->context->shop->id;
            $widget->link = trim(Tools::getValue('link'));
            $widget->custom_class = trim(Tools::getValue('custom_class'));
    ...
    public function getWidgetsByHook($hook, $enabled = false)
    {
        ...
        $sql = "SELECT w.*, wl.title, wl.subtitle, wl.description 
                ...
                WHERE w.hook = '$hook' ".($enabled ? ' AND enabled=1 ' : '').$where." AND w.id_shop = ".(int)$shop_id."

...
class Ets_pmn_massedit_condition_action extends ObjectModel
{
    ...
    {
        $condisionFields = Db::getInstance()->executeS('SELECT id_ets_pmn_massedit_condition_action FROM  `'._DB_PREFIX_.'ets_pmn_massedit_condition_action` WHERE id_ets_pmn_massedit='.(int)$id_massedit.' AND id_ets_pmn_massedit_history=0');
        if($condisionFields)
        {

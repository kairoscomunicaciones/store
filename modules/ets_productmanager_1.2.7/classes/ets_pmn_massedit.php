...
class Ets_pmn_massedit extends ObjectModel
{
    ...
    public function save($null_values = false, $auto_date = true)
    {
        ...
        {
            return Db::getInstance()->execute('UPDATE  `'._DB_PREFIX_.'ets_pmn_massedit_condition` SET id_ets_pmn_massedit ="'.(int)$this->id.'" WHERE id_ets_pmn_massedit=1');
        }
    }
                            ...
                            if($edit_action['value'])
                            {
                                ...
                                if($specific_prices)
                                {
                                    ...
                                    if(isset($specific_prices['id_currency']) && ($id_currency= $specific_prices['id_currency']) && ($currency = new Currency($id_currency,$id_lang)) && Validate::isLoadedObject($currency) )
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Currency: %s'),$currency->name),'p');
                                    else
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($this->l('Currency: All currencies'),'p');
                                    if(isset($specific_prices['id_country']) && ($id_country= $specific_prices['id_country']) && ($country = new Country($id_country,$id_lang)) && Validate::isLoadedObject($country) )
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Country: %s'),$country->name),'p');
                                    else
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($this->l('Country: All countries'),'p');   
                                    if(isset($specific_prices['id_group']) && ($id_group= $specific_prices['id_group']) && ($group = new Group($id_group,$id_lang)) && Validate::isLoadedObject($group) )
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Customer group : %s'),$group->name),'p');
                                    else
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($this->l('Customer group: All groups'),'p'); 
                                    if(isset($specific_prices['id_customer']) && ($id_customer= $specific_prices['id_customer']) && ($customer = new Customer($id_customer)) && Validate::isLoadedObject($customer))
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Customer: %s'),$customer->firstname.' '.$customer->lastname.' ('.$customer->email.')'),'p');
                                    else
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($this->l('Customer: All customers'),'p');  
                                    if(isset($specific_prices['from']) && ($from = $specific_prices['from']) && Validate::isDate($from))
                                        $text .=  Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Available from: %s'),Tools::displayDate($from,$id_lang,false)),'p');
                                    if(isset($specific_prices['to']) && ($to = $specific_prices['to']) && Validate::isDate($to))
                                        $text .=  Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Available to: %s'),Tools::displayDate($to,$id_lang,false)),'p');
                                    if(isset($specific_prices['from_quantity']) && ($from_quantity = $specific_prices['from_quantity']))
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Starting at: %d'),$from_quantity),'p');
                                    if(isset($specific_prices['sp_reduction']) && ($sp_reduction = $specific_prices['sp_reduction']))
                                    {
                                        if(isset($specific_prices['sp_reduction_type']) && $specific_prices['sp_reduction_type']=='percentage')
                                            $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Apply a discount of: %s%s'),$sp_reduction,'%'),'p');
                                        else
                                        {
                                            if(isset($specific_prices['sp_reduction_tax']) && $specific_prices['sp_reduction_tax']==1)
                                                $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Apply a discount of: %s (Tax included.)'),Tools::displayPrice($sp_reduction)),'p');
                                            else
                                                $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Apply a discount of: %s (Tax excluded.)'),Tools::displayPrice($sp_reduction)),'p');
                                        }
                                    }
                                    if(isset($specific_prices['product_price']) && $specific_prices['product_price'])
                                        $text .= Module::getInstanceByName('ets_productmanager')->displayText(sprintf($this->l('Product price (tax excl.): %s'),Tools::displayPrice($specific_prices['product_price'])),'p');
                    ...
                    foreach($edit_action['languages'] as &$language)
                    {
                        if($language['value_lang'])
                        {
                            ...
                            foreach($value_lang as $val)
                                $text .= Module::getInstanceByName('ets_productmanager')->displayText($val,'p');
        ...
        if($customizations)
        {
            ...
            foreach($customizations as $customization)
            {
                if($customization['label'][$id_lang_default]!='')
                {
                    ...
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText($content,'p');
                }
                ...
                if(Validate::isLoadedObject($attributeGroup) && $attributes)
                {
                    ...
                    foreach($attributes as $id_attribute)
                    {
                        if(class_exists('AttributeCore'))
                            $attributeObj = new Attribute($id_attribute,$id_lang);
                        else
                    ...
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText($attributeGroup->name.': '.trim($attributeName,', '),'p');
                }
            }
            return $text;
        }
            ...
            foreach($id_features as $key=>$id_feature)
            {
                ...
                if(Validate::isLoadedObject($featureObj))
                {
                    if(isset($id_feature_values[$key]) && ($id_feature_value = (int)$id_feature_values[$key]) && ($featureValue = new FeatureValue($id_feature_value,$id_lang)) && Validate::isLoadedObject($featureValue))
                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($featureObj->name.': '.$featureValue->value,'p');
                    elseif(isset($feature_value_custom[$key]) && ($value = $feature_value_custom[$key]))
                        $text .= Module::getInstanceByName('ets_productmanager')->displayText($featureObj->name.': '.$value,'p');
                }
            }
            return $text;
         }   
    ...
    public function displayListCarriers($id_carriers)
    {
        ...
        if($carriers)
        {
            ...
            {
                $text .= Module::getInstanceByName('ets_productmanager')->displayText($carrier['name'] ? : Context::getContext()->shop->name,'p');
            }
            return $text;
        }
    }
    public function displayListProducts($id_products)
    {
        ...
        if($products = Db::getInstance()->executeS($sql))
        {
            ...
            {
                $text .= Module::getInstanceByName('ets_productmanager')->displayText($product['name'],'p');
            }
            return $text;
        }
    }
    public function displayListSuppliers($id_suppliers,$html= true)
    {
        ...
        if($suppliers)
        {
            ...
            foreach($suppliers as $supplier)
                $text .= $html ? Module::getInstanceByName('ets_productmanager')->displayText($supplier['name'],'p') : $supplier['name'].', ' ;
        ...
        if($id_categories)
        {
            ...
            if($categories)
            {
                ...
                foreach($categories as $category)
                    $text .= $html ? Module::getInstanceByName('ets_productmanager')->displayText($category['name'],'p') : $category['name'].', ';
                return $html ? $text : trim($text,', ');
            }
        }
    ...
    public function displayListBrands($id_manaufacturers,$html = true)
    {
        ...
        if($brands)
        {
            ...
            foreach($brands as $brand)
                $text .= $html ? Module::getInstanceByName('ets_productmanager')->displayText($brand['name'],'p') : $brand['name'].', ';
            return $html ? $text : trim($text,', ');
        }
    }
    public function displayListAttributes($id_attributes,$html=true)
    {
        ...
        if($attributes)
        {
            ...
            foreach($attributes as $attribute)
                $text .= $html ? Module::getInstanceByName('ets_productmanager')->displayText($attribute['name'],'p') :$attribute['name'].', ';
            return $html ? $text : trim($text,', ');
        }
    }
    public function displayFeaturesList($id_features,$html = true)
    {
        ...
        if($features)
        {
            ...
            foreach($features as $feature)
                $text .= $html ? Module::getInstanceByName('ets_productmanager')->displayText($feature['name'],'p') :$feature['name'].', ';
            return $html ? $text : trim($text,', ');
        }
    }
    ...
    public function getProducts($conditions,$total=false,$start=0,$limit=20,$excludeProducts='')
    {
        $filters = '';
        $leftJoin = '';
            ...
            foreach($conditions as $condition)
            {
                ...
                if($compared_value!='')
                {
                    if(($id_lang = (int)$condition['id_lang']) && in_array($filtered_field,array(Ets_pmn_massedit_condition::FILTERED_FIELD_NAME,Ets_pmn_massedit_condition::FILTERED_FIELD_DESCRIPTION,Ets_pmn_massedit_condition::FILTERED_FIELD_SUMMARY)))
                    {
                        ...
                        if($field && $operator)
                        {
                            $filters .= ' AND '.pSQL($field).' '.(string)$operator;
                        }
                    ...
                    if($filtered_field == Ets_pmn_massedit_condition::FILTERED_FIELD_BRAND)
                    {
                        if($condition['operator']=='in')
                            $filters .=' AND p.id_manufacturer IN ('.implode(',',array_map('intval',$compared_value)).')';
                        else
                            $filters .=' AND p.id_manufacturer NOT IN ('.implode(',',array_map('intval',$compared_value)).')';
                    }
                    if($filtered_field == Ets_pmn_massedit_condition::FILTERED_FIELD_SUPPLIER)
                    {
                        if($condition['operator']=='only_default')
                        {
                            $filters .=' AND p.id_supplier IN ('.implode(',',array_map('intval',$compared_value)).')';
                        }
                        else
                        {
                            if(!$search_supplier)
                            ...
                            if($condition['operator']=='in')
                                $filters .=' AND psu.id_supplier IN ('.implode(',',array_map('intval',$compared_value)).')';
                            else
                                $filters .=' AND (psu.id_supplier NOT IN ('.implode(',',array_map('intval',$compared_value)).') OR psu.id_supplier is null)';
                        }
                    }
                    if($filtered_field == Ets_pmn_massedit_condition::FILTERED_FIELD_CATEGORIES)
                    {
                        if($condition['operator']=='only_default')
                        {
                            $filters .=' AND p.id_category_default IN ('.implode(',',array_map('intval',$compared_value)).')';
                        }
                        else
                        {
                            if(!$search_category)
                            ...
                            if($condition['operator']=='in')
                                $filters .=' AND cp.id_category IN ('.implode(',',array_map('intval',$compared_value)).')';
                            else
                                $filters .=' AND cp.id_category NOT IN ('.implode(',',array_map('intval',$compared_value)).')';
                        }
                    }
                    if($filtered_field== Ets_pmn_massedit_condition::FILTERED_FIELD_ATTRIBUTE || $filtered_field == Ets_pmn_massedit_condition::FILTERED_FIELD_COLOR)
                    {
                        ...
                        if($condition['operator']=='in')
                            $filters .=' AND product_attribute.id_product is not null';
                        else
                            $filters .=' AND product_attribute.id_product is null';
                    }
                    if($filtered_field== Ets_pmn_massedit_condition::FILTERED_FIELD_FEATURES)
                    {
                        if(!$search_features)
                        ...
                        if($condition['operator']=='in')
                            $filters .=' AND fp.id_feature_value is NOT NULL';
                        else
                            $filters .=' AND fp.id_feature_value is null';
                    }
                }
        ...
        if($total)
        {
            ...
            INNER JOIN  `'._DB_PREFIX_.'stock_available` sa ON (sa.id_product=ps.id_product AND sa.id_product_attribute=0 AND sa.id_shop="'.(int)Context::getContext()->shop->id.'")
            '.(string)$leftJoin.'
            WHERE p.state=1 AND ps.id_shop = "'.(int)Context::getContext()->shop->id.'"'.($filters ? (string)$filters:'').($excludeProducts ? ' AND p.id_product NOT IN ('.implode(',',array_map('intval',explode(',',$excludeProducts))).')':'');
            return Db::getInstance()->getValue($sql);
        }
        else
        {
            ...
            '.(string)$leftJoin.'
            WHERE p.state=1 AND ps.id_shop = "'.(int)Context::getContext()->shop->id.'"'.($filters ? (string)$filters:'').($excludeProducts ? ' AND p.id_product NOT IN ('.implode(',',array_map('intval',explode(',',$excludeProducts))).')':'').' ORDER BY p.id_product'.($limit ? ' LIMIT '.(int)$start.','.(int)$limit:'');
    ...
    public function renderListProductsByConditions($conditions,$product_excluded,$excluded = true,$page=1,$limit=10,$stepNumber=0)
    {
        if($limit<=0)
            $limit =10;
        $fields_list = array(
        ...
        if($page<=0)
            $page = 1;
        $totalRecords = (int) $this->getProducts($conditions,true,0,false,!$excluded ? $product_excluded:'');
        ...
        if($start < 0)
            $start = 0;
        $products = $this->getProducts($conditions,false,$start,$paggination->limit,!$excluded ? $product_excluded:'');
        if($excluded)
        {
            if($product_excluded)
            {
        ...
        if($products)
        {
            foreach($products as &$product)
            {
                ...
                $product['image'] = Module::getInstanceByName('ets_productmanager')->displayText(Module::getInstanceByName('ets_productmanager')->displayText('','img',array('src'=>$product['image'])),'a',array('href'=>$product['link']));
                if($excluded)
                {
                    $attr_datas = array(
                        'type' => 'checkbox',
                    ...
                    if($product_excluded)
                    {
                        ...
                        $attr_datas['title'] = $this->l('Click to unselect');
                    $product['edit_action'] = $module->displayText($module->displayText('','input',$attr_datas).$module->displayText('','span',array('class'=>'check_input_color')),'div',array('class'=>'ets_mg_edit_action'));
                }
            }
        }
        $listData = array(
    ...
    {
        $context = Context::getContext();
        $sql = 'SELECT ag.id_attribute_group,ag.is_color_group,agl.name FROM  `'._DB_PREFIX_.'attribute_group` ag
        INNER JOIN  `'._DB_PREFIX_.'attribute_group_shop` ags ON (ag.id_attribute_group=ags.id_attribute_group AND ags.id_shop="'.(int)$context->shop->id.'")
        ...
        if($attributeGroups)
        {
            foreach($attributeGroups as &$attributeGroup)
            {
                ...
                INNER JOIN `'._DB_PREFIX_.'attribute_shop` ash ON (a.id_attribute = ash.id_attribute AND ash.id_shop="'.(int)$context->shop->id.'")

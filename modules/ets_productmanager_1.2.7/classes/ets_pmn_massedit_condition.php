...
class Ets_pmn_massedit_condition extends ObjectModel
{
    ...
    public function getListFields()
    {
        ...
        if(!$this->getAttributes())
            unset($fields[Ets_pmn_massedit_condition::FILTERED_FIELD_ATTRIBUTE]);
        if(!$this->getFeatures())
            unset($fields[Ets_pmn_massedit_condition::FILTERED_FIELD_FEATURES]);
        if(!$this->getAttributeColors())
            unset($fields[Ets_pmn_massedit_condition::FILTERED_FIELD_COLOR]);
        if(!$this->getManufacturers())
            unset($fields[Ets_pmn_massedit_condition::FILTERED_FIELD_BRAND]);
        if(!$this->getSuppliers())
            unset($fields[Ets_pmn_massedit_condition::FILTERED_FIELD_SUPPLIER]);
        return $fields;
    }
    ...
    public function getFeatures($id_feature_values = false)
    {
        ...
        INNER JOIN `'._DB_PREFIX_.'feature_shop` fs ON (f.id_feature = fs.id_feature AND fs.id_shop="'.(int)$this->context->shop->id.'")

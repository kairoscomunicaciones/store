...
class Ets_pmn_massedit_history extends ObjectModel
{
    ...
    public function getProducts($conditions,$total=false,$start=0,$limit=20,$excludeProducts='')
    {
        ...
        else
        {
            ...
            '.(string)$leftJoin.'
            WHERE p.state=1 AND ps.id_shop = "'.(int)Context::getContext()->shop->id.'"'.($filters ? (string)$filters:'').($excludeProducts ? ' AND p.id_product NOT IN ('.implode(',',array_map('intval',explode(',',$excludeProducts))).')':'').' GROUP BY p.id_product ORDER BY p.id_product'.($limit ? ' LIMIT '.(int)$start.','.(int)$limit:'');
    ...
    public function updateProductFeature($id_product,$condition_field)
    {
        ...
        if($condition=='remove')
        {
            ...
            {
                Db::getInstance()->execute('DELETE FROM  `'._DB_PREFIX_.'feature_product` WHERE id_product='.(int)$id_product.' AND id_feature IN ('.implode(',',array_map('intval',$id_features)).')');
            } 
        }
        if($condition=='add' || $condition=='replace')
        {
    ...
    public function displayProductFeatures($product)
    {
        ...
        if($features)
        {
            ...
            {
                $text .= Module::getInstanceByName('ets_productmanager')->displayText($feature['name'].': '.$feature['value'],'p');
            }
            return $text;
        }
    ...
    public function displayProductCombinations($product)
    {
        ...
        if($product_attributes)
        {
            ...
            foreach($product_attributes as $product_attribute)
            {
                $text .= Module::getInstanceByName('ets_productmanager')->displayText(Ets_pmn_defines::getInstance()->getProductAttributeName($product_attribute['id_product_attribute']),'p');
            }
            return $text;
        }
    }
    ...
    public function displayListCombinations($combinations)
    {
        if($combinations)
        {
            ...
            {
                $text .= Module::getInstanceByName('ets_productmanager')->displayText($combination['name'],'span');
            }
            return $text;
        }
    }
    public function displayListStocks($stocks)
    {
        if($stocks)
        {
            ...
            foreach($stocks as $stock)
            {
                ...
                $text .= Module::getInstanceByName('ets_productmanager')->displayText($val,'span');
            }
            return $text;
        }
    }
        ...
        if($features)
        {
            ...
            foreach($features as $feature)
            {
                ...
                if(Validate::isLoadedObject(self::$features[$id_feature]) && Validate::isLoadedObject(self::$feature_values[$id_feature_value]))
                    $text.= Module::getInstanceByName('ets_productmanager')->displayText(self::$features[$id_feature]->name.': '.self::$feature_values[$id_feature_value]->value,'span');
            }
            return $text;
        }
    }
        ...
        if($categories)
        {
            ...
            foreach($categories as $category)
            {
                ...
                if(Validate::isLoadedObject(self::$categories[$category['id_category']]))
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText(self::$categories[$category['id_category']]->name,'span');
            }
            return $text;
        }
    }
        ...
        if($carriers)
        {
            ...
            foreach($carriers as $carrier)
            {
                ...
                if(Validate::isLoadedObject(self::$carriers[$id_carrier_reference]))
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText(self::$carriers[$id_carrier_reference]->name,'span');
            }
            return $text;
        }
    }
        ...
        if($products)
        {
            ...
            foreach($products as $product)
            {
                ...
                if(Validate::isLoadedObject(self::$products[$id_product]))
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText(self::$products[$id_product]->name,'span');            
            }
            return $text;
        }
    }
    ...
    public function displayListTags($tags)
    {
        if($tags)
        {
            ...
            foreach($tags as $tag)
            {
                ...
                $text .=Module::getInstanceByName('ets_productmanager')->displayText(self::$tags[$id_tag]->name,'span');
            }
            return $text;
        }
    }
        ...
        if($specific_prices)
        {
            ...
            foreach($specific_prices as $specific_price)
            {
                if($specific_price['reduction_type']=='amount')
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText('-'.Tools::displayPrice($specific_price['reduction']),'span');
                else
                    $text .= Module::getInstanceByName('ets_productmanager')->displayText('-'.Tools::ps_round($specific_price['reduction']*100,2).'%','span');
            }
            return $text;
        }
    }

...
{  
    ...
    xhrAjax = $.ajax({
        ...
        success: function(html){
            if(html)
            {
               var begin =  html.indexOf('<!--begin_tbody_list_product!-->');
               var end =  html.indexOf('<!--end_tbody_list_product!-->');
               if(begin >0 && end >0)
               {
                    var list_products = html.substr(begin,end-begin);
                   $('.tbody_list_product').replaceWith(list_products);
                   var begin_pagination =  html.indexOf('<!--begin_pagination_list_product!-->');
                   var end_pagination =  html.indexOf('<!--end_pagination_list_product!-->');
...
$(document).ready(function(){
   ...
   $(document).on('click','#desc-product-arrange,#desc-product-arrange2',function(e){
        ...
        if($('tr[data-product-id="'+id_product+'"] input[name="price_final"]').length)
        {
            if(parseFloat($(this).val()) >=0)
            {
                if($('tr[data-product-id="'+id_product+'"] select[name="tax_name"]').length)
                    id_tax_rules_group = $('tr[data-product-id="'+id_product+'"] select[name="tax_name"]').val();
                else
                    id_tax_rules_group = $('tr[data-product-id="'+id_product+'"] input[name="id_tax_rules_group"]').val();
    ...
    $(document).on('click','button[name="submitProductChangeInLine"],button[name="submitProductChangeInLine2"]',function(e){
        ...
        if(!$this.hasClass('loading'))
        {
            ...
            $.ajax({
                ...
                data: $('tr[data-product-id="'+id_product+'"] :input').serialize()+'&submitProductChangeInLine=1',
                type: 'post',
                dataType: 'json',
                success: function(jsonData){
                    ...
                    if(jsonData.success)
                    {
                        ...
                        $(jsonData.columns).each(function(){
            				if($('tr[data-product-id="'+id_product+'"] td.column.'+this.name +' .content').length)
           					{
           					    if($this.attr('name')=='submitProductChangeInLine')
           					        $('tr[data-product-id="'+id_product+'"] td.column.'+this.name +' .content').html('<div class="content_info">'+(this.value ? this.value:'--')+'</div>');
                                else
                                    $('tr[data-product-id="'+id_product+'"] td.column.'+this.name +' .content').html('<div class="content_info">'+(this.value ? this.value:'--')+'</div>');
           					}
            			});
    ...
    $(document).on('change','.id_features',function(){
        ...
        $(this).closest('.ets_pmn-product-feature').find('.id_feature_values').removeAttr('disabled');
        ...
        {
            $(this).closest('.ets_pmn-product-feature').find('.id_feature_values').attr('disabled','disabled');
        }
    });
...
function etsEditInlineProductAction($this)
{
    ...
    if($('tr[data-product-id="'+id_product+'"] .wapper-change-product .translatable-field').length && $('tr[data-product-id="'+id_product+'"] .wapper-change-product .translatable-field .dropdown-toggle.disabled').length )
    {
        $.ajax({
            url: $('#desc-product-arrange').attr('href').replace('&arrangeproduct=1',''),
                ...
                if(jsonData.inputs)
                {
                    $(jsonData.inputs).each(function(){
        				if($('tr[data-product-id="'+id_product+'"] td.column.'+this.name +' .wapper-change-product').length)
       					{
       					    $('tr[data-product-id="'+id_product+'"] td.column.'+this.name +' .wapper-change-product .form-group').append(this.form_html);
       					}
        			});

...
//        }
    ...
    public function showFreeShippingBannerCalculations()
    {
        ...
        $total_cart_amount = $this->context->cart->getOrderTotal(true, 4);
...
//            die;
    ...
    private function loadCarriers(
        ...
        if (Tools::isSubmit('ajax')) {
            /*Start: Added by Anshul Miital on 16-Jan-2020 for free version change*/
            $is_free = true;
            $shipping_cost = '';
            $sql = 'SELECT id_carrier FROM `'._DB_PREFIX_.'carrier` WHERE is_module = 0 AND active = 1 AND deleted = 0 AND is_free = 1 ORDER BY id_carrier';
            $id_carrier = DB::getInstance()->getValue($sql);
            if (!isset($id_carrier) || $id_carrier == '' || $id_carrier == 0) {
                $this->shipping_error[] = $this->module->l('No Free carrier available. Please enable any carrier which has free shipping enabled.', 'supercheckout');
                }
            ...
            /*End: Added by Anshul Miital on 16-Jan-2020 for free version change*/
    ...
    private function getShippingCost($id_carrier)
    {
        $delivery_options = $this->checkout_session->getDeliveryOptions();
        foreach ($delivery_options as $id_carrier_exist => $carrier) {
            if ($id_carrier_exist == $id_carrier.',') {
...
//        }
    ...
    protected function loadPaymentMethods($selected_payment_method = 0)
    {
        ...
        /*Start: Added by Anshul Mittal on 16-Jan-2020 for free Supercheckout version*/
        $free_payment_avail = array();
        $free_payment_avail['check_avail'] = false;
        $free_payment_avail['wire_avail'] = false;
        if ($available_payments) {
            foreach ($available_payments as $module_name => $module_options) {
                if ($module_name == 'ps_checkpayment') {
                    $free_payment_avail['check_avail'] = true;
                    foreach ($module_options as $option) {
                        $free_payment_avail['check_id'] = $option['id'];
                    }
                } else if ($module_name == 'ps_wirepayment') {
                    $free_payment_avail['wire_avail'] = true;
                    foreach ($module_options as $option) {
                        $free_payment_avail['wire_id'] = $option['id'];
                    }
                }
            }
        }
        ...
        $this->context->smarty->assign('free_payment_avail', $free_payment_avail);
        ...
        /*End: Added by Anshul Mittal on 16-Jan-2020 for free Supercheckout version*/
        $temp_vars = array(
            'html' => $this->context->smarty->fetch(
...
//            $delivery_option = Tools::unSerialize($this->context->cart->delivery_option);
        ...
        if (isset($this->supercheckout_settings['enable']) && $this->supercheckout_settings['enable'] == 1) {
            foreach ($delivery_option_list as $id_address => $options) {
                if (isset($this->supercheckout_settings['shipping_method']['default']) && $this->supercheckout_settings['shipping_method']['default']) {

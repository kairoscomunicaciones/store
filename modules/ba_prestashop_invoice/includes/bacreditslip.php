...
class BaCreditSlip extends ba_prestashop_invoice
{
    ...
    public function createbaCreditSlipList()
    {
        ...
        $helper->toolbar_btn['new'] =  array(
            ...
            .'&add'.$this->name.'_creditslip&task=creditslip&token='.Tools::getAdminTokenLite('AdminModules'),
            'desc' => $this->l('Add new')
        );
        $helper->toolbar_btn['import'] = array(
            'href' => AdminController::$currentIndex.'&configure='.$this->name.'&import'.$this->name
            .'_creditslip&task=creditslip&token='.Tools::getAdminTokenLite('AdminModules'),
            'desc' => $this->l('Import')
        );
        ...
        $helper->title = $this->l('Credit Slips Manager');
        $helper->table = $this->name.'_creditslip';
        $helper->list_id = $this->name.'_creditslip';
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name.'&task=creditslip';
        $page = Tools::getValue('submitFilter'.$helper->list_id);
        if (!empty($page)) {
            $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name
                .'&task=creditslip&submitFilter'.$helper->list_id.'='.(int)$page;
        }
        ...
        $html=$helper->generateList($this->getListContentCreditSlip($helper), $fields_list);
        return $html;
    }
    public function getTotalList($helper)
    {
        ...
        $sql = 'SELECT count(*) FROM '._DB_PREFIX_.'ba_prestashop_credit_slip WHERE id_shop='
    ...
    public function getListContentCreditSlip($helper)
    {
        $db = Db::getInstance(_PS_USE_SQL_SLAVE_);
        $orderby = pSQL(Tools::getValue($helper->list_id."Orderby", "name"));
        $orderway = pSQL(Tools::getValue($helper->list_id."Orderway", "ASC"));
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'ba_prestashop_credit_slip WHERE id_shop='.$this->context->shop->id;
        $sql .= $this->setWhereClause($helper);
    ...
    public function caseCreditSlip()
    {
        $html="";
        $adminControllers=AdminController::$currentIndex;
        $token='&token='.Tools::getAdminTokenLite('AdminModules');
        $configAndTask='&configure='.$this->name.'&task=creditslip';
        if (Tools::isSubmit('submitBaSave')) {
            $this->saveDataCreditSlip();
            return $html.=$this->createbaCreditSlipList();
            ...
            $url=$adminControllers.$token.$configAndTask.'&id='.$id.'&updateba_prestashop_invoice_creditslip&msg=1';
            ...
            $import="importba_prestashop_invoice_creditslip";
            $this->importDataPDF("ba_prestashop_credit_slip", "creditslip", $import);
            Tools::redirectAdmin($adminControllers.$token.$configAndTask);
        } elseif (Tools::isSubmit('importba_prestashop_invoice_creditslip')) {
            return $this->display("ba_prestashop_invoice", 'views/templates/admin/import.tpl');
        } elseif (Tools::isSubmit('submitResetba_prestashop_invoice_creditslip')) {
            Tools::redirectAdmin($adminControllers.$token.$configAndTask);
        } elseif (Tools::isSubmit('updateba_prestashop_invoice_creditslip')) {
            $this->getDataCreditSlip();
            return $this->display("ba_prestashop_invoice", 'views/templates/admin/order_credit/invoice_edit.tpl');
        } elseif (Tools::isSubmit('addba_prestashop_invoice_creditslip')) {
            $this->getDataCreditSlip();
            return $this->display("ba_prestashop_invoice", 'views/templates/admin/order_credit/invoice_add.tpl');
        } elseif (Tools::isSubmit('statusba_prestashop_invoice_creditslip')) {
            $id = Tools::getValue('id');
            $sql="SELECT id_lang,status FROM "._DB_PREFIX_."ba_prestashop_credit_slip WHERE id=".(int)$id;
            $result = Db::getInstance()->getRow($sql);
            if ($result['status']=='0') {
                $sql="UPDATE "._DB_PREFIX_."ba_prestashop_credit_slip SET status=0 WHERE id_lang="
                .(int)$result['id_lang'].' AND id_shop='.$this->context->shop->id;
                Db::getInstance()->execute($sql);
            }
            $sql="UPDATE "._DB_PREFIX_."ba_prestashop_credit_slip SET status=1-status WHERE id=".(int)$id;
            Db::getInstance()->execute($sql);

            $url = $adminControllers.$token.$configAndTask.'&msg=1';
            Tools::redirectAdmin($url);
            return true;
        } elseif (Tools::isSubmit('deleteba_prestashop_invoice_creditslip')) {
            $id = Tools::getValue('id');
            Db::getInstance()->delete('ba_prestashop_credit_slip', "id=".(int)$id);
            return $this->createbaCreditSlipList();
        } elseif (Tools::isSubmit('submitBulkdeleteba_prestashop_invoice_creditslip')) {
            $idArray = Tools::getValue('ba_prestashop_invoice_creditslipBox');
            $idString = implode(",", $idArray);
            Db::getInstance()->delete('ba_prestashop_credit_slip', "id IN (".pSQL($idString).")");
            return $this->createbaCreditSlipList();
        } elseif (Tools::isSubmit('duplicateba_prestashop_invoice_creditslip')) {
    ...
    public function processDuplicate()
    {
        $id = Tools::getValue('id');
        $db = Db::getInstance();
        $sql='INSERT INTO '._DB_PREFIX_.'ba_prestashop_credit_slip(`showShippingInProductList`,
        ...
        FROM '._DB_PREFIX_.'ba_prestashop_credit_slip WHERE id='. (int) $id;
        $db->query($sql);
    }
    private function saveDataCreditSlip()
    {
        $db = Db::getInstance();
        ...
        if (!empty($id)) {
            if ((int) Tools::getValue('status') == 1) {
                $db->update("ba_prestashop_credit_slip", array(
                        'status' => 0
            ...
            $db->insert("ba_prestashop_credit_slip", array(
                'name' => pSQL($name),
                'description' => $description,
                'showShippingInProductList' => $showShippingInProductList,
                'showDiscountInProductList' => $showDiscountInProductList,
    ...
    private function getDataCreditSlip()
    {
        $db = Db::getInstance();
        $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_credit_slip WHERE id='. (int) Tools::getValue('id');

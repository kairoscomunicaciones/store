...
class BaInvoiceHelper
{
    ...
    public function getNextNumber($id_shop, $type, $start = 1, $step = 1, $reset = 0, $reset_value = '')
    {
        ...
        $start_date = pSQL($this->databaseRef[$type]['start_date']);
        if ($reset == 0) {
            // = None
        ...
        if ($reset == 2) {
            // date
            $sql = sprintf('SELECT GREATEST( %d + COUNT(a.%s) * %d, 1) AS next_number FROM %s a', $s, $c, $se, $t);
            $date = date('Y-m-d', (int) $reset_value);
            $where .= sprintf(' AND a.%s >= \'%s\'', $start_date, pSQL($date));
        }
        if ($reset == 3) {
            // date
            $sql = sprintf('SELECT GREATEST( %d + COUNT(a.%s) * %d, 1) AS next_number FROM %s a', $s, $c, $se, $t);
    ...
    public static function getNumberFormatted($format, $counter, $length, $date)
    {
        ...
        $format = str_replace('[m]', sprintf('%02d', $date_arr['month']), $format);
        $format = str_replace('[Y]', $date_arr['year'], $format);
        $format = str_replace('[y]', Tools::substr($date_arr['year'], -2), $format);
        $format = str_replace('[d]', sprintf('%02d', $date_arr['day']), $format);
        return $format;
    }
    ...
    public function formatInvoiceByFilename($filename, $id_lang, $id_shop)
    {
        ...
        $number = str_replace($re_arr, '', $filename);
        $number = ltrim($number, '0');
        if (!$number || $this->isEnabledCustomNumber('INVOICE') == false) {
            return $filename;
        }
        $invoice = OrderInvoice::getInvoiceByNumber($number);
    ...
    public function formatDeliveryByFilename($filename, $id_lang, $id_shop)
    {
        ...
        $number = str_replace($re_arr, '', $filename);
        $number = ltrim($number, '0');
        if (!$number || $this->isEnabledCustomNumber('DELIVERY') == false) {
            return $filename;
        }
        $invoice = OrderInvoice::getInvoiceByNumber($number);

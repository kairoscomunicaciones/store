...
class HTMLTemplateOrderSlip extends HTMLTemplateOrderSlipCore
{
    ...
    public function __construct(OrderSlip $order_slip, $smarty)
    {
        ...
        if (Tools::version_compare(_PS_VERSION_, '1.6.1', '<')) {
            foreach ($this->order_slip->getProducts() as $value) {
                $this->order_slip->total_products_tax_excl += $value['amount_tax_excl'];
                $this->order_slip->total_products_tax_incl += $value['amount_tax_incl'];
                $this->order_slip->total_shipping_tax_excl += $value['total_shipping_price_tax_excl'];
                $this->order_slip->total_shipping_tax_incl += $value['total_shipping_price_tax_incl'];
            }
            $this->order_slip->total_shipping_tax_excl += $this->order_slip->shipping_cost_amount;
            $this->order_slip->total_shipping_tax_incl += $this->order_slip->shipping_cost_amount;
        }
        $this->order = new Order($this->order_slip->id_order);
        $this->invoice = new OrderInvoice($this->order->invoice_number);
        $this->smarty = $smarty;
        $this->date = $this->displayDate($this->order->invoice_date);
        $this->title = HTMLTemplateOrderSlip::l('Credit')
        .' #'.Configuration::get('PS_CREDIT_SLIP_PREFIX', Context::getContext()->language->id)
        .sprintf('%06d', $this->order_slip->id);
        ...
        if ($customnumber_setting['cre_custemp_status'] == 1) {
            $gr_customer = Customer::getGroupsStatic($this->order->id_customer)[0];
            $sqltemp_cre_grcus='SELECT * FROM '._DB_PREFIX_.'ba_template_credit_grcustumer WHERE id_lang='
                .(int)$this->order->id_lang.' AND id_group_customer='.$gr_customer.' AND id_shop='
                .$this->order->id_shop;
            $id_tem_gr_in = $db->ExecuteS($sqltemp_cre_grcus)[0]['id_template_credit'];
            $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_credit_slip WHERE'
            .' id='.(int)$id_tem_gr_in;
        } else {
    ...
    public function taxGroup()
    {
        ...
        foreach ($this->order_slip->getProducts() as $value) {
            for ($i = 0; $i < count($taxGroup); $i ++) {
                if ($taxGroup[$i]['id_product'] == $value['product_id']) {
                    $taxGroup[$i]['product_qty'] = $value['product_quantity'];
                }
            }
        }
    ...
    {
        if (Module::isEnabled('ba_prestashop_invoice')==false || get_class($this)!='HTMLTemplateOrderSlip') {
    ...
    private function checkContentType($content_type, $product, $j)
    {
        ...
        switch ($content_type) {
            ...
            case "4":
                ...
                .' product_list_col_'.($j+1).'" 
                >';
                $unit_price_tax_incl = $product['amount_tax_incl'] / $product['product_quantity_refunded'];
                $unit_price_tax_excl = $product['amount_tax_excl'] / $product['product_quantity_refunded'];
                $html .= '- ' . Tools::displayPrice($unit_price_tax_incl, (int) $this->order->id_currency);
            ...
            case "5":
                $html.='<td class="product_list_content product_list_content_'.($j+1).' product_list_col_'.($j+1).'" 
                >';
                $total_price_tax_incl=$product['amount_tax_incl'];
                $total_price_tax_excl=$product['amount_tax_excl'];
            ...
            case "8":
                ...
                .' product_list_col_'.($j+1).'" >';
                $html .= '- '. Tools::displayPrice($product['amount_tax_incl'], (int) $this->order->id_currency);
                $html.='</td>';
                break;
            case "9":
            ...
            case "11":
                ...
                $productPriceOld = $product['original_product_price'];
            ...
            case "14":
                $html.='<td class="product_list_content product_list_content_'.($j+1).' product_list_col_'.($j+1).'">';
                $html .= '- ' . Tools::displayPrice($product['amount_tax_excl'], (int) $this->order->id_currency);
                $html.='</td>';
                break;
            case "15":
            ...
            case "16":
                ...
                $taxRate = (($unit_price_tax_incl-$unit_price_tax_excl)/$unit_price_tax_excl)*100;
                $productPriceOld = $product['original_product_price'] + ($product['original_product_price']*($taxRate/100));
            ...
            case "27":
                ...
                if (!empty($product['product_reference'])) {
                    $barcode ='<div class="barcode_cell">';
                    $barcode .='<barcode style="margin-left: -15px;" code="'.$product['product_reference'].'"';
                    $barcode .=' type="C128B" class="barcode" text="1">';
                    $barcode .='</barcode>';
                    $barcode .='</div>';
                }
                $html .= $barcode;
    ...
    {
        $displayPDFInvoice = Hook::exec('displayPDFInvoice', array('object' => $this->order_slip));
        $html = str_replace("[displayPDFInvoice]", $displayPDFInvoice, $html);
        
        $displayPDFDeliverySlip = Hook::exec('displayPDFDeliverySlip', array('object' => $this->order_slip));
        $html = str_replace("[displayPDFDeliverySlip]", $displayPDFDeliverySlip, $html);
        
        $orderIdCurrency=(int)$this->order->id_currency;
        $this->date = $this->displayDate($this->order_slip->date_add);
        $db = Db::getInstance(_PS_USE_SQL_SLAVE_);
        $search_order = 'SELECT * FROM ' . _DB_PREFIX_ . 'orders WHERE id_order = ' . $this->order_slip->id_order;
        ...
        $messagesArr=CustomerMessage::getMessagesByOrderId((int)$this->order->id, true);
        $htmlMessage=null;
        if (count($messagesArr)) {
            foreach ($messagesArr as $message) {
                $htmlMessage.="<p class='order_message'>";
                $htmlMessage.= nl2br($message['message']);
                $htmlMessage.= "</p>";
            }
        }
        $html = str_replace("[order_message]", $htmlMessage, $html);
        
        $total_paid_tax_excl = $this->order_slip->total_products_tax_excl;
        $total_paid_tax_incl = $this->order_slip->total_products_tax_incl;
        ...
        $total_shipping_tax_incl=$this->order_slip->total_shipping_tax_incl;
        $total_shipping_tax_excl=$this->order_slip->total_shipping_tax_excl;
        ...
        $total_order_excl_tax='- '.Tools::displayPrice($total_paid_tax_excl, $orderIdCurrency);
        ...
        $total_order_incl_tax2 = $total_shipping_tax_incl+$total_paid_tax_incl;
        $total_order_incl_tax='- '.Tools::displayPrice($total_order_incl_tax2, $orderIdCurrency);
        $html = str_replace("[total_order_incl_tax]", $total_order_incl_tax, $html);
        
        $html = str_replace("[invoice_date]", $this->date, $html);
        $html = str_replace("[credit_date]", $this->displayDate($this->order_slip->date_add), $html);
        ...
        if ($helper->isEnabledCustomNumber('CREDIT') == true) {
            $n = $this->order_slip->number;
            $d = $this->order_slip->date_add;
            $creditNumber = $helper->formatCreditbyNumber($n, $d, $id_lang, $id_shop);
        } else {
            $creditNumber=Configuration::get('PS_CREDIT_SLIP_PREFIX', $id_lang, null, $id_shop)
            .sprintf('%06d', $this->order_slip->id);
        }
        $html = str_replace("[credit_number]", $creditNumber, $html);
        ...
        $order_subtotal=$this->order_slip->total_products_tax_excl;
        $order_subtotal = '- '.Tools::displayPrice($order_subtotal, $orderIdCurrency);
        $html= str_replace("[order_subtotal]", $order_subtotal, $html);
        $order_shipping_cost='- '.Tools::displayPrice($this->order_slip->total_shipping_tax_incl, $orderIdCurrency);
        $html = str_replace("[order_shipping_cost]", $order_shipping_cost, $html);
        $totalTax = ($this->order_slip->total_products_tax_incl - $this->order_slip->total_products_tax_excl);
        $html = str_replace("[order_tax]", '- '.Tools::displayPrice($totalTax, $orderIdCurrency), $html);
        $productListObj = $this->order_slip->getProducts();
        ...
        foreach ($productListObj as $productList) {
            ...
            $total_product_quantity += $productList['product_quantity'];
        }
        $total_product_quantity = number_format($total_product_quantity);
        $html = str_replace("[total_product_quantity]", $total_product_quantity, $html);
        ...
        $order_total= $this->order_slip->total_products_tax_incl + $this->order_slip->total_shipping_tax_incl;
        $html = str_replace("[order_total]", '- '.Tools::displayPrice($order_total, $orderIdCurrency), $html);
        $customer = new CustomerCore($this->order->id_customer);
        ...
        $billing_due_date = strtotime($this->order_slip->date_add)+$max_payment_days*24*60*60;
        ...
        $html = str_replace("[order_notes]", nl2br($this->order->note), $html);
        
        $invoiceNumberBarcode=sprintf('%06d', $this->order_slip->id);
        ...
        $product_list = $this->order_slip->getProducts();
        ...
        if ($showDiscountInProductList=="Y") {
            ...
            foreach ($product_list as $productDiscount) {
                ...
                if (isset($productDiscount['reduction_amount']) && $productDiscount['reduction_amount']>0) {
                    ...
                    $discountList["original_product_price"] = $productDiscount['reduction_amount'];
                    ...
                    $discountList["original_product_price"] = $discountAmount;
                } else {
                    $discountList["unit_price_tax_excl"] = 0;
                    $discountList["unit_price_tax_incl"] = 0;
                    ...
                    $discountList["original_product_price"] = 0;
                }
            ...
            $discountOrder["unit_price_tax_excl"] = -$this->order_slip->total_discount_tax_excl;
            $discountOrder["unit_price_tax_incl"] = -$this->order_slip->total_discount_tax_incl;
            $discountOrder["total_price_tax_incl"] = -$this->order_slip->total_discount_tax_incl;
            $discountOrder["total_price_tax_excl"] = -$this->order_slip->total_discount_tax_excl;
            ...
            $discountOrder["original_product_price"] = 0;
        ...
        if ($showShippingInProductList=="Y") {
            ...
            $shipping["unit_price_tax_excl"] = $this->order_slip->total_shipping_tax_excl;
            $shipping["unit_price_tax_incl"] = $this->order_slip->total_shipping_tax_incl;
            $shipping["reduction_percent"] = null;
            $shipping["reduction_amount"] = null;
            $shipping["product_quantity"] = "1";
            $shipping["product_price"] = $this->order_slip->total_shipping_tax_excl;
            $shipping["original_product_price"] = $this->order_slip->total_shipping_tax_excl;
            $shipping["product_reference"] = "--";
            $shipping["total_price_tax_incl"] = $this->order_slip->total_shipping_tax_incl;
            $shipping["total_price_tax_excl"] = $this->order_slip->total_shipping_tax_excl;

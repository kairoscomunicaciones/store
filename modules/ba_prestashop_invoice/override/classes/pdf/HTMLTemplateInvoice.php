...
class HTMLTemplateInvoice extends HTMLTemplateInvoiceCore
{
    ...
    public function __construct(OrderInvoice $order_invoice, $smarty)
    {
        ...
        if ($customnumber_setting['invoice_custemp_status'] == 1) {
            $gr_customer = Customer::getGroupsStatic($this->order->id_customer)[0];
            $sqltemp_invoice_grcus='SELECT * FROM '._DB_PREFIX_.'ba_template_invoice_grcustumer WHERE id_lang='
                .(int)$this->order->id_lang.' AND id_group_customer='.$gr_customer.' AND id_shop='
                .$this->order->id_shop;
            $id_tem_gr_in = $db->ExecuteS($sqltemp_invoice_grcus)[0]['id_template_invoice'];
            if ($controller == "AdminPdf") {
                $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_invoice WHERE id_lang='
                .(int)$this->order->id_lang.' AND id='.(int)$id_tem_gr_in
                .' AND (useAdminOrClient=1 OR useAdminOrClient=0)';
            } else {
                $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_invoice WHERE id_lang='
                .(int)$this->order->id_lang.' AND id='.(int)$id_tem_gr_in
                .' AND (useAdminOrClient=2 OR useAdminOrClient=0)';
            }
        } else {
            if ($controller == "AdminPdf") {
                $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_invoice WHERE id_lang='
                .(int)$this->order->id_lang.' AND status=1 AND (useAdminOrClient=1 OR useAdminOrClient=0) AND id_shop='
                .$this->order->id_shop;
            } else {
                $sql='SELECT * FROM '._DB_PREFIX_.'ba_prestashop_invoice WHERE id_lang='
    ...
    private function replaceToken($html)
    {
        ...
        */
        ...
        if ($helper->isEnabledCustomNumber('INVOICE') == true) {
            $n = $this->order_invoice->number;
            $d = $this->order_invoice->date_add;
            $invoiceNumber = $helper->formatInvoicebyNumber($n, $d, $id_lang, $id_shop);
        } else {
            $invoiceNumber = Configuration::get('PS_INVOICE_PREFIX', $id_lang, null, $id_shop)
        ...
        $discountTaxIncl=Tools::displayPrice($this->order->total_discounts_tax_incl, $orderIdCurrency);
        $html = str_replace("[total_discounts_tax_incl]", $discountTaxIncl, $html);
        $discountTaxExcl=Tools::displayPrice($this->order->total_discounts_tax_excl, $orderIdCurrency);
        ...
        if ($showShippingInProductList=="Y") {
            ...
            $shipping["original_product_price"] = $this->order_invoice->total_shipping_tax_excl;
    ...
    public function getFilename()
    {
        ...
        $number = $this->order_invoice->number;
        $date_add = $this->order_invoice->date_add;
        $id_lang = $this->order->id_lang;
        $id_shop = $this->order->id_shop;
        $file = $helper->formatInvoicebyNumber($number, $date_add, $id_lang, $id_shop);

...
class ba_prestashop_invoice extends Module
{
    ...
    public function getContent()
    {
        ...
        if ($taskBar == "orderinvoice") {
            ...
            $sqltabinvoicecus = "SELECT * FROM ". _DB_PREFIX_ ."ba_template_invoice_grcustumer";
            $sqltabinvoicecus .= ' WHERE id_shop = '.(int)$id_shop;
            $tabinvoicecus = Db::getInstance()->ExecuteS($sqltabinvoicecus);
            $tabinvoicecus1 = array();
            foreach ($tabinvoicecus as $vtabinvoicecus) {
                $tabinvoicecus1[$vtabinvoicecus['id_group_customer']][$vtabinvoicecus['id_lang']] = $vtabinvoicecus;
            }
    
            $sqltabdelicus = "SELECT * FROM ". _DB_PREFIX_ ."ba_template_delivery_grcustumer ";
            $sqltabdelicus .= ' WHERE id_shop = '.(int)$id_shop;
            $tabdelicus = Db::getInstance()->ExecuteS($sqltabdelicus);
            $tabdelicus1 = array();
            foreach ($tabdelicus as $vtabdelicus) {
                $tabdelicus1[$vtabdelicus['id_group_customer']][$vtabdelicus['id_lang']] = $vtabdelicus;
            }

            $sqltabcrecus = "SELECT * FROM ". _DB_PREFIX_ ."ba_template_credit_grcustumer ";
            $sqltabcrecus .= ' WHERE id_shop = '.(int)$id_shop;
            $tabcrecus = Db::getInstance()->ExecuteS($sqltabcrecus);
            $tabcrecus1 = array();
            foreach ($tabcrecus as $vtabcrecus) {
                $tabcrecus1[$vtabcrecus['id_group_customer']][$vtabcrecus['id_lang']] = $vtabcrecus;
            }

            $sqlgetinvoice = "SELECT id, name, id_lang, status";
            $sqlgetinvoice .= " FROM ". _DB_PREFIX_ ."ba_prestashop_invoice";
            $sqlgetinvoice .= ' WHERE id_shop = '.(int)$id_shop;
            $sqlgetinvoice .= ' ORDER BY id ASC';
            $bagetinvoice = Db::getInstance()->ExecuteS($sqlgetinvoice);

            $sqlgetdeli = "SELECT id, name, id_lang, status FROM ". _DB_PREFIX_ ."ba_prestashop_delivery_slip";
            $sqlgetdeli .= ' WHERE id_shop = '.(int)$id_shop;
            $sqlgetdeli .= ' ORDER BY id ASC';
            $bagetdeli = Db::getInstance()->ExecuteS($sqlgetdeli);

            $sqlgetcre = "SELECT id, name, id_lang, status FROM ". _DB_PREFIX_ ."ba_prestashop_credit_slip";
            $sqlgetcre .= ' WHERE id_shop = '.(int)$id_shop;
            $sqlgetcre .= ' ORDER BY id ASC';
            ...
            $this->smarty->assign('languages_name', $languages_name);
    ...
    public function saveCustomSetting()
    {
        ...
        foreach ($baidtempinvoice as $kbaidtempinvoice => $vbaidtempinvoice) {
            foreach ($vbaidtempinvoice as $id_lang2 => $id_invoice) {
                $data = array(
                    'id_template_invoice' => (int) $id_invoice,
                    'id_shop' => (int) $id_shop,
                    'id_lang' => (int) $id_lang2,
                    'id_group_customer' => (int) $kbaidtempinvoice,
                );
                $addinvoicegrcs = "UPDATE ". _DB_PREFIX_ ."ba_template_invoice_grcustumer SET id_template_invoice = ";
                $addinvoicegrcs .= (int)$id_invoice;
                
                $where = " WHERE id_shop = ".(int)$id_shop." AND id_lang = ";
                $where .= (int)$id_lang2." AND id_group_customer = ".(int)$kbaidtempinvoice;
                
                $select = 'SELECT id FROM '._DB_PREFIX_."ba_template_invoice_grcustumer";
                $select .= $where;
                $id = $db->getValue($select);
                if (!empty($id)) {
                    $db->query($addinvoicegrcs.$where);
                } else {
                    $db->insert('ba_template_invoice_grcustumer', $data);
                }
            }
        };

        foreach ($baidtempdeli as $kbaidtempdeli => $vbaidtempdeli) {
            foreach ($vbaidtempdeli as $id_lang2 => $id_delivery) {
                $data = array(
                    'id_template_delivery' => (int) $id_delivery,
                    'id_shop' => (int) $id_shop,
                    'id_lang' => (int) $id_lang2,
                    'id_group_customer' => (int) $kbaidtempdeli,
                );
                $sql = "UPDATE ". _DB_PREFIX_ ."ba_template_delivery_grcustumer SET id_template_delivery = ";
                $sql .= (int)$id_delivery;
                
                $where = " WHERE id_shop = ".(int)$id_shop." AND id_lang = ";
                $where .= (int)$id_lang2." AND id_group_customer = ".(int)$kbaidtempdeli;
                
                $select = 'SELECT id FROM '._DB_PREFIX_."ba_template_delivery_grcustumer";
                $select .= $where;
                $id = $db->getValue($select);
                if (!empty($id)) {
                    $db->query($sql.$where);
                } else {
                    $db->insert('ba_template_delivery_grcustumer', $data);
                }
            }
        };

        foreach ($baidtempcre as $kbaidtempcre => $vbaidtempcre) {
            foreach ($vbaidtempcre as $id_lang2 => $id_credit) {
                $data = array(
                    'id_template_credit' => (int) $id_credit,
                    'id_shop' => (int) $id_shop,
                    'id_lang' => (int) $id_lang2,
                    'id_group_customer' => (int) $kbaidtempcre,
                );
                $sql = "UPDATE ". _DB_PREFIX_ ."ba_template_credit_grcustumer SET id_template_credit = ";
                $sql .= (int)$id_credit;
                
                $where = " WHERE id_shop = ".(int)$id_shop." AND id_lang = ";
                $where .= (int)$id_lang2." AND id_group_customer = ".(int)$kbaidtempcre;
                
                $select = 'SELECT id FROM '._DB_PREFIX_."ba_template_credit_grcustumer";
                $select .= $where;
                $id = $db->getValue($select);
                if (!empty($id)) {
                    $db->query($sql.$where);
                } else {
                    $db->insert('ba_template_credit_grcustumer', $data);
                }
            }
        };
        $lang = new language($id_lang);

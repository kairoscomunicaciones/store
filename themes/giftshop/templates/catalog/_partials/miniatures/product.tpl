...
*}
<article class="{if isset($is) && $is > 8}product_miniature_hidden {/if}product-miniature js-product-miniature" data-id-product="{$product.id_product|escape:'html':'UTF-8'}" data-id-product-attribute="{$product.id_product_attribute|escape:'html':'UTF-8'}" itemscope itemtype="http://schema.org/Product">
  <div class="thumbnail-container">
    ...
    <div class="product-description">
      ...
      {if isset($product.description_short) && $product.description_short !=''}
        <div class="short_description">{$product.description_short nofilter}</div>
      {/if}
              ...
              {*<form action="{if isset($urls)}{$urls.pages.cart|escape:'html':'UTF-8'}{/if}" method="post">
               ...
               <input name="qty" type="hidden" class="form-control atc_qty" value="1" onfocus="if(this.value == '1') this.value = '';" onblur="if(this.value == '') this.value = '1';"/>
                {if $product.is_virtual}
                    <button class="add_to_cart btn btn-primary" onclick="location.href = '{$product.link}';">
                        <i class="fa fa-shopping-cart"></i>{l s='Reserve Ahora' d='Shop.Theme.Actions'}
                    </button>
                {else}
        ...
        {foreach from=$product.flags item=flag}
            {if $flag.type == 'new'}
              <li class="{$flag.type|escape:'html':'UTF-8'}">
                <span>{l s='New' d='Shop.Theme.Actions'}</span>
              </li>
            {elseif $flag.type == 'on-sale'}
              <li class="{$flag.type|escape:'html':'UTF-8'}">
                <span>{l s='Sale' d='Shop.Theme.Actions'}</span>
              </li>
            {/if}
        {/foreach}
      </ul>

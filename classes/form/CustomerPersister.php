...
class CustomerPersisterCore
{
	...
	$customer->validator =  $_POST['other'];
	$sql ='select validator from validacion';
        $result =Db::getInstance()->executes($sql);
       
        $code  =array_column($result,'validator');
        
        if(!in_array($nid, $code)){
                $register = "fail";
        }
        /*if ('V 12445678' != $national_id && 'V 98765432' != $national_id) {
                $register = "fail";
	}*/
      /*  $telephone_code = substr($telephone,0,4);
        ...
        if($register == 'fail'){
                 $customer->active= 0;
                 $customer->update();
                 $this->sendAdditionalCustomerMail($customer);
            	 Tools::redirect('additionalcustomer');
        }
        else{
           $customer->validator =  $_POST['other'];
	$ok = $customer->save();
    ...
    private function sendAdditionalCustomerMail(Customer $customer)
    {
	$to = Configuration::get('PS_SHOP_EMAIL');
        ...
        return Mail::Send(
            ...
            [
                ...
                '{telephone}' => $customer->telephone,
                '{national_id}' => $customer->validator,
            ],
            $to,
            $to_name
        );
